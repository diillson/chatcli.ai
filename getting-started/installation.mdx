...
<Tabs>
  ...
  <Tab title="xAI (Grok)">
    ...
    # (Optional) Model — default: grok-4.3
    # XAI_MODEL="grok-4.3"
    ```
  </Tab>
  <Tab title="AWS Bedrock">
    ...
    # Credentials come from the AWS SDK chain — pick ONE source:
    AWS_PROFILE="my-profile"
    # AWS_ACCESS_KEY_ID="AKIAxxxxxxxxxxxxxxxx"
    # AWS_SECRET_ACCESS_KEY="xxxxxxxxxxxxxxxxxxxxxxxx"

    # Region for Bedrock (falls back to AWS_REGION)
    BEDROCK_REGION="us-east-1"

    # (Optional) Default model — catalog aliases work
    # BEDROCK_MODEL="claude-opus-4-8"
    ```

    <Info>
    Bedrock **does not use an API key** — authentication is the AWS SDK credential chain: env vars → `~/.aws` profiles (SSO, assume-role) → IAM role (EC2/ECS/EKS). For SSO, VPC endpoints, schema overrides and inference profiles, see [AWS Bedrock](/features/bedrock).
    </Info>
  </Tab>
  ...
  <Tab title="ZAI (Zhipu AI)">
    ...
    # (Optional) GLM Coding Plan subscribers: same key, subscription endpoint
  ...
  <Tab title="GitHub Copilot">
    ...
    # (Optional) Model — e.g. gpt-4o, claude-sonnet-4, gemini-2.5-flash
    # COPILOT_MODEL="gpt-4o"
    ```

    <Info>
    Prefer OAuth? Skip the token and run `/auth login github-copilot` (Device Flow) — see the **OAuth** tab.
    </Info>
  </Tab>
  <Tab title="GitHub Models">
    ...
    # (Optional) Dedicated token instead of GITHUB_TOKEN
    # GITHUB_MODELS_TOKEN="github_pat_xxxxxxxxxxxx"
    ...
    <Info>
    Works with a regular GitHub Personal Access Token — no extra signup. See [GitHub Models](/features/github-models) for the model catalog and rate limits.
    </Info>
  </Tab>
  <Tab title="Devin CLI">
    ```env
    ...
    # (Optional) Model — default: claude-sonnet-4.6 (slugs use dots)
    # DEVIN_MODEL="claude-sonnet-4.6"

    # (Optional) Explicit binary path — default: devin from PATH
    # DEVIN_CLI_PATH="/usr/local/bin/devin"

    # (Optional) Value for the CLI's --respect-workspace-trust — default: false
    # Turns run in a throwaway temp directory the CLI cannot raise a trust
    # prompt about, so ChatCLI waives the check. true restores the CLI default.
    # DEVIN_CLI_RESPECT_WORKSPACE_TRUST="false"
    ```

    <Info>
    No API key in ChatCLI — authentication belongs to the Devin CLI itself (`devin auth login`). ChatCLI drives the local `devin` binary purely as a model transport; see [Devin Provider](/features/devin-provider).
    </Info>
  </Tab>
