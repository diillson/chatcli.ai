...
Set `CHATCLI_AUTO_UPDATE=auto` to stage updates silently in the background: the next start opens on the new version. The default (`notify`) shows an update hint on the welcome screen. See [Auto-Update](/features/auto-update) for the full picture.
