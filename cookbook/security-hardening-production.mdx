...
<Note>
Este Secret é **diferente** do `chatcli-api-keys` consumido pelo chatcli server (que carrega `OPENAI_API_KEY`, `ANTHROPIC_API_KEY` etc. via `Instance.spec.apiKeys.name`). Veja [Segurança — Autenticação](/features/security#auten%C3%A7%C3%A3o-do-operator) para a tabela comparativa.
</Note>
