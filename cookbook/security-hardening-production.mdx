...
<Warning>
In Kubernetes, leaving `bindAddress` unset means the server listens on
**every interface**, not on loopback. A reachable server with no credential
would admit every caller as an administrator, so it refuses to start and
the operator does not provision the Deployment — it raises the
`AuthenticationConfigured` condition on the Instance instead.

Set `server.token`, `security.jwtSecretRef` (HS256), `security.jwtPublicKey`
(RS256) or `security.tlsClientCA` (mutual TLS; callers are then identified by
their certificate with the role in `security.mtlsRole`), or bind loopback with
`security.bindAddress: "127.0.0.1"` when the server is only reached from
inside its own pod.
</Warning>
...
```bash
# Equivalent environment variable — the audit trail is a path, not a flag:
# setting it turns the trail on and says where it is written.
...
<Warning>
ChatCLI does not verify plugin signatures. Signing the artifact as shown
above documents its provenance for your own distribution process, and
nothing in the CLI checks it at load time — treat every plugin you install
as code you are choosing to run.

What the CLI does enforce is that a plugin subprocess never inherits your
secrets: the encryption key, the JWT secret, the auth directory and the
audit path are stripped from its environment, and anything it needs must
be passed explicitly. Install plugins from sources you control.
</Warning>
...
# values-security.yaml (partial)
env:
  # NOTE: this variable ADDS to the 17 built-in kinds, it does not replace
  # them, and kinds match by exact spelling ("Deployment", not "deployments").
  # To genuinely restrict the operator, restrict its RBAC Role instead.
...
| Setting | Effect |
|---------|--------|
| `CHATCLI_ALLOWED_RESOURCE_TYPES` | Adds kinds to the 17 the operator may act on (it does not narrow them — use RBAC for that) |
| `CHATCLI_ALLOWED_DIAGNOSTIC_COMMANDS` | Limits which diagnostic verbs it may run |
