...
**ErrShellPolicyDeny** — o comando (ou um padrão que casa com ele) está na denylist em `~/.chatcli/coder_policy.json`. `--i-know` NÃO tira essa rejeição; denylist é autoritativa. Remova ou edite a regra via `/coder deny forget <pattern>` ou edite o JSON direto.
...
1. Pré-aprovar este job específico com `--i-know`:
   ...
   ```
2. Adicionar permanentemente à allowlist (via `/coder allow forever <pattern>` no modo coder, ou editar `coder_policy.json`).
