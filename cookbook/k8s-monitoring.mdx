...
<Info>
Em produção, habilite TLS no `server` (campo `tls:` com `secretName`). O Secret precisa conter `tls.crt`, `tls.key` e `ca.crt` com SANs corretos — o passo-a-passo está em [AIOps Production Setup §2.1](/cookbook/aiops-production-setup#2-1-criar-o-secret-chatcli-tls-corretamente-sans-ca).
</Info>
