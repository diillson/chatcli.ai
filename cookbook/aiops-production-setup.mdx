...
O §2.5 acima cobre o caso self-signed gerado na mão, que é o mais frágil. Com cert-manager ou AWS ACM o setup simplifica, mas cada emissor tem pegadinha própria:
...
| Self-signed (openssl manual — §2.5) | **Sim** — `ca.crt=tls.crt` (o cert é seu próprio CA) | Definido via `subjectAltName` no `openssl.cnf` | Service in-cluster |
