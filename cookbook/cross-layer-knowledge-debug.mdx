...
Um incidente raramente mora numa camada só. O pod não sobe — é o **manifesto do Argo**? o **node group do Terraform**? o **health check no código**? Esta receita indexa as três camadas como [knowledge bases](/features/knowledge-base) separadas com [`@docs-flatten kind=code`](/features/knowledge-base) e deixa o agente **cruzar** as bases para achar a causa raiz.
...
<Note>
Nenhuma API key é necessária: o knowledge mode usa BM25 puro-Go por padrão. Embeddings ([Voyage/OpenAI/Bedrock](/features/quality/rag-hyde)) são um boost opcional.
</Note>
...
<Tip>
Não precisa dizer "isto é código" para cada repo: o agente escolhe `kind=code` sozinho pela intenção e por um hint auto-corretivo. Veja [Knowledge Base](/features/knowledge-base).
</Tip>

## Próximos passos

<CardGroup cols={2}>
  <Card title="Knowledge Base" icon="book-open" href="/features/knowledge-base"></Card>
  <Card title="Contextos Persistentes" icon="box-archive" href="/features/persistent-context"></Card>
  <Card title="Coder Mode" icon="code" href="/features/coder-plugin"></Card>
  <Card title="Monitoramento K8s" icon="dharmachakra" href="/cookbook/k8s-monitoring"></Card>
</CardGroup>
