...
The i18n system holds **1387 translation keys** covering all user-facing strings (parity across `en.json`, `en-US.json`, and `pt-BR.json`). Every new user-facing string introduced in code goes through `i18n.T()` wrapping.
...
| `cmd.*` | CLI commands (`/auth`, `/connect`, `/switch`, ...) |
