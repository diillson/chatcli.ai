...
The **`@session`** tool lets the agent **recall earlier saved conversations** — "what did we decide about the cache last week?". Implemented natively over the existing [SessionManager](/en/features/session-management). Inspired by hermes-agent's `session_search`.
...
- Pair with [Session Management](/en/features/session-management) (`/session save`) to build a searchable history.
