...
| `CHATCLI_BIND_ADDRESS` | Server bind address. Changed from `0.0.0.0` to `127.0.0.1` for security. | `127.0.0.1` |

<Warning>
gRPC reflection now requires **two conditions**: the `--grpc-reflection` flag **AND** the `CHATCLI_GRPC_REFLECTION=true` variable. This prevents accidental exposure in production. See the [security documentation](/en/features/security) for all hardening measures.
</Warning>

<Note>
The default bind address has changed from `0.0.0.0` to `127.0.0.1`. To expose the server on the network, explicitly set `CHATCLI_BIND_ADDRESS=0.0.0.0`.
</Note>
...
CHATCLI_BIND_ADDRESS=127.0.0.1
