...
description: "Tool calls via native OpenAI, Anthropic, ZAI, MiniMax, and OpenRouter APIs, with cache_control support for KV cache optimization."
icon: "wrench"
---

ChatCLI supports **tool calls via native API** for OpenAI, Anthropic, ZAI (Zhipu AI), MiniMax, and OpenRouter, replacing the XML-embedded-in-prompt approach with structured API calls. This improves accuracy, reduces token consumption, and enables cache optimizations.
...
| Compatibility | Any provider | OpenAI, Anthropic, ZAI, MiniMax, OpenRouter (others continue with XML) |
