...
Send a **voice message** on Telegram and the answer comes back as a **voice note** — transcription on the way in ([local-first whisper](/en/features/chat-gateway)), synthesis on the way out, with **any** configured TTS backend. Everything here is **provider-agnostic**: from macOS `say` to the [embedded Kokoro engine](/en/features/text-to-speech).
