...
`/config security` only **reads** the values; it does not edit rules. The practical paths today:

1. **Through the `/coder` interactive prompt** — when you run something in coder mode and it asks "Allow once / Allow always / Deny forever", choosing **Allow always** persists an `allow` rule to `~/.chatcli/coder_policy.json`. **Deny forever** persists a `deny`. This is the idiomatic path: run the action once interactively, approve it, and from there on the scheduler recognizes it.

2. **Edit the JSON directly** — either global (`~/.chatcli/coder_policy.json`) or per-project (`<root>/coder_policy.json`):
   ```json
   {
     "rules": [
   ...
   ```
   Patterns use prefix matching on the full command as the PolicyManager sees it: `@coder exec <cmd>`. Deny wins over allow. Restart the session or wait for the next fire — the scheduler reloads the file on every `RunShell`.

<Info>
A `/coder policy {list,add,remove}` command to edit rules without opening JSON does not exist yet. Within the scope of this feature, the expected routine is: if you want to schedule `my-tool --backup` outside the allowlist, either (a) run `/coder` once interactively to approve, (b) use `--i-know` on `/schedule` as a per-job pre-authorization persisted in the job record, or (c) edit the JSON.
</Info>
