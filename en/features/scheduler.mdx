...
Each action passes through its own security door:
- `shell` → CLIBridge.RunShell → CoderMode allowlist from `/config security`.
- `webhook` → http.Client with timeout and max response size.
- CoderMode bypass requires `CHATCLI_SCHEDULER_SHELL_ALLOW_BYPASS=true` **and** `--bypass_safety` in the spec.
