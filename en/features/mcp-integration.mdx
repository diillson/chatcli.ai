...
Beyond the minimum (`name`, `command`, `args`, `env`, `transport`, `enabled`, `overrides`), each server entry accepts an opt-in set of fields covering the same cases Claude Code, Cline, Cursor, and the AWS EKS MCP server support — tool auto-approval, `enabledTools`/`disabledTools` filtering, per-server timeouts, working directory, custom HTTP headers, and SSE authentication (`bearer`/`basic`/`header`), plus metadata (`description`, `tags`, `category`) shown in `/mcp status`.
...
<CardGroup cols={2}>
  <Card title="MCP Channels" icon="message" href="/en/features/mcp-channels">
    Push messages from MCP servers (stdio/SSE) to the CLI in real time.
  </Card>
