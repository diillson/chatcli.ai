...
sequenceDiagram
    ...
    alt Standard mode (Runbook)
        RR->>K8s: Executes actions (19 types: Scale/Restart/Rollback/HelmRollback/ArgoSync/AdjustHPA/...)
    else Agentic mode
...
**Supported Actions (19 types):**

| Category | Type | What It Does | Key Parameters |
|----------|------|-------------|----------------|
| Workload | `ScaleDeployment` | Adjusts replicas | `replicas` |
| Workload | `RestartDeployment` | Rollout restart | -- |
| Workload | `RollbackDeployment` | Rollback to previous/healthy/specific revision | `toRevision` |
| Workload | `PatchConfig` | Updates ConfigMap | `configmap`, `key=value` |
| Workload | `AdjustResources` | Adjusts CPU/memory | `memory_limit`, `cpu_limit`, etc. |
| Workload | `DeletePod` | Removes the sickest pod | `pod` (auto-selects) |
| Workload | `RestartStatefulSetPod` | Restart StatefulSet pod | `pod` (or rolling restart) |
| GitOps | `HelmRollback` | Rollback Helm release | `revision` |
| GitOps | `ArgoSyncApp` | Trigger ArgoCD sync | `revision` |
| Autoscaling | `AdjustHPA` | Modifies HPA min/max/target | `minReplicas`, `maxReplicas` |
| Infra | `CordonNode` | Marks node unschedulable | `node` |
| Infra | `DrainNode` | Evicts pods from node | `node` |
| Storage | `ResizePVC` | Expands PVC | `pvc`, `size` |
| Security | `RotateSecret` | Updates Secret | `secret`, `key=value` |
| Networking | `UpdateIngress` | Modifies Ingress | `ingress`, `backendService` |
| Networking | `PatchNetworkPolicy` | Opens port in NetworkPolicy | `networkPolicy`, `allowPort` |
| Advanced | `ApplyManifest` | Applies manifest from ConfigMap | `configmap`, `key` |
| Advanced | `ExecDiagnostic` | Whitelisted diagnostic command | `command` |
| -- | `Custom` | **Blocked** -- requires manual approval | -- |

<Warning>
**Safety Checks (pre-execution):** Scale to 0 replicas is blocked. AdjustResources limit cannot be less than request. DeletePod refuses if only 1 pod exists. Custom actions are blocked. Blast radius prediction checks PDB violations, resource quotas, and affected services before execution.

**Automatic Rollback (post-failure):** Before any action, a structured `ResourceSnapshot` captures the complete resource state (replicas, images, CPU/memory, HPA). If an action fails or the health verification expires (90s), the `RollbackEngine` automatically restores the resource to the pre-remediation state. The plan transitions to `RolledBack` and `PostFailureHealthy` confirms whether the resource is healthy. Works for Deployments, StatefulSets, DaemonSets, Nodes, and HPAs.
</Warning>
...
The operator has 110 tests (146 with subtests) covering all components:
...
<Steps>
  ...
  <Step title="Install operator RBAC">
