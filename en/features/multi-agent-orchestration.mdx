...
| **tool_call** | `<tool_call name="@coder" args="..." />` | Quick fixes, error diagnosis, point patches, post-agent validation |
...
In addition to custom agents via persona (loaded automatically), the Registry system allows programmatic extension:

```go
// Register a custom agent via code
registry.Register(myCustomAgent)

// Replace a builtin agent
registry.Unregister(workers.AgentTypeFile)
registry.Register(myBetterFileAgent)
```

Any type that implements the `WorkerAgent` interface can be registered:
