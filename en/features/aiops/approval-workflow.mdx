...
| `approverGroups` | []string | No | Groups that can approve (when empty, any user) |
...
<Tabs>
  ...
  <Tab title="quorum">
    **Quorum**: Requires approval from `requiredApprovers` people from **different** groups (if `approverGroups` is defined). Ensures that approval does not depend on a single team.
    ...
    In this example, at least 2 approvers are needed, from different teams (`sre-team` AND `platform-engineering`).
  </Tab>
</Tabs>
