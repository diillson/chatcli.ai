...
The embedded engine is fully self-contained, but rasterizes with a **bundled font** and no cairo/pango — fonts like `Helvetica`/`Menlo` fall back and text-heavy diagrams look plainer. A system `dot` uses **fontconfig + the OS fonts + cairo** for crisper text and better layout. Under `auto`, if a system `dot` render fails there is a **transparent fallback** to the embedded engine.
