...
| **Chat** | ✅ (controlled exception) | Chat is tool-less by design; the exception opens **only** for `ask_user` and the [read-only knowledge base query](/en/features/knowledge-base), never for execution tools |
...
Chat mode is **tool-less by design** — it never executes tools. There are two sanctioned exceptions, both harmless by construction: `ask_user` (interactive) and the [read-only knowledge base query](/en/features/knowledge-base) (`/config chat knowledge`). When enabled, chat offers **exclusively** those tools (no file read/write/exec tools).
