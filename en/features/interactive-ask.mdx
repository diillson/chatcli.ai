...
| **Chat** | ✅ (controlled exception) | Chat is tool-less by design; the exception opens **only** for `ask_user`, the [read-only knowledge base query](/en/features/knowledge-base), [`@graphview`](/en/features/graphview) and [memory/profile](/en/features/bootstrap-memory) — never for execution tools |
...
Chat mode is **tool-less by design** — it never executes tools. There are four sanctioned exceptions, all harmless by construction: `ask_user` (interactive), the [read-only knowledge base query](/en/features/knowledge-base) (`/config chat knowledge`), [`@graphview`](/en/features/graphview) (render-once, `/config chat graphview`) and [memory/profile](/en/features/bootstrap-memory) (`/config chat memory` — persists durable facts you reveal, in the same turn). When enabled, chat offers **exclusively** those tools (no file read/write/exec tools).
