...
The file contains access tokens, refresh tokens, and metadata for each provider. Tokens are automatically renewed when they expire. Additionally, if a request fails with a 401 error (expired token), ChatCLI automatically attempts to refresh the token using the refresh token, recreates the client, and retries the request — all transparently to the user.
...
<AccordionGroup>
  ...
  <Accordion title="Expired token">
    ...
    - **Proactively**: when resolving credentials, if the token is expired, a refresh is attempted before the request.
