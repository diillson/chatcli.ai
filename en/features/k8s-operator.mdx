...
|-------|------|:--------:|-------------|
| `name` | string | **Yes** | Provider name: OPENAI, OPENAI_ASSISTANT, CLAUDEAI, BEDROCK, GOOGLEAI, XAI, ZAI, MINIMAX, OPENROUTER, STACKSPOT, OLLAMA, COPILOT, GITHUB_MODELS |
...
`ExecDiagnostic` does an exact-string match against a short read-only command allowlist. Any variation (different flags, alternate host, etc.) is rejected with `command "..." not in approved diagnostic commands whitelist`.

**Default approved commands (~30):**

| Category | Commands |
|----------|----------|
| **Process / shell** | `env`, `whoami`, `id`, `uname -a`, `ps aux`, `ps -ef` |
| **Filesystem / resources** | `df -h`, `free -m`, `free -h`, `mount`, `uptime` |
| **Network (read-only)** | `netstat -tlnp`, `netstat -an`, `ss -tlnp`, `ss -an`, `ip addr`, `ip route`, `ifconfig` |
| **DNS / resolver** | `cat /etc/hosts`, `cat /etc/resolv.conf`, `nslookup kubernetes.default.svc.cluster.local` |
| **Local health probes** | `curl -s localhost{,:8080,:9090}/{health,healthz,ready,readyz,metrics,-/ready}` |
