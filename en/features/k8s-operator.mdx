...
The operator has its own dedicated Helm chart at `deploy/helm/chatcli-operator/`. A single command installs everything: **17 CRDs + RBAC + Deployment + Service + Dashboard**.
...
graph TD
    ...
    subgraph "Operator"
        ...
        PLAN -->|RemediationReconciler<br/>executes| ACTIONS[19 action types:<br/>Scale / Restart / Rollback / Patch<br/>HelmRollback / ArgoSync / AdjustHPA<br/>CordonNode / DrainNode / ResizePVC...]
...
| **7. PostMortem** | IssueReconciler | **All remediations** (not just agentic) generate PostMortem CR with timeline, root cause, lessons, **metrics**, **git correlation**, **cascade chain**, **trending** (recurring incidents), **dev feedback**. Agentic remediation also generates reusable Runbook. |
...
| `disk_pressure` | Disk pressure on node |
...
| `action` | string | Action type (19 action types available -- see [Action Types](#action-types-19-types)) |
...
# Tests (110 functions, 146 with subtests)
go test ./... -v
