...
| `BEDROCK_REGION` / `AWS_REGION` / `AWS_PROFILE` / AWS credentials | per chat Bedrock | — | Required for `provider=bedrock` — reuses the same chain as the [AWS Bedrock](/en/features/bedrock) feature (IAM role, SSO, assume-role, profile). |

<Tip>
**Bedrock embeddings** support Titan (single text per call — parallelized internally with an 8-worker pool) and Cohere v3 (native batch). Family dispatch is automatic from the model id prefix. See [RAG + HyDE](/en/features/quality/rag-hyde) for the full architecture.
</Tip>
