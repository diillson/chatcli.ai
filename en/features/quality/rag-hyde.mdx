...
description: "Two retrieval layers over memory.Fact: Phase 3a generates an LLM hypothesis and extracts keywords to expand the query; Phase 3b adds vector embeddings (Voyage/OpenAI) + pure-Go cosine search. Lazy backfill, JSON-persisted, zero CGO."
icon: "database"
---
...
| `CHATCLI_EMBED_PROVIDER` | — | `voyage\|openai\|null` — **single source of truth** for provider selection |
| `CHATCLI_EMBED_MODEL` | provider default | E.g. `voyage-3`, `text-embedding-3-small`, `text-embedding-3-large` |
| `CHATCLI_EMBED_DIMENSIONS` | model native | OpenAI only; set only to truncate via Matryoshka |
...
<Warning>**Privacy**: the user query is sent to the embedding provider. For sensitive workloads, consider self-hosting an embedding model (roadmap: Ollama-embedding provider).</Warning>
