...
description: "Use Anthropic Claude models hosted on AWS Bedrock with authentication via IAM, AWS_PROFILE, or ~/.aws/credentials."
icon: "aws"
---

ChatCLI supports **AWS Bedrock** as a native provider (`BEDROCK`), letting you invoke Anthropic Claude models directly through your AWS account — no Anthropic API key required. Ideal for corporate environments that already manage billing, compliance, and access control through AWS.
...
<CardGroup cols={2}>
  ...
  <Card title="Full Claude lineup" icon="layer-group">
    Claude 3, 3.5, 3.7, 4, 4.1, 4.5, 4.6 — all invokable via inference profiles.
  </Card>
