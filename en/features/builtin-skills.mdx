...
ChatCLI now **embeds a set of essential skills inside the binary** and seeds them into `~/.chatcli/skills/` on startup. Previously skills only existed in the project's `.agent/skills/` (visible only inside the repo checkout) or via [skills.sh](/en/features/skill-registry) (opt-in remote install) — a user installing via `go install`/brew had **no skills at all**. Now they ship by default.
...
**Meta**
`skill-authoring` — teaches the agent to [author and evolve its own skills](/en/features/skill-authoring)
...
- [Skill Authoring (@skill)](/en/features/skill-authoring) — create, evolve, stats and packs
- [Skill Registry (skills.sh)](/en/features/skill-registry) — install community skills
- [Customizable Agents](/en/features/customizable-agents)
