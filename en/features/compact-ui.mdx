...
<Tip>
**`CHATCLI_CODER_UI` and [`CHATCLI_TURNUI`](/en/features/split-pane-ui) are orthogonal.** `CHATCLI_CODER_UI` controls the **card / inline style** for the agent; `CHATCLI_TURNUI` controls the **terminal architecture** (pinned status row on top, pinned input at the bottom, scroll region in the middle). Combine `CHATCLI_CODER_UI=compact CHATCLI_TURNUI=on` for the most minimalist experience possible, similar to Claude Code.
</Tip>
