...
description: "Turn documentation corpora into a knowledge base: a ~900-token index card in the prompt, hybrid BM25 + embeddings retrieval per turn, and the @knowledge tool for the agent to investigate — all without an API key."
icon: "book-open"
---
...
<Steps>
  ...
  <Step title="Flatten">
    `@docs-flatten` with `root=<dir>`, `repo=<git>` or `url=<site>` → produces the JSONL corpus.
  </Step>
