...
<Tabs>
  ...
  <Tab title="Multi-namespace (ClusterRoleBinding + shared ClusterRole)">
    When targets are in different namespaces, the Operator creates only a per-Instance `ClusterRoleBinding` pointing at the shared `chatcli-watcher` ClusterRole, which is pre-provisioned by the operator Helm chart (H5 hardening — see [k8s-operator#tls-and-rbac](/en/features/k8s-operator#tls-and-rbac)). The `chatcli-watcher` rules match the Role above, applied cluster-wide.
  </Tab>
</Tabs>
