...
| `CHATCLI_THEME` | Color theme for the whole interface (chat, cards, markdown, spinners): `dark` · `light`. Switch at runtime via `/config ui theme`. See the [Color Theme](#color-theme) section. | `"dark"` |
...
| Theme | When to use |
| --- | --- |
| `dark` (default) | Dark-background terminals. |
...
/config theme light            # shorthand equivalent
```

<Tip>
Autocomplete: `/config ui theme <TAB>` (or `/config theme <TAB>`) offers `dark | light`.
</Tip>
