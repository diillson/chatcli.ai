...
| `CHATCLI_TURNUI` | Opt-in for the `/coder` split-pane UI (status row pinned to the top, input row pinned to the bottom, ↑/↓ history, ←/→ navigation). Only `on` enables it; any other value keeps the legacy renderer. Requires a terminal that honors DECSTBM + ESC 7/8 (xterm, iTerm2, Hyper, Terminal.app, Alacritty). See [features/split-pane-ui](/en/features/split-pane-ui). | `""` (disabled) |
...
| `ANTHROPIC_MODEL` | The model to use. E.g.: `claude-sonnet-4-6`, `claude-opus-4-6`, `claude-sonnet-4`. | No |
