...
<Tabs>
  ...
  <Tab title="Anthropic (Claude)">
    ...
    <Info>
    `claude-opus-4-7` ships with **1M native context** (no extra flag). `claude-opus-4-6` can also use **1M context** by setting `ANTHROPIC_1MTOKENS_SONNET=true`. Different models may use distinct `anthropic-version` headers, managed automatically by the catalog.
    </Info>

    <Note>
    **Catalog order**: the 4.x entries are declared newest-first in the registry. This prevents a silent alias collision where `opus-4-5`, `opus-4-6` and `opus-4-7` (typed as shortcuts) would resolve to `claude-opus-4-20250514` (a mere 20K context) because the 4.0 entry's `opus-4` alias is a prefix of all of them. If you add a Claude 5.x in the future, keep this newest-first order.
    </Note>
  </Tab>
  <Tab title="AWS Bedrock (Claude)">
    ...
    **OpenAI GPT-OSS (open-weights)** — OpenAI models hosted on Bedrock. Use the OpenAI Chat Completions schema (auto-detected by `openai.*` prefix, or forced via `BEDROCK_PROVIDER=openai`).
    ...
    <Info>
    The dynamic listing (`/switch --model`) merges `bedrock:ListFoundationModels` and `bedrock:ListInferenceProfiles` with the static catalog above, filtered to supported families (`anthropic` and `openai`). Use the command to see exactly which models your AWS account can invoke in the configured region.
    </Info>
  </Tab>
