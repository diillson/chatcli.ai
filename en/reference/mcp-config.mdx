...
| `channels` | string[] | Allow-list of **MCP push-channels** delivered by the server. Empty/omitted = accept any channel; with a list, only literally matched channels pass the filter (exact match — no globs). Explicit `"*"` = accept all. Filtering happens at receive time (before the ring/persistence). See [MCP Channels](/en/features/mcp-channels) |
...
When an MCP server emits **push notifications** (JSON-RPC messages without `id`), ChatCLI forwards them to the [MCP Channels](/en/features/mcp-channels) ring — a durable structure that feeds the system prompt, the inbox banner, and the trigger engine.
...
Works across all three transports: `sse` (via stream), `http` (via GET listener) and `stdio` (via notifications on stdout). See [MCP Channels](/en/features/mcp-channels) for what happens **after** the filter.
...
Separate from rules, ChatCLI keeps a durable JSONL file at `~/.chatcli/mcp/channels.jsonl` with every received message. Rotates at 10 MiB to `channels.jsonl.1` (one backup). Automatic replay on boot (last 200 messages). Full details: [MCP Channels — Persistence](/en/features/mcp-channels#persistence).
