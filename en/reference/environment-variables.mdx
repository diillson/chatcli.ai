...
| Variable | Description | Default |
| --- | --- | --- |
| `CHATCLI_MOA_MODELS` | Ensemble proposers, as CSV `provider:model` (e.g. `openai:gpt-5,claudeai:claude-opus-4-8,googleai:gemini-2.5-pro`). Without it, `/moa` has no proposers and just warns. | -- |
...
| Variable | Description | Default |
| --- | --- | --- |
| `CHATCLI_TRANSCRIPTION_PROVIDER` | Pin the STT backend: `command`, `url`, `groq`, `openai` (or `auto`). | `auto` |
...
| `CHATCLI_TRANSCRIPTION_MODEL` | Model (e.g. `whisper-1` in the cloud; `base`/`small` for local whisper). | `whisper-1` |
...
<Note>
Voice notes are OGG/**Opus**; local whisper.cpp needs **ffmpeg** to decode them (cloud/self-hosted backends decode server-side). With no provider configured, the bot replies with a hint instead of dropping the audio.
</Note>
...
Synthesizes the reply to audio. **Local-first**: command → self-hosted URL → `say`/`espeak` on PATH → OpenAI → Groq → Gemini → disabled. See [Text-to-Speech](/en/features/text-to-speech).

| Variable | Description | Default |
| --- | --- | --- |
| `CHATCLI_GATEWAY_VOICE_REPLY` | Enable gateway voice replies (Telegram delivers a voice note). | `false` |
| `CHATCLI_TTS_PROVIDER` | Pin the backend: `command`/`url`/`openai`/`groq`/`google` (or `auto`). | `auto` |
...
| `CHATCLI_TTS_VOICE` | Voice (backend-dependent). | `alloy` |
| `CHATCLI_TTS_VOICE_FORMAT` | Format requested for the gateway reply (`ogg` = Telegram voice note). | `ogg` |

### Image generation (@image)

Generates images from text. **Local-first**: SD WebUI → OpenAI-compatible URL → OpenAI → Google Imagen → xAI grok-image. See [Image Generation](/en/features/image-generation).

| Variable | Description | Default |
| --- | --- | --- |
| `CHATCLI_IMAGE_PROVIDER` | Pin the backend: `sdwebui`/`url`/`openai`/`google`/`xai` (or `auto`). | `auto` |
...
| `CHATCLI_IMAGE_MODEL` | Model (e.g. `dall-e-3`, `grok-2-image`, `imagen-3.0-generate-002`). | `dall-e-3` |
| `CHATCLI_IMAGE_STEPS` | Sampling steps (SD WebUI). | `25` |

<Note>
xAI (image) and Groq (voice) use their standard keys (`XAI_API_KEY`, `GROQ_API_KEY`); Google uses `GOOGLEAI_API_KEY`/`GEMINI_API_KEY`. The `@osv`, `@session` and `@skill` tools need no variables.
</Note>
