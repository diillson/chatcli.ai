...
| `CHATCLI_ALLOWED_DIAGNOSTIC_COMMANDS` | Additional commands accepted by `ExecDiagnostic` (comma-separated, extends the ~30-entry default allowlist — see [k8s-operator#execdiagnostic-allowlist](/en/features/k8s-operator#execdiagnostic-allowlist)) | `""` (defaults only) |
