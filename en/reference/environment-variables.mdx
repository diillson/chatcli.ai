...
| `CHATCLI_GRPC_TLS_CA` | gRPC CA certificate for mTLS verification. In the operator this is only a fallback — when using the Instance CR, WatcherBridge reads `ca.crt` from the Secret referenced by `spec.server.tls.secretName` automatically (see [cookbook §2.5](/en/cookbook/aiops-production-setup#2-5-create-the-chatcli-tls-secret-correctly-sans-ca)). | `""` |
