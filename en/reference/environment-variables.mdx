...
| `CHATCLI_TURNUI` | Enables the split-pane UI (fixed status row + scroll region + always-typable input) in `/coder`. Only `on` activates; any other value (including unset) keeps the legacy single-line spinner. Requires a cooperative terminal (xterm, iTerm2, Hyper, Terminal.app, Alacritty). The GoLand integrated terminal does **not** honor the DECSTBM and cursor save/restore sequences the feature relies on. | unset (legacy) |
