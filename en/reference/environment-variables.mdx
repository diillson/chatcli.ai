...
| `CHATCLI_THEME` | Interface color theme (chat, cards, markdown, spinners). 11 themes: `dark`, `light` + 9 community palettes (dracula, nord, tokyo-night, solarized-dark/light, gruvbox, catppuccin-mocha, monokai, one-dark). Switch at runtime via `/config ui theme`. See the [Theme System](/en/features/ui-theme). | `dark` |
...
An ensemble where **several models propose** an answer in parallel and an **aggregator** synthesizes the best one. Distinct from the parallel orchestration above (which dispatches specialist *agents*): here the axis is **model/provider diversity**. Triggered by the `/moa` command. Every participant — proposers and aggregator — gets the same briefing as a chat turn (attached contexts, workspace memory, skills) and read-only tools: knowledge, CCR recall and long-term memory recall. See [Mixture-of-Agents](/en/features/mixture-of-agents).
...
| `CHATCLI_WEBFETCH_RENDER` | Headless rendering of JS pages in `@webfetch`: `auto` (heuristics detect SPA shells), `always`, `never`. See [Web Tools](/en/features/web-tools#rendering-javascript-pages-spa). | `auto` |
...
Content-aware, reversible compression of tool output, logs, diffs, JSON and prose. See [Context Compression](/en/features/context-compression).
...
Variables for the harness/quality pipeline that implements the seven LLM-agent patterns. See [full overview](/en/features/quality/overview) and [detailed configuration](/en/features/quality/configuration).
...
<Tip>
**Bedrock embeddings** support `amazon.titan-embed-text-v2:0` (default), `amazon.titan-embed-text-v1`, `cohere.embed-english-v3`, and `cohere.embed-multilingual-v3`. Titan parallelizes batches with an 8-worker pool (the API accepts only 1 text per call); Cohere v3 sends the entire batch in one call. Dispatch is automatic from the model id prefix. See [RAG + HyDE](/en/features/quality/rag-hyde) and [AWS Bedrock](/en/features/bedrock).
</Tip>
...
<Note>
The MCP subsystem also manages `~/.chatcli/mcp/` for durable state: `channels.jsonl` (durable push-notification ring, with rotation) and `triggers.json` (opt-in trigger engine rules). These paths are fixed — no environment variables override them. Details: [MCP Channels](/en/features/mcp-channels) and [MCP Config](/en/reference/mcp-config).
</Note>
...
See [Scheduler (Chronos)](/en/features/scheduler) for the full design. All boolean variables accept `enabled`/`disabled`/`true`/`false`/`1`/`0`.
...
| `CHATCLI_PARK_DIR` | Override of the `@park` snapshot directory — useful for tests and isolated environments. See [Agent Park & Resume](/en/features/agent-park) | `$XDG_CONFIG_HOME/chatcli/parked` |
...
| `CHATCLI_GRPC_TLS_CA` | gRPC CA certificate for mTLS verification. Must be an **absolute** path (relative values return an error). In the operator this is only a fallback — when using the Instance CR, WatcherBridge reads `ca.crt` from the Secret referenced by `spec.server.tls.secretName` automatically (see [cookbook §2.1](/en/cookbook/aiops-production-setup#2-1-create-the-chatcli-tls-secret-correctly-sans-ca)). | `""` |
...
Transcribes voice notes to text before the pipeline. **Local-first** selection: command → self-hosted URL → whisper CLI on PATH (auto, downloads the model) → Groq → OpenAI → disabled. See [Chat Gateway → Voice messages](/en/features/chat-gateway).

| Variable | Description | Default |
| --- | --- | --- |
| `CHATCLI_TRANSCRIPTION_PROVIDER` | Pin the STT backend: `embedded`, `command`, `url`, `groq`, `openai` (or `auto`). In `auto`, with nothing configured, it falls back to the [embedded Whisper](/en/features/chat-gateway) (one-time download at daemon startup). | `auto` |
...
Home channels for the [`@send`](/en/features/proactive-messaging) tool when the target is just the platform.
...
Synthesizes the reply to audio. **Local-first**: command → self-hosted URL → **embedded (if provisioned)** → `say`/`espeak` on PATH → OpenAI → Groq → Gemini → disabled. See [Text-to-Speech](/en/features/text-to-speech) and [Voice Replies](/en/features/voice-replies).
...
Generates images from text. **Local-first**: SD WebUI → OpenAI-compatible URL → OpenAI → Google Imagen → xAI grok-image → Bedrock. See [Image Generation](/en/features/image-generation).
...
Renders Graphviz DOT to PNG/SVG/JPG with crisp, correct text. Graphviz is **embedded** (WASM, no install); when a system `dot` is on PATH, the `auto` backend uses it for more polished output. See [Diagrams (@diagram)](/en/features/diagram).
...
Renders an interactive, Obsidian-style force-directed graph (draggable) to a self-contained HTML file opened in the browser. See [Interactive graph (@graphview)](/en/features/graphview).
...
| `CHATCLI_VISION_COMPRESS` | Compress images before sending (downscale + JPEG re-encode, pure-Go). `on`/`off`. See [Context Compression](/en/features/context-compression). | `on` |
