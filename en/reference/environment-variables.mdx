...
| `CHATCLI_SCHEDULER_ACTION_ALLOWLIST` | CSV of allowed action types | `slash_cmd,llm_prompt,agent_task,worker_dispatch,hook,noop,webhook,shell` |
...
| `CHATCLI_SCHEDULER_SHELL_ALLOW_BYPASS` | Allows `bypass_safety=true` on shell actions (skips CoderMode allowlist) | `false` |
