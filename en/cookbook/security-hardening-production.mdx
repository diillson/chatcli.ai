...
<Note>
This Secret is **different** from the `chatcli-api-keys` Secret consumed by the chatcli server (which carries `OPENAI_API_KEY`, `ANTHROPIC_API_KEY` etc. via `Instance.spec.apiKeys.name`). See [Security — Operator Authentication](/en/features/security#operator-authentication) for the comparison table.
</Note>
