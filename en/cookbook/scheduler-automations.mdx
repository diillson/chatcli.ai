...
**ErrShellPolicyDeny** — the command (or a matching pattern) is on the denylist in `~/.chatcli/coder_policy.json`. `--i-know` does NOT clear this rejection; denylist is authoritative. To remove, edit the JSON directly — there is no inline subcommand for this today.
...
1. Pre-authorize this specific job with `--i-know`:
   ...
   ```
2. Permanently add to allowlist — either run the command once via interactive `/coder` and choose **"Allow always"** on the prompt (this persists the rule in `~/.chatcli/coder_policy.json`), or edit the JSON directly. `/config security` only **reads** values, it does not edit.
