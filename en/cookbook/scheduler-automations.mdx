...
<Accordion title="Shell action blocked by CoderMode">
`CHATCLI_SCHEDULER_ACTION_ALLOWLIST` allows `shell`, but the specific command hit the coder denylist. Two options:

1. Add the command to the allowlist (`/config security` → `CHATCLI_AGENT_ALLOWLIST`).
2. Explicit opt-in bypass:
   ```bash
   export CHATCLI_SCHEDULER_SHELL_ALLOW_BYPASS=true
   # in the JSON spec: {"bypass_safety": true}
   ```
</Accordion>
</AccordionGroup>
