...
<Info>
For production, enable TLS on `server` (field `tls:` with `secretName`). The Secret must contain `tls.crt`, `tls.key` and `ca.crt` with correct SANs — full walkthrough at [AIOps Production Setup §2.1](/en/cookbook/aiops-production-setup#2-1-create-the-chatcli-tls-secret-correctly-sans-ca).
</Info>
