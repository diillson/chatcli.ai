...
<CardGroup cols={2}>
  ...
  <Card title="Agent Mode" icon="robot" href="/core-concepts/agent-mode">
    Delegate tasks with `/agent`. The AI plans, suggests commands, and executes them with your approval. Built-in safety blocks dangerous actions.
  </Card>
  ...
  <Card title="Smart Context" icon="eye" href="/core-concepts/context-commands">
    Commands like `@file`, `@git`, `@command`, `@env`, and `@history` inject context from your environment directly into the prompt.
  </Card>
  ...
  <Card title="K8s Watcher and AIOps" icon="dharmachakra" href="/features/k8s-watcher">
    Monitor Kubernetes workloads in real time. The Operator with 17 CRDs implements an autonomous AIOps platform with 19 remediation action types, log analysis, Prometheus metrics, and GitOps integration.
  </Card>
  <Card title="Conversation Control" icon="clock-rotate-left" href="/en/features/conversation-control">
    `/compact` compacts history preserving what matters. `/rewind` and Esc+Esc go back to any point in the conversation.
  </Card>
</CardGroup>
