...
`/coder` has **two independent UI axes**, controlled by separate env vars:

| Axis | Variable | What it controls |
| --- | --- | --- |
| Card style | [`CHATCLI_CODER_UI`](/en/features/compact-ui) | `full` / `minimal` / `compact` — how much detail each tool call / response takes on screen |
| Terminal architecture | [`CHATCLI_TURNUI`](/en/features/split-pane-ui) | `on` enables split-pane (pinned status + pinned input + scroll region); unset = legacy single-line spinner |

The two are **orthogonal** — any combination is valid. The most minimalist choice (Claude Code style) is `CHATCLI_CODER_UI=compact CHATCLI_TURNUI=on`. See [Compact UI](/en/features/compact-ui) and [Split-Pane UI](/en/features/split-pane-ui) for the details of each.

<Warning>
`CHATCLI_TURNUI` is **opt-in** because it depends on the terminal honoring DECSTBM + cursor save/restore. Works on iTerm2, Terminal.app, Hyper, Alacritty, Windows Terminal; **does not work** on the GoLand integrated terminal. When not enabled, the legacy renderer keeps working as before.
</Warning>
...
The list is shared with the TaskTracker via `liveTodoAdapter`, so the checkbox UI renders the same plan that appears in `<reasoning>`. Details in [Atomic Tools](/en/features/atomic-tools).
...
| `delegate --prompt "..." --tools [...]` | Delegate sub-task to a subagent with isolated context — see [Subagent Delegation](/en/features/subagent-delegation) |

<Tip>
`/coder` mode also receives the `CHATCLI_AGENT_TMPDIR` variable pointing to the [session scratch dir](/en/features/session-workspace). It is the recommended path for creating temporary scripts and intermediate files without polluting the project.
</Tip>
...
The AI receives the response and continues with `<reasoning>` + `<tool_call>` as usual.

### Multiline Responses

For complex responses, use [multiline mode](/en/features/multiline-input) with `---` (`` ``` `` is also accepted as a fence opener):
