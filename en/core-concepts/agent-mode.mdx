...
<Tip>
For a complete overview of all security measures, see the [Security and Hardening documentation](/en/features/security).
</Tip>
...
| `CHATCLI_AGENT_<NAME>_MODEL` | _(empty)_ | Model override for a specific built-in agent (e.g., `CHATCLI_AGENT_PLANNER_MODEL=claude-opus-4-6`). See [Multi-Agent](/en/features/multi-agent-orchestration#configuration). |
...
Details in [Session Workspace](/en/features/session-workspace) and [Subagent Delegation](/en/features/subagent-delegation).
