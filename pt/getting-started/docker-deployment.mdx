...
<Warning>
**Duas configurações das quais a porta publicada depende.** A porta mapeada
de um container só alcança um servidor que escute na interface do
container, então sem `CHATCLI_BIND_ADDRESS=0.0.0.0` o servidor escuta em
loopback *dentro* do container: o health check continua passando, porque
roda lá dentro, enquanto nada de fora conecta.

E um servidor alcançável de fora do container precisa de credencial — ele
recusa subir sem uma, porque senão admitiria todo chamador como
administrador. Para rodar sem token, mantenha o servidor dentro do
container com `CHATCLI_BIND_ADDRESS=127.0.0.1` e use `docker exec`.
</Warning>
...
services:
  chatcli-server:
    ...
    environment:
      ...
      # A porta publicada só chega a um servidor que escute na interface do container.
      CHATCLI_BIND_ADDRESS: "${CHATCLI_BIND_ADDRESS:-0.0.0.0}"
      # Obrigatório com o bind acima: um servidor alcançável recusa subir sem credencial.
      CHATCLI_SERVER_TOKEN: "${CHATCLI_SERVER_TOKEN:-}"

      # JWTs por usuário em vez do (ou junto com o) token compartilhado:
      # segredo HS256, ou chave pública RS256 (PEM) via CHATCLI_JWT_PUBLIC_KEY.
      ...
      # Provedor de LLM (configure pelo menos um)
      ...
      # AWS Bedrock (chaves estáticas; prefira instance role ou SSO em hosts reais)
      ...
      # Bundle de CA corporativa para toda conexão TLS de saída (caminho PEM dentro do container)
      CHATCLI_CA_BUNDLE: "${CHATCLI_CA_BUNDLE:-}"

      # Cadeia de fallback de provedores (separados por vírgula, ex.: "OPENAI,CLAUDEAI,GOOGLEAI")
...
> O container roda com filesystem **read-only** e `no-new-privileges` por padrão. O diretório `/tmp` usa tmpfs em memória (limitado a 100MB). Os volumes nomeados (`chatcli-sessions`, `chatcli-plugins`) são os únicos pontos graváveis. Veja a [documentação de segurança](/pt/features/security) para detalhes.
...
| `prometheusUrl` | Deprecado: lido só pelo operator AIOps, o servidor ignora. Defina `prometheusUrl` no chart `chatcli-operator` | `""` |
...
| Valor | Descrição | Padrão |
|-------|-----------|--------|
| `llm.provider` | Provedor padrão: `OPENAI`, `OPENAI_ASSISTANT`, `CLAUDEAI`, `BEDROCK`, `GOOGLEAI`, `XAI`, `ZAI`, `MINIMAX`, `MOONSHOT`, `STACKSPOT`, `OLLAMA`, `COPILOT`, `OPENROUTER` (`DEVIN` é um transporte de CLI local e não está disponível na imagem do servidor) | `""` |
...
| `secrets.openrouterApiKey` | Chave do OpenRouter |
| `secrets.moonshotApiKey` | Chave da Moonshot (Kimi) |
...
| Valor | Descrição | Padrão |
|-------|-----------|--------|
| `copilot.model` | Modelo padrão do Copilot (ex: `gpt-6-astra`, `gpt-4o`, `claude-sonnet-4-6`, `gemini-2.5-flash`); renderizado como `COPILOT_MODEL` | `""` |
| `copilot.maxTokens` | Máximo de tokens para resposta; renderizado como `COPILOT_MAX_TOKENS` | `""` (limite do provedor) |
| `copilot.apiBaseUrl` | Override da URL base da API para ambientes enterprise; renderizado como `COPILOT_API_BASE_URL` | `""` (`https://api.githubcopilot.com`) |
...
> Quando `readOnlyRootFilesystem` está `true`, o chart monta automaticamente um tmpfs em `/tmp` e um emptyDir em `/home/chatcli/.chatcli` (200Mi) para dados de runtime. A variável `HOME=/home/chatcli` é definida automaticamente. Para monitorar múltiplos namespaces, habilite `rbac.clusterWide: true`. Veja a [documentação de segurança](/pt/features/security) para detalhes.
>
...
stringData:
  ...
  MOONSHOT_API_KEY: "sk-xxx"       # opcional
...
| `security.bindAddress` | Endereço de bind do servidor. Auto-detecta `0.0.0.0` em Kubernetes via `KUBERNETES_SERVICE_HOST`. | `127.0.0.1` / `0.0.0.0` (K8s) |
...
| `security.jwtPublicKey` | Chave pública RSA (PEM ou caminho) para JWTs RS256; prevalece sobre o segredo HS256 | `""` |
| `security.tlsClientCA` | Caminho do bundle de CA contra o qual certificados de cliente são verificados (TLS mútuo); exige cert e key TLS | `""` |
| `security.mtlsRole` | Papel de chamadores identificados só pelo certificado de cliente (`viewer`, `user`, `admin`); renderizado como `CHATCLI_MTLS_ROLE` | `""` (user) |
...
<Warning>
Em produção, sempre configure `security.jwtSecretRef` (HS256) ou `security.jwtPublicKey` (RS256) para habilitar autenticação JWT. Um servidor escutando em toda interface recusa subir sem credencial — um `server.token` compartilhado, um segredo ou chave pública JWT, ou uma CA de cliente (`security.tlsClientCA`).
</Warning>
...
<CardGroup cols={3}>
  <Card title="Servidor" icon="server" href="/pt/features/server-mode">
    Configurar o servidor gRPC
  </Card>
  <Card title="Conexão Remota" icon="plug" href="/pt/features/remote-connect">
    Conectar ao servidor
  </Card>
  <Card title="K8s Watcher" icon="dharmachakra" href="/pt/features/k8s-watcher">
