...
<Warning>
Em Kubernetes, deixar `bindAddress` em branco faz o servidor escutar em
**toda interface**, não em loopback. Um servidor alcançável sem credencial
admitiria todo chamador como administrador, então ele recusa subir e o
operator não provisiona o Deployment — em vez disso levanta a condition
`AuthenticationConfigured` na Instance.

Defina `server.token`, `security.jwtSecretRef` (HS256), `security.jwtPublicKey`
(RS256) ou `security.tlsClientCA` (TLS mútuo; os chamadores passam a ser
identificados pelo certificado, com o papel em `security.mtlsRole`), ou use
loopback com `security.bindAddress: "127.0.0.1"` quando o servidor só é
alcançado de dentro do próprio pod.
</Warning>
...
# A trilha de auditoria é um caminho, não um sinalizador: definir a
# variável liga a trilha e diz onde ela é escrita.
...
<Warning>
O ChatCLI não verifica assinaturas de plugins. Assinar o artefato como
acima documenta a procedência no seu próprio processo de distribuição, e
nada na CLI confere isso no carregamento — trate todo plugin instalado
como código que você escolheu executar.

O que a CLI garante é que o subprocesso de um plugin nunca herda seus
segredos: a chave de criptografia, o segredo JWT, o diretório de auth e o
caminho da trilha de auditoria são removidos do ambiente dele, e o que ele
precisar tem de ser passado explicitamente. Instale plugins de fontes que
você controla.
</Warning>
...
# values-security.yaml (parcial)
env:
  # ATENÇÃO: esta variável SOMA aos 17 kinds embutidos, não substitui, e os
  # kinds casam pela grafia exata ("Deployment", não "deployments").
  # Para restringir o operator de verdade, restrinja a Role de RBAC dele.
