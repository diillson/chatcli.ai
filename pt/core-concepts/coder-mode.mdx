...
## Handoff a partir do chat

Você não precisa saber de antemão que um pedido exige o workspace. No modo chat o assistente reconhece isso (alterar código, rodar ou depurar o projeto, trabalho em várias etapas no repositório), responde brevemente com o seu entendimento e propõe a troca por conta própria:

```
  🛠  O assistente propõe mudar para o modo coder com esta tarefa:
     Corrigir os testes do parser que falham em cli/parser_test.go
     Enter (ou y) entra no modo coder, n permanece no chat; qualquer outro texto continua a conversa.
```

A sua próxima entrada é a resposta: **Enter** (ou `y`, `yes`, `s`, `sim`, `ok`) entra no modo coder com aquela tarefa, exatamente como se você tivesse digitado `/coder <tarefa>`; `n` / `não` permanece no chat; qualquer outro texto é um turno normal de chat e a proposta é descartada. Texto puro nunca muda de modo sozinho — você sempre confirma.

A proposta só acontece numa sessão interativa atendida; o gateway, MCP/ACP e o modo one-shot (`-p`) nunca a recebem. Defina `CHATCLI_CHAT_CODER_HANDOFF=false` para desligar (veja [variáveis de ambiente](/pt/reference/environment-variables)).
...
<Info>
Desative com `CHATCLI_AGENT_PARALLEL_MODE=false` se necessário. Veja a [documentação completa](/pt/features/multi-agent-orchestration).
</Info>
...
| `<plan>` ausente antes de `tool_call` | "Bloco de plano obrigatório antes de qualquer tool_call" |
...
1. **Parse** da resposta — extrai `<plan>`, `<tool_call>`, blocos de código e texto solto
2. **Checagem de plano** — se `<tool_call>` existe mas `<plan>` não, rejeita
...
| **Formato de saída** | Estrito: `<plan>` + `<tool_call name="@coder">` apenas | Flexível: qualquer tool, blocos `execute`, texto livre |
...
| **Anchor de formato** | Lembrete de `<plan>` + `@coder` a cada turno | Lembrete de `tool_call` e `execute` a cada turno |
...
Além do TaskTracker (que extrai tarefas do bloco `<plan>` automaticamente), o modo agente expõe um **plugin atômico** `@todo` com paridade ao **TodoWrite** do Claude Code. A IA pode emitir explicitamente:
...
A lista é compartilhada com o TaskTracker via `liveTodoAdapter`, então o checkbox UI renderiza o mesmo plano que aparece em `<plan>`. Detalhes em [Atomic Tools](/pt/features/atomic-tools).
...
O anchor lembra:
- Formato obrigatório: `<plan>` seguido de `<tool_call name="@coder">`
...
O modo `/coder` utiliza o plugin [@coder](/pt/features/coder-plugin), que já vem **embutido no ChatCLI** — nenhuma instalação adicional necessária.
...
| `delegate --prompt "..." --tools [...]` | Delegar sub-tarefa para um subagente com contexto isolado — ver [Subagent Delegation](/pt/features/subagent-delegation) |

<Tip>
O modo `/coder` também recebe automaticamente a variável `CHATCLI_AGENT_TMPDIR` apontando para o [scratch dir da sessão](/pt/features/session-workspace). É o caminho recomendado para criar scripts temporários e arquivos intermediários sem poluir o projeto.
</Tip>
...
A IA recebe a resposta e continua com `<plan>` + `<tool_call>` normalmente.

### Respostas multilinha

Se a resposta for complexa, use o [modo multilinha](/pt/features/multiline-input) com `---` (também aceita `` ``` `` como abridor de fence):
...
|:--------|:--------------|
| Largura | Cresce até a largura **real** do terminal — sem cap de 80 colunas |
| Bracketed paste | Conteúdo multilinha colado é **preservado** (não submete na primeira newline) |
| Edição | Setas, Home/End, Ctrl+A/E, navegação por palavra (Alt/Ctrl+Setas) |
| Detecção de paste | Pastes grandes são capturados via placeholder (mesmo mecanismo do chat) e expandidos antes do envio para a IA |
| Stdin não-TTY (pipes, CI) | Fallback automático para `bufio.Reader` — o agente ainda lê uma linha por iteração |

<Tip>
Antes essa entrada era uma versão limitada do `golang.org/x/term` que travava em 80 colunas e tratava cada `\n` colado como Enter. Se você ainda vê esse comportamento, atualize o ChatCLI — a paridade com o chat vale para qualquer terminal moderno.
</Tip>
