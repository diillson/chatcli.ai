...
| --- | --- | --- |
| **Plano** | `<plan>...</plan>` | Exibido como card com ícone de bússola — as ações que a IA vai executar (a grafia antiga `<reasoning>` continua sendo lida) |
...
- **Fila type-ahead** — Mensagens digitadas pelo usuário durante o processamento da IA são enfileiradas e processadas assim que o turno atual terminar. Isso evita perda de input. Funciona em **/agent E /coder**. O spinner mostra `(N na fila)` em tempo real conforme você pressiona Enter — não é preciso esperar o turn fechar pra ver o contador subir. Uma instrução na fila nunca se perde no caminho: ela sobrevive ao prompt de resposta final (se o modelo responder sem tool calls com instrução pendente, o coder pula o "aguardando input" e vai direto para o próximo turno), a overlays interativos como o `@ask` e ao input guard dos prompts de segurança. O que você estava digitando sem ter dado Enter passa para o prompt editável. O modelo recebe o follow-up rotulado como nova instrução do usuário, com prioridade sobre o plano atual. Se a execução terminar (max turns, erro) com instruções ainda na fila, elas são mostradas de volta para você reenviar.
...
<Tip>
Para uma visão completa de todas as medidas de segurança, consulte a [documentação de Segurança e Hardening](/pt/features/security).
</Tip>
...
| `CHATCLI_AGENT_<NAME>_MODEL` | _(vazio)_ | Override de modelo para um agent embarcado específico (ex.: `CHATCLI_AGENT_PLANNER_MODEL=claude-opus-4-6`). Veja [Multi-Agent](/pt/features/multi-agent-orchestration#configura%C3%A7%C3%A3o). |
...
Detalhes em [Workspace de Sessão](/pt/features/session-workspace) e [Subagent Delegation](/pt/features/subagent-delegation).
...
<CardGroup cols={2}>
  ...
  <Card title="Workspace de Sessão" icon="folder-tree" href="/pt/features/session-workspace">
  ...
  <Card title="Gerenciamento de Sessões" icon="floppy-disk" href="/pt/features/session-management">
