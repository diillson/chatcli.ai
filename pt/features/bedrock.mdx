...
- **Converse API (default)** — schema unificado da AWS que cobre tudo o mais: OpenAI GPT-5.6 (Sol/Terra/Luna — só Converse no Bedrock), xAI Grok 4.6, Llama, Amazon Nova (incl. Nova 2 Lite), Mistral, Cohere, AI21 Jamba, DeepSeek, Stability, Writer Palmyra, Moonshot Kimi, MiniMax, Qwen, Z.AI/GLM, Google Gemma, NVIDIA Nemotron, TwelveLabs Pegasus, e qualquer provider que a AWS adicionar no futuro.
...
<CardGroup cols={2}>
  ...
  <Card title="Embeddings nativos" icon="vector-square">
    Provider de embeddings reusa a mesma cadeia de credenciais AWS. Titan v1/v2 + Cohere v3. Veja [RAG + HyDE](/pt/features/quality/rag-hyde).
  </Card>
</CardGroup>
...
- **Profile selecionado:** `BEDROCK_PROFILE` ou `AWS_PROFILE` — do ambiente do processo ou do arquivo de ambiente resolvido. `BEDROCK_PROFILE` tem precedência e limita a escolha ao ChatCLI, deixando o `AWS_PROFILE` livre para o resto do shell; ambos são honrados igualmente por chat, agent, listagem de modelos, embeddings e geração de imagem.
...
- **`aws login` (sign-in do AWS CLI v2):** perfil com `login_session` em `~/.aws/config` — esses perfis não têm material de chave em lugar nenhum
- **Assume-role / credential_process:** perfis com `role_arn` ou `credential_process` em `~/.aws/config`
- **Token cache de login:** arquivos em `~/.aws/sso/cache/` ou `~/.aws/login/cache/` (um `aws sso login` / `aws login` anterior)
- **API key do Bedrock:** `AWS_BEARER_TOKEN_BEDROCK`
...
export BEDROCK_MODEL=claude-opus-4-8  # modelo default opcional (aliases do catálogo funcionam)
chatcli
```
...
**Importante:** o AWS SDK **não** sabe qual profile está "logado". Você **precisa** indicar o profile via `BEDROCK_PROFILE`/`AWS_PROFILE` (env, arquivo de ambiente, ou flag). Se seu profile SSO se chama `default`, ele é usado automaticamente sem isso.
</Info>

<Warning>
**Rodando o ChatCLI a partir de uma IDE ou de um cliente MCP?** Eles sobem o `chatcli acp` / `chatcli mcp-server` como processo filho **sem o seu shell profile**, então um profile exportado no `.zshrc`/`.bashrc` — e o próprio `CHATCLI_DOTENV` — nunca chega neles. O arquivo de ambiente passa a ser descoberto como `./.env` → `~/.chatcli/.env` → `~/.env`; se nenhum deles tiver o seu profile, o SDK cai no profile `default` e o Bedrock responde pela conta errada, ou a cadeia de credenciais esgota no enganoso `no EC2 IMDS role found`.

Mantenha o profile em `~/.chatcli/.env` (ou `~/.env`), ou fixe-o no bloco `env` do cliente. O `/config` mostra o *Profile AWS efetivo* e a variável de onde ele veio, e os erros de credencial nomeiam o profile, o arquivo de ambiente em efeito e o comando `aws sso login` para renovar a sessão.
</Warning>
...
| `openai.gpt-oss-*`, `us.openai.gpt-oss-*`, etc. | **OpenAI Chat Completions** | `messages`, `max_completion_tokens` | Schema estável e amplamente coberto pra GPT-OSS |
| `global./us./in.openai.gpt-5.6-sol/-terra/-luna` | **Converse API** (capability `bedrock_converse_only` do catálogo) | `messages`, `system`, `inferenceConfig` | O GPT-5.6 no Bedrock fala Converse/Responses/Chat Completions, mas **não `InvokeModel`**; a flag do catálogo vence o sniff do vendor `openai.`, então não precisa de env var |
...
<Tip>
**Por que Anthropic e OpenAI ficam fora do Converse?** Anthropic mantém cache_control breakpoints e extended-thinking que mapeiam pro Converse com shape diferente — escolhemos não perturbar o cache planner que já está provado em produção. OpenAI gpt-oss roda estável no `InvokeModel` direto e a cobertura do Converse pra esses IDs varia por região. A exceção é o **GPT-5.6** (Sol/Terra/Luna), que não tem superfície `InvokeModel` no Bedrock — as entradas dele no catálogo carregam `bedrock_converse_only` e sempre vão pelo Converse. Se quiser experimentar, `BEDROCK_PROVIDER=converse` força tudo no Converse.
</Tip>
...
A geração mais nova de modelos Claude no Bedrock (**Fable 5.1, Fable 5, Opus 5.5, Opus 5, Sonnet 5, Opus 4.8, Opus 4.7**) usa **IDs dateless** — `anthropic.claude-fable-5-1`, `anthropic.claude-fable-5`, `anthropic.claude-opus-5-5`, `anthropic.claude-opus-5`, `anthropic.claude-sonnet-5`, `anthropic.claude-opus-4-8`, `anthropic.claude-opus-4-7` — **sem** IDs ARN-versionados (`...-v1:0`). No caminho `InvokeModel` (Opus 4.8/4.7) o ID dateless puro **não é invocável on-demand** — a AWS responde "retry with the ID or ARN of an inference profile" — então o ChatCLI os invoca pelo **inference profile `global.`** (`global.anthropic.claude-opus-4-8`); os modelos do endpoint Messages (Opus 5, Sonnet 5, Fable 5, Fable 5.1) usam o ID dateless puro.

**Claude Opus 5**, **Claude Sonnet 5**, **Claude Fable 5** e **Claude Fable 5.1** têm uma particularidade: são servidos **exclusivamente** pelo endpoint *Claude in Amazon Bedrock* — a Messages API em `https://bedrock-mantle.{região}.api.aws/anthropic/v1/messages`. O Opus 5 e o Sonnet 5 não existem no `InvokeModel` legado, e o Fable 5 / Fable 5.1 o rejeitam com `400 ValidationException: data retention mode 'default' is not available for this model` (eles exigem retenção de dados de 30 dias, disponível só sob o acordo *Claude in Amazon Bedrock*). O ChatCLI cuida disso sozinho:

- O catálogo marca esses modelos com a capability `bedrock_mantle_only` e o client roteia a request pro endpoint Messages automaticamente — `/switch --model claude-opus-5` (ou `claude-opus-5-5` / `claude-sonnet-5` / `claude-fable-5` / `claude-fable-5-1`) simplesmente funciona. O Fable 5.1 (`anthropic.claude-fable-5-1`, 1/set/2026) é exposto só via profiles `us.`/`global.` e tem o mesmo preço da API first-party ($10/$50, cache reads a $0,25).
...
<Note>
Opus 4.8 e Opus 4.7 continuam no `InvokeModel` por padrão (servidos pela mesma infraestrutura do endpoint Messages); use `BEDROCK_ANTHROPIC_ENDPOINT=mantle` se quiser movê-los pro endpoint novo também. Pré-requisito pra Opus 5/Sonnet 5/Fable 5/Fable 5.1: habilite o modelo em **Model access** no console do Bedrock com um data retention mode selecionado.
</Note>
...
Modelos Anthropic **da era 4.x–4.6 (4, 4.1, 4.5, 4.6) NÃO aceitam invocação on-demand direto pelo ID base** (a nova geração dateless — Fable 5.1, Fable 5, Opus 5.5, Opus 5, Sonnet 5, Opus 4.8/4.7 — não precisa de profile; veja a seção acima). Se você tentar com um modelo da era antiga, recebe:
...
<Info>
O ChatCLI já usa um inference profile global como **modelo padrão** (`global.anthropic.claude-sonnet-4-6`). A linha Claude 3.x saiu do Bedrock (o Haiku 3 chega ao EOL em 10/set/2026) e foi removida do catálogo; Sonnet 4 (EOL 14/out/2026) e Opus 4.1 (EOL 8/jan/2027) estão **Legacy**, mas continuam servidos pelos profiles `global.`/`us.`/`eu.`.
</Info>
...
export AWS_ENDPOINT_URL_BEDROCK=https://bedrock.vpc.internal   # control plane (listagem de modelos)
```
...
| :--- | :--- | :--- |
| `BEDROCK_MODEL` | Modelo default quando nenhum foi selecionado — mesmo papel do `COPILOT_MODEL`/`MOONSHOT_MODEL` nos outros providers; aliases do catálogo funcionam (`claude-opus-4-8`) | `global.anthropic.claude-sonnet-4-6` |
...
| `BEDROCK_BASE_URL` | Base URL personalizada de toda a superfície Bedrock — runtime e listagem de modelos (VPC endpoints, DNS corporativo) — paridade com o `ANTHROPIC_BEDROCK_BASE_URL` do Claude Code; vence as `AWS_ENDPOINT_URL*` | — |
| `AWS_ENDPOINT_URL_BEDROCK_RUNTIME` | Override de endpoint do Bedrock Runtime | — |
| `BEDROCK_CONTROL_BASE_URL` | Override opcional do control plane (listagem de modelos) quando ele vive em host diferente da `BEDROCK_BASE_URL` — VPC endpoints são por serviço; vence a `AWS_ENDPOINT_URL_BEDROCK` | herda `BEDROCK_BASE_URL` |
...
| `BEDROCK_ANTHROPIC_ENDPOINT` | Roteamento do wire Claude: `auto` (default — catálogo decide, com fallback Mantle→InvokeModel em falha), `mantle` (pina o endpoint Messages, sem fallback) ou `invoke` (só InvokeModel legado) | `auto` |
...
Todas essas vars aparecem no `/config providers` (chat) e `/config quality` (embeddings). Veja [Variáveis de Ambiente](/pt/reference/environment-variables) pra referência completa.
...
<AccordionGroup>
  <Accordion title="bedrock: model X requires an inference profile">
    Mensagem do ChatCLI quando você seleciona um ID base que precisa de inference profile (Claude 4.x, 4.5, 4.6 e equivalentes em outros providers). A mensagem **já sugere o caminho**:
...
| `cohere.embed-v4:0` (também profiles `us.`/`eu.`/`global.`) | Cohere v4 | 1536 (default) |
| `amazon.nova-2-multimodal-embeddings-v1:0` | Nova MME | 256 / 384 / 1024 / 3072 (default 3072) |

Reusa **a mesma cadeia de credenciais** do chat client — `BEDROCK_REGION` / `AWS_REGION` / `AWS_PROFILE` / `~/.aws/credentials` etc. Veja [RAG + HyDE](/pt/features/quality/rag-hyde) para arquitetura completa do retrieval.
...
<CardGroup cols={2}>
  <Card title="Provider Fallback" icon="arrows-rotate" href="/pt/features/provider-fallback">
    Configure failover automático entre Bedrock e outros provedores
  </Card>
  <Card title="RAG + HyDE" icon="database" href="/pt/features/quality/rag-hyde">
    Embeddings via Bedrock Titan/Cohere para retrieval semântico
  </Card>
  <Card title="Modelos Suportados" icon="list" href="/pt/reference/supported-models">
