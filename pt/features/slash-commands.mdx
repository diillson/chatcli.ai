...
| One-shot | `chatcli -p "/review-pr 1326"` expande antes do roteamento; stdin de pipe vira parte de `$ARGUMENTS`; comandos mode coder rodam pelo engine do coder |
...
```bash
git diff | chatcli -p "/review"                # corpo do pipe → $ARGUMENTS
kubectl logs deploy/api | chatcli -p "/triage" # funciona com qualquer comando
```
...
Um run via pipe não tem terminal para responder um prompt de aprovação, então linhas `!` resolvem só pela sua [política de segurança](/pt/features/coder-security). Para automatizar um comando headless — cron, CI, scripts — adicione uma regra **allow** explícita para exatamente o que ele executa em `~/.chatcli/coder_policy.json`:

```json
{
  "rules": [
    { "pattern": "exec git log", "action": "allow" }
  ]
}
```
...
```bash
chatcli --no-anim -p "/standup 3" >> standup-notes.md
```
