...
<Info>Esta dashboard vai a fundo em **um run de task graph**. Para acompanhar o runtime inteiro ao vivo (cada agent, requisição de LLM, tool, skill, servidor MCP, padrão e job em background, entre processos), use [`/dash`](/pt/features/live-dashboard); os runs de task graph também aparecem lá.</Info>
...
<Frame caption="Um run em andamento: a tarefa de design concluída, as duas de implementação terminadas (uma depois de um retry), as de verificação rodando e a de entrega esperando por elas; o feed à direita é a trilha append-only. (Dados sintéticos.)">
  <img src="/images/task-graph.png" alt="Dashboard de task graph do ChatCLI: um DAG em raias por fase com tarefas concluídas, rodando e pendentes, curvas de dependência e o feed de eventos" />
</Frame>
...
<Frame caption="O mesmo run do início ao fim: as tarefas acendem conforme o ready set avança, um gate falho manda uma tarefa de volta para a segunda tentativa, um clique abre a evidência com as duas tentativas, e a aba Results soma caminho crítico, tempos e custo. (Dados sintéticos.)">
  <img src="/images/task-graph.gif" alt="Dashboard de task graph do ChatCLI animada: tarefas passando de pendente para rodando, em revisão e concluída, um retry depois de gate falho, o popover de evidência e a aba Results" />
</Frame>
