...
| Executar 3 ações independentes em paralelo | ❌ Use [`<agent_call>` multi-agent](/pt/features/multi-agent-orchestration) — paraleliza melhor |
...
| `read_only` | boolean | Não | Se `true` (default), bloqueia `write`, `patch`, `multipatch`, `exec`. Defina `false` para conceder o conjunto completo de tools. |
...
A saída é truncada pelo mesmo sistema de [orçamento de resultados](/pt/features/tool-result-management) que aplica aos outros tools — então mesmo que o subagente seja prolixo, o impacto no contexto do pai fica limitado. A truncagem é **recuperável**: conteúdo acima do limite inline é persistido num arquivo de overflow `result_*.txt` (referenciado inline) e output comprimido reversivelmente carrega marcadores `<<ccr:KEY>>` que o pai — ou o próprio subagente — pode expandir com a tool `recall_output`.
...
| Output inline máximo devolvido ao pai | ~30 KB (texto completo persistido em arquivo de overflow, referência mantida inline) | `MaxWorkerOutputBytes` (constante interna) |
...
<Warning>
O subagente herda o **mesmo cliente LLM e modelo** do pai. Não há roteamento por modelo nessa tool — para isso, use o sistema de [skill model hints](/pt/features/customizable-agents) ou despache para um agent customizado via [`<agent_call>`](/pt/features/multi-agent-orchestration).
</Warning>
...
| Aspecto | `delegate_subagent` | [`<agent_call>`](/pt/features/multi-agent-orchestration) | Tool call inline |
|:--------|:---------------------|:----------------------------------------------|:-----------------|
...
<CardGroup cols={2}>
  <Card title="Multi-Agent" icon="users" href="/pt/features/multi-agent-orchestration">
    Para tarefas paralelas em vez de delegação isolada.
  </Card>
  <Card title="Session Workspace" icon="folder-tree" href="/pt/features/session-workspace">
