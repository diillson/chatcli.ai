...
| `agent` | O loop ReAct autônomo completo com todas as ferramentas do ChatCLI. | ✓ |
| `coder` | O loop focado em código: ler/editar arquivos, executar comandos, iterar — com plano ao vivo. | |
...
| Sessão | `/session`, `/newsession`, `/compact`, `/export` |
...
| Método | Notas |
|--------|-------|
| `initialize` | Retorna `agentInfo {name: "chatcli", version}`, capabilities (`embeddedContext: true`), sem auth. |
