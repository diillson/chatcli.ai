...
Digite `/` na caixa de prompt: a IDE autocompleta a superfície de comandos do ChatCLI (anunciada via `available_commands_update`). Três grupos:
...
- As **regras `ask`** da sua [política de segurança](/pt/features/coder-security) abrem o mesmo dialog: a regra default do `@coder exec`, operações safety-immune e qualquer regra `ask` que você criou recebem uma decisão humana de verdade — antes um run unattended as auto-aprovava.
- Qualquer coisa que não seja um *Allow* explícito — reject, dialog fechado, crash do cliente, timeout — **nega** a ação. O agente vê a recusa in-band e replaneja.
