...
| `--provider <nome>` | Sobrescreve o provedor LLM do servidor: `OPENAI`, `OPENAI_ASSISTANT`, `CLAUDEAI`, `BEDROCK`, `GOOGLEAI`, `XAI`, `ZAI`, `MINIMAX`, `MOONSHOT`, `STACKSPOT`, `OLLAMA`, `COPILOT`, `OPENROUTER`, `DEVIN` (DEVIN é um transporte de CLI local: precisa do Devin CLI no host do servidor e não está na imagem do servidor) | |
...
| `--max-tokens <int>` | Máximo de tokens na resposta (sem ela: o override `*_MAX_TOKENS` do provedor no servidor, senão o teto do catálogo do modelo) | |
...
- **Custo**: `/cost` precifica o uso real de tokens que o servidor reporta em cada resposta

---

## Streaming e Custo

As respostas chegam em streaming conforme o provedor as produz: o servidor encaminha cada fragmento pelo RPC `StreamPrompt` e o cliente renderiza conforme chega, exatamente como um provedor local faria. A mensagem final carrega o uso de tokens e o stop reason que o provedor reportou, então o cost tracker local precifica tokens reais — o `/cost` numa conexão remota mostra os mesmos números de uma sessão local, não uma estimativa por caracteres. O provedor e o modelo da resposta seguem a atribuição do servidor (a cadeia de fallback do servidor pode responder com outro provedor), a menos que você os fixe com `--provider`/`--model`.

<Note>
Uma requisição que não nomeia provedor nem modelo e não encaminha credencial é atendida pela cadeia de fallback do servidor quando há uma configurada (`CHATCLI_FALLBACK_*`), senão pelo provedor padrão do servidor. `--max-tokens` sobrescreve a escolha do servidor; sem ela o servidor aplica o override `*_MAX_TOKENS` do provedor, senão o teto do catálogo do modelo.
</Note>
...
<Info>
Para um guia completo de segurança (autenticação, hardening de containers, RBAC, etc.), veja a [documentação de segurança](/pt/features/security).
</Info>
...
chatcli connect ci-server:50051 --provider GOOGLEAI --llm-key AIzaSy-xxx \
  -p "Análise este diff: $(git diff HEAD~1)" --raw
