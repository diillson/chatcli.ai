...
── ChatCLI · Atualização ─────────────────────────────────────────
  ...
  Atual: v1.169.0 · Última: v1.173.0

── Novidades da v1.173.0 ─────────────────────────────────────────
  ...
  https://github.com/diillson/chatcli/releases/tag/v1.173.0

  Baixando v1.173.0 (chatcli-darwin-arm64)...
  ✓ Atualizado para v1.173.0 — reinicie o chatcli para usar a nova versão.
```
...
sudo curl -fsSL -o /usr/local/bin/chatcli https://github.com/diillson/chatcli/releases/download/v1.173.0/chatcli-darwin-arm64 && sudo chmod +x /usr/local/bin/chatcli
```
...
```text
  Versão: 1.173.0 (commit: 52441581)
  ✓ Atualizado automaticamente para v1.173.0
```

No modo `notify` (o default), a tela de boas-vindas mostra o convite:

```text
  ⬆ v1.173.0 disponível — rode /update para atualizar
```

O banner nunca espera uma checagem fresca: ele confia no último dado de release conhecido mesmo depois da janela diária de re-checagem. E quando o refresh do boot descobre uma release nova *depois* que a tela de boas-vindas já foi impressa, o ChatCLI anuncia na própria sessão, no turno seguinte do prompt — você não precisa reiniciar (nem rodar `/version`) para ficar sabendo:

```text
  ⬆ ChatCLI v1.173.0 disponível — rode /update para atualizar.
```
...
🔄 Auto-update
   ...
   Versão atual                    1.173.0
   Última release                  1.173.0
