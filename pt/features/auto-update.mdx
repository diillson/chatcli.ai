...
> /update
  ...
  Atual: v1.169.0 · Última: v1.170.1
  Baixando v1.170.1 (chatcli-darwin-arm64)...
  ✅ Atualizado para v1.170.1 — reinicie o chatcli para usar a nova versão.
```
...
sudo curl -fsSL -o /usr/local/bin/chatcli https://github.com/diillson/chatcli/releases/download/v1.170.1/chatcli-darwin-arm64 && sudo chmod +x /usr/local/bin/chatcli
```
...
```text
  Versão: 1.170.1 (commit: 52441581)
  ✓ Atualizado automaticamente para v1.170.1
```

No modo `notify` (o default), a tela de boas-vindas mostra o convite:

```text
  ⬆ v1.170.1 disponível — rode /update para atualizar
```
...
🔄 Auto-update
   ...
   Versão atual                    1.170.1
   Última release                  1.170.1
