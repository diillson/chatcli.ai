...
Os RPCs `GetAlerts`, `AnalyzeIssue` e `AgenticStep` são usados pelo [Operator AIOps](/pt/features/k8s-operator) para alimentar o pipeline autônomo de remediação. As respostas de `AnalyzeIssue` e `AgenticStep` também carregam os campos `usage`, `provider` e `model` descritos acima.
