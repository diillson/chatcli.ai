...
<CardGroup cols={2}>
  ...
  <Card title="Performance" icon="bolt">
    Comunicação via gRPC com suporte a TLS e streaming real do provedor
  </Card>
</CardGroup>
...
| `--token` | Token de autenticação. Obrigatório a menos que o servidor escute em loopback — veja abaixo | `""` | `CHATCLI_SERVER_TOKEN` |
...
Toda superfície — o REPL, execuções one-shot, o gateway, o servidor MCP/ACP e este servidor gRPC — pode enviar seus contadores de sessão a um collector OpenTelemetry por OTLP/HTTP (codificação JSON, sem dependência de SDK). A configuração é só o contrato de ambiente padrão do OTel:
...
Somas cumulativas enviadas a cada intervalo (e mais uma vez no encerramento), com `service.name`, `chatcli.surface` e `chatcli.tenant` no recurso: `chatcli.llm.tokens` (`kind` = input/output/cache_read/cache_write/reasoning, `provider`, `model`), `chatcli.llm.cost` (USD por provider/modelo), `chatcli.context.compactions` (`level` = summary/truncation) e `chatcli.context.compaction_cost`, `chatcli.cache.requests` (`outcome` = total/miss/expected_rebuild) e `chatcli.cache.storage_cost`, `chatcli.session.cost`. `/config server` mostra o endpoint, a contagem de envios e o último erro.
...
| `CHATCLI_SERVER_TLS_CLIENT_CA` | Bundle de CA contra o qual certificados de cliente são verificados (`--tls-client-ca`); habilita TLS mútuo e identifica chamadores pelo certificado. Exige `--tls-cert`/`--tls-key`. | `""` |
...
<Note>
O endereço de bind padrão é `127.0.0.1` (seguro para uso local). Em Kubernetes, o servidor auto-detecta o ambiente via `KUBERNETES_SERVICE_HOST` e automaticamente usa `0.0.0.0`. Um valor explícito em `CHATCLI_BIND_ADDRESS` sempre prevalece.
</Note>

<Warning>
**Um servidor alcançável precisa de credencial.** A autenticação é
opcional só em loopback, onde a fronteira de confiança é a máquina. Em
qualquer outro endereço — inclusive o `0.0.0.0` que o Kubernetes usa — um
servidor sem credencial admitiria todo chamador como administrador, então
ele **recusa subir** e diz o que falta.

Forneça uma destas:

- `--token` / `CHATCLI_SERVER_TOKEN` — uma credencial compartilhada;
- `CHATCLI_JWT_SECRET` (HS256) ou `CHATCLI_JWT_PUBLIC_KEY` (RS256) — JWTs
  por usuário, carregando identidade e papel. Os tokens precisam do claim
  `exp`; um sem expiração é recusado;
- `--tls-client-ca` / `CHATCLI_SERVER_TLS_CLIENT_CA` — TLS mútuo: todo
  chamador apresenta um certificado assinado pela CA e é identificado por
  ele (veja a aba TLS abaixo);
- `CHATCLI_BIND_ADDRESS=127.0.0.1` — sem credencial, alcançável apenas da
  própria máquina.

No Docker, a porta publicada só chega a um servidor que escute na
interface do container, então `CHATCLI_BIND_ADDRESS=0.0.0.0` e um token
andam juntos ali.
</Warning>
...
<Tabs>
  <Tab title="Sem Autenticação">
    Em loopback, o servidor não exige autenticação e qualquer cliente local pode conectar (um bind alcançável recusa subir sem credencial — veja o aviso acima):
  ...
  <Tab title="TLS (HTTPS)">
    ...
    **mTLS (Mutual TLS):** Para autenticação mútua, o servidor verifica os certificados de cliente contra um bundle de CA e o cliente apresenta o seu certificado:

    ```bash
    # Servidor: exige certificado assinado por ca.pem (env: CHATCLI_SERVER_TLS_CLIENT_CA)
    chatcli server --tls-cert cert.pem --tls-key key.pem --tls-client-ca ca.pem

    # Cliente: apresenta o próprio certificado
    ...
    Com `--tls-client-ca`, um chamador que não envia bearer token é identificado pelo certificado verificado: o principal é `mtls:<CN>` (o primeiro URI SAN, depois o primeiro DNS SAN, quando o CN está vazio) e o papel vem de `CHATCLI_MTLS_ROLE` (`viewer`, `user` ou `admin`; padrão `user`; valor não reconhecido vira somente-leitura). Um bearer token, quando presente, continua prevalecendo, pois carrega o papel que o emissor escolheu. O Helm chart expõe isso como `security.mtlsRole`.
...
| `StreamAlerts` | Empurra alertas do K8s Watcher conforme surgem, com heartbeats (streaming do servidor; usado pelo Operator) |
| `AnalyzeIssue` | Envia contexto de um Issue ao LLM e retorna análise + ações sugeridas |
| `AgenticStep` | Executa um turno do loop de remediação guiado por IA (usado pelo Operator) |
| `ResolveActiveConversation` | Retorna a conversa atual do principal, criando uma no primeiro contato (conversation hub) |
| `NewConversation` | Rotaciona a conversa ativa do principal para um thread novo; todo canal acompanha |
| `AppendEvent` | Anexa um turno de diálogo ao log compartilhado de uma conversa |
| `ReadConversation` | Retorna os eventos após um número de sequência (hidratação/resume) |
| `SubscribeConversation` | Acompanha uma conversa ao vivo: backlog e depois os eventos que qualquer canal anexar (server streaming) |
| `SetBinding` | Mapeia uma identidade de canal (plataforma, user id) a um principal |
| `ListBindings` | Lista os vínculos canal → principal |
| `ChatTurn` | Turno de chat com o pipeline completo no servidor (memória, contextos, skills, knowledge, compactação); opt-in, veja [RPCs de pipeline](#rpcs-de-pipeline) |
| `RunCoder` | Roda o loop do coder no servidor com as ferramentas dele (streaming: linhas da transcrição e, por fim, a resposta final) |
| `RunAgent` | Roda o loop do agent no servidor (mesmo contrato) |
| `ListPipelineTools` | Lista as ferramentas que o engine hospedado no servidor expõe |
| `RunPipelineTool` | Invoca uma dessas ferramentas |
...
`StreamPrompt` transmite o texto conforme o provedor o produz: cada chunk é um fragmento que o provedor emitiu, não uma fatia de uma resposta pronta. A mensagem final (`done: true`) carrega o `usage` que o provedor reportou e o `stop_reason`. Quando a rota escolhida não consegue fazer streaming, a resposta é concluída em uma chamada e entregue como um único chunk, então o cliente nunca precisa de um segundo caminho de código.

### Atribuição da Resposta e Uso de Tokens

As respostas de `SendPrompt`, `AnalyzeIssue` e `AgenticStep`, e a mensagem final do `StreamPrompt`, carregam uma mensagem `TokenUsage` e um `stop_reason`, e nomeiam o `provider` e o `model` que de fato responderam (a cadeia de fallback pode responder com um provedor diferente do configurado):
...
message TokenUsage {
  ...
  bool estimated = 6;   // provedor não retornou usage; contagem derivada do tamanho em caracteres
  double cost_usd = 7;  // precificado pelo servidor com as mesmas tabelas do /cost
  bool cost_known = 8;  // false quando o modelo não tem preço publicado (cost_usd é 0)
}
```

`stop_reason` é o que o provedor reportou (`end_turn`, `max_tokens`, `refusal`, ...). O `chatcli connect` alimenta o cost tracker local com esse usage, então o `/cost` numa conexão remota precifica os tokens reais em vez de uma estimativa por caracteres.

### Roteamento das Requisições

Todo RPC de prompt (`SendPrompt`, `StreamPrompt`, `InteractiveSession`, `AnalyzeIssue`, `AgenticStep`) resolve seu cliente de LLM pelas mesmas regras:

- Uma credencial encaminhada pelo chamador (`--llm-key`, `--use-local-auth`, campos do StackSpot) ou um override explícito de provedor/modelo recebe um cliente dedicado.
- Uma requisição que não nomeia provedor nem modelo e não encaminha credencial passa pela **cadeia de fallback** configurada (`CHATCLI_FALLBACK_*`) quando existe uma, senão pelo cliente padrão do servidor. A resposta nomeia a entrada da cadeia que respondeu.
- `max_tokens` é o valor da requisição quando informado, senão o override de ambiente `*_MAX_TOKENS` do provedor (`OPENAI_MAX_TOKENS`, `ANTHROPIC_MAX_TOKENS`, `GOOGLEAI_MAX_TOKENS`, `BEDROCK_MAX_TOKENS`, ...), senão o teto do catálogo para o modelo.
- Uma credencial **do servidor** expirada (401/403 do provedor) dispara um refresh dos provedores e uma nova tentativa. O refresh é limitado a um a cada 30 segundos por processo, então uma rajada de 401 sob carga reconstrói os provedores uma vez só. Credenciais encaminhadas pelo chamador nunca são renovadas: o cliente remoto é dono delas.
- Uma resposta interrompida por classificador de segurança (`stop_reason: refusal`) é reenviada uma vez no modelo irmão do mesmo provedor, como o loop do agente faz. Num stream isso só acontece enquanto nenhum texto foi exibido.

As requisições não são serializadas: cada RPC continua rodando concorrentemente na sua própria instância de cliente.

### RPCs de pipeline

`SendPrompt` é um proxy de modelo: com `chatcli connect`, o pipeline de enriquecimento (memória, anexos do `/context`, skills, recuperação de knowledge, compactação) roda na **sua** CLI e o servidor só fala com o modelo. Clientes finos, o operator e automações que querem que o **servidor** seja dono do turno inteiro usam os RPCs de pipeline, o mesmo engine que os servidores MCP e ACP em stdio expõem:

```bash
CHATCLI_SERVER_PIPELINE=true chatcli server --port 50051 --token meu-token
```

| RPC | O que roda no servidor | Papel |
|---|---|---|
| `ChatTurn` | um turno de chat com o pipeline completo (`plain: true` pede o passthrough cru); provider/modelo opcionais por turno | qualquer chamador autenticado |
| `RunCoder`, `RunAgent` | o loop do coder / agent com as ferramentas do servidor; o stream traz as linhas da transcrição e, por fim, a resposta final | `admin` |
| `ListPipelineTools`, `RunPipelineTool` | as ferramentas admitidas pela policy (`@git`, `@read`, ...) | `admin` |

As sessões são namespaced pelo principal autenticado (`<subject>/<sessão>`; a sessão padrão quando nenhuma é informada), então dois chamadores nunca dividem uma conversa por escolher o mesmo id. `GetServerInfo` informa `pipeline_enabled`.

<Warning>
O engine hospeda um ChatCLI dentro do processo do servidor: os workers dele (memória, servidores MCP, scheduler) rodam ali e os turnos são **serializados**; chamadores concorrentes entram na fila, limitados pelo próprio deadline. É exclusivo com o gateway co-localizado (`CHATCLI_GATEWAY_IN_SERVER`), que constrói o próprio engine; com os dois definidos o pipeline fica desligado e o motivo vai para o log. Prefira uma Instance (ou conjunto de réplicas) dedicada ao tráfego de pipeline.
</Warning>
...
Os RPCs `GetAlerts`, `StreamAlerts`, `AnalyzeIssue` e `AgenticStep` são usados pelo [Operator AIOps](/pt/features/k8s-operator) para alimentar o pipeline autônomo de remediação. As respostas de `AnalyzeIssue` e `AgenticStep` também carregam os campos `usage`, `provider` e `model` descritos acima.
...
Empurra os alertas no instante em que o watcher os gera, então o operator não precisa mais fazer polling. É streaming do lado do servidor sobre a mesma conexão autenticada:
...
message StreamAlertsRequest {
  string namespace = 1;     // Filtrar por namespace (vazio = todos)
  string deployment = 2;    // Filtrar por deployment (vazio = todos)
  bool include_current = 3; // Abrir com os alertas ativos agora
}

message StreamAlertsResponse {
  WatcherAlert alert = 1;   // Vazio num heartbeat
  bool heartbeat = 2;       // Sinal de vida a cada 15s enquanto o watcher está quieto
}
```

- Com `include_current` o stream abre com os alertas que `GetAlerts` devolveria e depois só chegam os novos. O dedup do próprio watcher (mesmo tipo e objeto dentro da janela) garante que cada alerta é empurrado uma vez.
- Heartbeats a cada 15 segundos permitem distinguir um watcher quieto de uma conexão morta. Sem watcher configurado o stream carrega só heartbeats, equivalente ao `GetAlerts` devolver lista vazia.
- Cada assinante tem um buffer limitado. Quem fica para trás é derrubado com `ABORTED`: reabra o stream com `include_current` para ressincronizar. `GetAlerts` continua disponível para leituras pontuais e para clientes que preferem polling.
...
O servidor implementa rate limiting por cliente usando token bucket para proteger contra abuso. O limitador roda **depois** da autenticação e usa como chave o sujeito do chamador — o `sub` do JWT ou o principal do certificado — então tenants atrás do mesmo NAT ou ingress não dividem um bucket; chamadores anônimos são chaveados pelo endereço. Rajadas não autenticadas são contidas à parte pelo limitador de falhas de autenticação por endereço.
...
CHATCLI_JWT_SECRET=minha-chave-secreta      # HS256, ou:
...
# mTLS (lado do servidor)
CHATCLI_SERVER_TLS_CLIENT_CA=/path/to/ca.pem
CHATCLI_MTLS_ROLE=user   # papel de quem se identifica só pelo certificado

# mTLS (lado do cliente, chatcli connect)
