...
Skills recém-casadas são injetadas como mensagem append-only `[SKILL AUTO-ACTIVATION — MID-TASK]` no próximo limite de turno, então o prefixo cacheado do system prompt nunca é invalidado no meio da execução. Cada skill dispara no máximo **uma vez por execução** (dedup contra a injeção do startup), e o set de dedup sobrevive a `@park`/resume. Hints de `model:`/`effort:` de skills mid-task só valem quando nenhuma skill do startup já os reivindicou.
