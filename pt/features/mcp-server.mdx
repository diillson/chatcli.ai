...
| `manage_session` | Persiste e restaura as conversas por trás do parâmetro `session` do `ask_chatcli` — save/load/list/delete/clear/active, mais **`search` full-text** no store de sessões salvas e **`fork`** para copiar uma sessão salva. Compartilha o store do `/session` com o REPL interativo. Sessões vivas também têm **autosave por padrão** como espelhos rolling `mcp-<session>` após cada turno (`CHATCLI_MCP_SESSION_AUTOSAVE`; sem ele segue `CHATCLI_SESSION_AUTOSAVE`), e o `clear` salva uma última vez antes de descartar. |
