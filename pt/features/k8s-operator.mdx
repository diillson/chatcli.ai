...
<Info>
Para documentação detalhada de cada componente da plataforma AIOps, consulte as páginas dedicadas em [AIOps Platform](/pt/features/aiops/notifications).
</Info>
...
<Accordion title="Valores configuráveis">
  ...
  | `prometheusUrl` | `""` | URL do Prometheus para coleta de métricas (defina aqui, não no chart do servidor, onde está deprecado) |
  | `security.allowedDiagnosticCommands` | `""` | Comandos de diagnóstico read-only extras para o `ExecDiagnostic` (separados por vírgula, somados à allowlist embutida); renderizado como `CHATCLI_ALLOWED_DIAGNOSTIC_COMMANDS` no pod do operator |
...
graph TD
    subgraph "ChatCLI Server"
        W[K8s Watcher<br/>collectors] --> GA[StreamAlerts RPC<br/>fallback GetAlerts]
        AI_RPC[AnalyzeIssue RPC<br/>LLM call]
        AG_RPC[AgenticStep RPC<br/>observe-decide-act]
    end

    subgraph "Operator"
        WB[WatcherBridge<br/>stream aberto] -->|gRPC| GA
...
|------|-----------|-----------|
| **1. Detecção** | WatcherBridge | Recebe alertas pelo stream `StreamAlerts` do servidor (só consulta `GetAlerts` a cada 30s num servidor sem ele). Cria Anomaly CRs (dedup SHA256). Invalida dedup quando Issue atinge estado terminal. |
...
spec:
  replicas: 1
  provider: CLAUDEAI       # OPENAI, OPENAI_ASSISTANT, CLAUDEAI, BEDROCK, GOOGLEAI, XAI, ZAI, MINIMAX, MOONSHOT, OPENROUTER, STACKSPOT, OLLAMA, COPILOT, DEVIN
  ...
  server:
    security:
      jwtSecretRef:                 # HS256: o operator emite os próprios tokens a partir dele
        name: chatcli-jwt
        key: secret
      # jwtPublicKeyRef:            # RS256 em vez disso: chave pública PEM com que o servidor verifica
      #   name: chatcli-jwt
      #   key: public.pem
      # jwtIssuer: "meu-idp"        # claims iss / aud exigidos, quando seu emissor os define
      # jwtAudience: "chatcli"
      # operatorTokenRef:           # credencial que o operator apresenta (necessária com RS256)
      #   name: chatcli-operator-jwt
      #   key: token
      rateLimitRps: 20
      # bindAddress: "0.0.0.0"  # Em branco significa toda interface no Kubernetes — não loopback
...
<Warning>
**Uma Instance precisa de credencial.** O servidor que uma Instance
provisiona escuta em toda interface dentro do cluster, então um spec sem
`server.token`, sem `security.jwtSecretRef` e sem `security.jwtPublicKeyRef`
provisionaria um servidor que recusa subir. O operator não cria o Deployment nesse caso — levanta uma
condition na Instance:

```bash
kubectl get instance minha-instance \
  -o jsonpath='{.status.conditions[?(@.type=="AuthenticationConfigured")].message}'
```

Use loopback com `security.bindAddress: "127.0.0.1"` quando o servidor só
é alcançado de dentro do próprio pod. Uma credencial passada por
`extraEnv` como `CHATCLI_SERVER_TOKEN`, `CHATCLI_JWT_SECRET`,
`CHATCLI_JWT_PUBLIC_KEY` ou `CHATCLI_SERVER_TLS_CLIENT_CA` também conta.
</Warning>

#### Como o operator se autentica

O pipeline AIOps (alertas, análise, passos de remediação) chama o mesmo
servidor, então o operator precisa de uma credencial própria. Ele escolhe
uma nesta ordem e registra a escolha na Instance:

| Precedência | Campo | O que o operator envia |
|---|---|---|
| 1 | `spec.server.token` | o token compartilhado, como antes |
| 2 | `spec.server.security.operatorTokenRef` | o valor referenciado, sem alteração: um JWT emitido para `chatcli-operator` pelo seu identity provider (servidores RS256), ou um token compartilhado dedicado |
| 3 | `spec.server.security.jwtSecretRef` | tokens HS256 de curta duração que ele mesmo emite (`sub: chatcli-operator`, `role: operator`, uma hora, renovados aos 45 minutos), carimbados com `jwtIssuer` / `jwtAudience` quando definidos |
| 4 | `spec.server.security.operatorClientCertSecretName` | o certificado de cliente dele (`tls.crt` / `tls.key`), apresentado em toda conexão; com `spec.server.tls.clientCASecretName` no servidor e sem credencial bearer, o certificado é a identidade inteira |

```bash
kubectl get instance minha-instance \
  -o jsonpath='{.status.conditions[?(@.type=="OperatorCredentialConfigured")]}'
```

`OperatorCredentialConfigured=False` significa que o servidor está protegido
de um jeito que o operator não consegue satisfazer (uma chave pública RS256
sem `operatorTokenRef`, ou uma credencial passada só por `extraEnv`): a
Instance ainda é provisionada, mas `AnalyzeIssue` e `AgenticStep` serão
recusados até que um dos campos acima seja definido. O token emitido carrega
o papel `operator`, que é o que os RPCs de AIOps precisam e nada mais.

#### ServerSecuritySpec (`spec.server.security`)

| Campo | Tipo | Obrigatório | Descrição |
|-------|------|-------------|-----------|
| `jwtSecretRef` | SecretKeyRef | Não | Segredo de assinatura HS256 (`CHATCLI_JWT_SECRET`); o operator também emite os próprios tokens a partir dele |
| `jwtPublicKeyRef` | SecretKeyRef | Não | Chave pública RSA (PEM) com que o servidor verifica tokens RS256 (`CHATCLI_JWT_PUBLIC_KEY`). O operator não consegue assinar com ela: defina `operatorTokenRef` também |
| `jwtIssuer` | string | Não | Claim `iss` exigido (`CHATCLI_JWT_ISSUER`); carimbado nos tokens que o operator emite |
| `jwtAudience` | string | Não | Claim `aud` exigido (`CHATCLI_JWT_AUDIENCE`); carimbado nos tokens que o operator emite |
| `operatorTokenRef` | SecretKeyRef | Não | Credencial que o operator apresenta a este servidor: um JWT emitido fora de banda, ou o token compartilhado |
| `mtlsRole` | string | Não | Papel de quem se identifica só pelo certificado de cliente (`viewer`, `user`, `operator`, `admin`; `CHATCLI_MTLS_ROLE`, padrão `user`) |
| `operatorClientCertSecretName` | string | Não | Secret TLS (`tls.crt`, `tls.key`) que o operator apresenta como certificado de cliente ao discar para esta Instance |
| `rateLimitRps` / `rateLimitBurst` | int32 | Não | Rate limit por cliente (`CHATCLI_RATE_LIMIT_RPS` / `CHATCLI_RATE_LIMIT_BURST`) |
| `maxRecvMsgSize` / `maxSendMsgSize` / `maxConcurrentStreams` | int32 | Não | Limites de mensagem e de streams gRPC |
| `bindAddress` | string | Não | Endereço de bind do servidor (em branco = toda interface no Kubernetes) |

O TLS mútuo do lado do servidor fica em `spec.server.tls`: `clientCASecretName` nomeia um Secret cujo `ca.crt` é a CA contra a qual os certificados de cliente são verificados (montado em `/etc/chatcli/client-ca`, passado como `--tls-client-ca`); exige `enabled: true` e `secretName`, e conta como credencial na verificação de bind alcançável.

### O que está realmente rodando

Assim que o Deployment fica pronto o operator sonda o servidor com o próprio transporte e a própria credencial (`Health`, depois `GetServerInfo`) e registra o resultado, então `kubectl get instance` mostra a versão que respondeu e se o pipeline AIOps consegue conduzi-la:

```bash
kubectl get instance my-instance
# NAME          READY   REPLICAS   PROVIDER   VERSION   AGE
# my-instance   true    1          CLAUDEAI   1.211.1   3d
kubectl get instance my-instance \
  -o jsonpath='{.status.conditions[?(@.type=="ServerReachable")]}'
```

| Condição | Significado |
|---|---|
| `ServerReachable=True`, reason `Serving` | o servidor respondeu com versão, provider e modelo |
| `ServerReachable=False`, reason `ProbeFailed` | falha de transporte ou de credencial; uma mensagem citando a verificação de credencial significa que os pods estão de pé mas a credencial do operator foi recusada |
| `ServerReachable=False`, reason `NotServing` | o servidor respondeu `Health` com `NOT_SERVING` (drenando) |
| `ServerReachable=Unknown` | ainda sem réplica pronta, nada sondado |

`status.serverVersion` mantém a última versão conhecida mesmo após uma sonda falha; `status.serverProbeTime` é quando o operator tentou pela última vez.

O pipeline AIOps contabiliza o gasto de LLM a partir dos tokens que o servidor informa em `AnalyzeIssue` e `AgenticStep` (o operator estimava as análises por contagem de caracteres e nunca contabilizava os passos agênticos), atribuído ao provider e modelo que de fato responderam, precificado com as mesmas tabelas do `/cost` da CLI.

### Features, scheduling e ServiceAccount (tipados)

O que antes exigia `spec.extraEnv` agora tem campos tipados:

| Campo | Renderiza | Notas |
|---|---|---|
| `spec.features.memory` (`enabled`, `mode`) | `CHATCLI_MEMORY_ENABLED`, `CHATCLI_MEMORY_MODE` | `mode`: `index`, `pull` ou `off` |
| `spec.features.knowledge` | `CHATCLI_CHAT_KNOWLEDGE` | nil mantém o padrão do servidor |
| `spec.features.budget` (`sessionUSD`, `dailyUSD`, `hardStop`) | `CHATCLI_SESSION_BUDGET_USD`, `CHATCLI_DAILY_BUDGET_USD`, `CHATCLI_BUDGET_HARD_STOP` | |
| `spec.features.hub` | `CHATCLI_HUB_ENABLED` | hub de conversas |
| `spec.features.caBundleSecretName` | monta `ca.crt` em `/etc/chatcli/ca`, `CHATCLI_CA_BUNDLE` | CA corporativa confiada por toda conexão TLS de saída |
| `spec.features.allowHTTPProviders` | `CHATCLI_ALLOW_HTTP_PROVIDERS` | desligado mantém a postura só-TLS |
| `spec.features.encryptionKeyRef` | `CHATCLI_ENCRYPTION_KEY` a partir do Secret | criptografia em repouso |
| `spec.features.logRotation` (`maxSizeMB`, `maxBackups`, `maxAgeDays`, `compress`) | `CHATCLI_LOG_*` | |
| `spec.pipeline.enabled` | `CHATCLI_SERVER_PIPELINE` | os [RPCs de pipeline](/pt/features/server-mode#rpcs-de-pipeline) |
| `spec.scheduling` (`nodeSelector`, `tolerations`, `affinity`, `imagePullSecrets`, `priorityClassName`, `podAnnotations`) | template do pod | os hashes de rollout do operator vencem uma anotação de pod conflitante |
| `spec.serviceAccount.annotations` | o ServiceAccount gerenciado | IRSA / Workload Identity; anotações adicionadas por outras ferramentas são mantidas |

Só o que está definido é renderizado, então os padrões do servidor continuam valendo para o resto.
| `auditLogPath` | string | Não | Caminho do arquivo de auditoria (`CHATCLI_AUDIT_LOG_PATH`) |
| `debug` | bool | Não | Logging de debug |
| `enableReflection` | bool | Não | gRPC reflection (só para debugging) |
...
| Campo | Tipo | Obrigatório | Descrição |
|-------|------|:-----------:|-----------|
| `name` | string | **Sim** | Nome do provedor: OPENAI, OPENAI_ASSISTANT, CLAUDEAI, BEDROCK, GOOGLEAI, XAI, ZAI, MINIMAX, MOONSHOT, OPENROUTER, STACKSPOT, OLLAMA, COPILOT, DEVIN |
...
<Note>
A partir da v1.105.1, o operator **não cria mais `ClusterRole` em runtime** (hardening H5). As ClusterRoles compartilhadas — `chatcli-watcher` para o watcher e `chatcli-role-{viewer,operator,admin,superadmin}` para as platform roles — são instaladas pelo Helm chart do operator. O ServiceAccount do operator tem o verb `bind` restrito a esses nomes específicos via `resourceNames`, impedindo escalonamento de privilégio mesmo em caso de comprometimento.
</Note>

<Note>
**Upgrade a partir de v1.105.0**: clusters com Instances multi-namespace pré-existentes tinham um `ClusterRoleBinding` apontando para uma ClusterRole por-Instance (formato antigo). Como `roleRef` é imutável no Kubernetes, um `helm upgrade` direto travava o reconcile com `cannot change roleRef`. A partir de v1.151.1, o operator detecta o `roleRef` divergente no início de `reconcileClusterRBAC`, deleta o CRB obsoleto e recria apontando para `chatcli-watcher` — migração transparente, sem intervenção manual.
</Note>
...
Extender via env var `CHATCLI_ALLOWED_DIAGNOSTIC_COMMANDS` (comma-separated, lido uma vez na inicialização). Quem lê essa variável é o processo do **operator**, não o servidor, então defina-a no chart `chatcli-operator` com `security.allowedDiagnosticCommands` — uma entrada no `spec.extraEnv` de uma Instance vai parar no pod do servidor e não tem efeito:

```bash
helm upgrade chatcli-operator oci://ghcr.io/diillson/charts/chatcli-operator \
  --namespace chatcli-system --reuse-values \
  --set security.allowedDiagnosticCommands="dig +short redis.default.svc.cluster.local\, nc -zv redis.default.svc.cluster.local 6379"
```
...
- **Stream primeiro**: Mantém aberto o stream `StreamAlerts` do servidor e transforma cada alerta em Anomaly no instante em que chega. O stream abre com os alertas ativos agora, então nada gerado enquanto a bridge estava fora se perde
- **Liveness**: O servidor manda heartbeat a cada 15 segundos; 60 segundos de silêncio reabrem o stream. As reaberturas recuam de 1s a 30s, e três tentativas silenciosas seguidas derrubam a conexão para redescobrir a Instance (endereço ou credenciais podem ter mudado)
- **Fallback por polling**: Um servidor sem `StreamAlerts` (release antigo) responde `UNIMPLEMENTED`; a bridge então consulta `GetAlerts` a cada 30 segundos e volta a sondar o stream depois de 10 minutos, então um upgrade do servidor é aproveitado sem reiniciar o operator. `CHATCLI_OPERATOR_ALERT_TRANSPORT=poll` (valor `alertTransport` do chart) força o polling
- **Resync**: Se o servidor derruba a bridge por ficar para trás (`ABORTED`), o stream é reaberto na hora com o snapshot atual; o dedup absorve as repetições
...
# Testes unitários (client fake, reconcilers acionados à mão)
go test ./... -v

# Testes de integração: os controllers rodam contra um API server real
# (envtest: kube-apiserver + etcd) com os CRDs de config/crd/bases,
# ligados exatamente como o main.go liga, sobre um servidor ChatCLI em memória.
# Os binários são baixados uma vez em ./bin; sem KUBEBUILDER_ASSETS a
# suíte pula, então um `go test ./...` comum continua verde.
make test-integration

# Preview do web dashboard: sem cluster. Serve o dashboard embutido em
# http://127.0.0.1:8085/ sobre um client fake com incidentes sintéticos,
# runbooks, SLOs e uma aprovação pendente. Entre com a API key "preview".
# Troque o endereço com DASHPREVIEW_ADDR=127.0.0.1:9090.
make dash-preview
...
<Note>
**Dois Secrets distintos no projeto** — não confunda com as LLM keys do servidor (`chatcli-api-keys` consumido pelo chatcli server via `Instance.spec.apiKeys.name`). Tabela comparativa em [Segurança — Autenticação](/pt/features/security#auten%C3%A7%C3%A3o-do-operator).

O Secret `chatcli-operator-secrets` precisa estar **no mesmo namespace que o pod do operator** (o controller resolve via `POD_NAMESPACE` env / arquivo do ServiceAccount, com fallback `chatcli-system`). Se você fez `helm install --namespace <X>`, crie o Secret em `<X>`.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Modo Servidor" icon="server" href="/pt/features/server-mode">
    RPCs StreamAlerts, GetAlerts e AnalyzeIssue
  </Card>
