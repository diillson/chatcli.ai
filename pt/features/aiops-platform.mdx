...
sequenceDiagram
    ...
    Note over WB,Server: Servidor sem StreamAlerts: GetAlerts consultado a cada 30s
...
| `poll()` | Consulta GetAlerts uma vez (fallback para servidor sem StreamAlerts, ou `CHATCLI_OPERATOR_ALERT_TRANSPORT=poll`) |
...
O servidor empurra os alertas do K8s Watcher por `StreamAlerts` (streaming do servidor com heartbeats; veja [Modo Servidor](/pt/features/server-mode#streamalerts)) e continua expondo-os para leituras pontuais via gRPC:
