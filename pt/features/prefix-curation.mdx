...
**Onde a leitura é barata o bastante, a entrada é mantida quente em vez disso.** Uma leitura de cache renova o timer da entrada de graça, então reenviar a requisição anterior com `max_tokens: 0` pouco antes de a entrada de 5 minutos expirar a mantém viva pelo preço de uma leitura e nenhum output. No Claude Fable 5.1 a leitura custa 2,5% do preço de input, o que vence pagar 2x em cada token escrito pela entrada de uma hora, a menos que as pausas se aproximem de uma hora. Então nesses modelos o REPL interativo renova a entrada a cada 4,5 minutos enquanto você está ocioso, até doze vezes, e a promoção para uma hora fica de fora. Cada renovação é contabilizada como qualquer requisição, então o `/cost` mostra quanto custou. Nos demais modelos Claude a leitura custa 10% do input e a aritmética favorece a hora, então eles mantêm a promoção por evidência. `CHATCLI_PROMPT_CACHE_KEEPALIVE` é `auto` (pelo preço de leitura), `on` (todo cliente que consegue renovar) ou `off`; o gateway nunca renova, porque lá um cliente serve todos os tenants.
