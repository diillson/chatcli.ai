...
<Tabs>
  ...
  <Tab title="Sufixo por turno">
    ...
    No chat o sufixo viaja como uma mensagem user-role sinalizada `[TURN CONTEXT]` logo antes do seu turno. Em agent e coder, os blocos que pertencem a um run inteiro — o retrieval dirigido pela query e o bloco de skills — viajam como mensagens de histórico sinalizadas, anexadas uma vez por run (`[RUN CONTEXT]` e a mensagem de skills), nunca dentro do system message: o array de system fica à frente de todas as mensagens, então mesmo um bloco sem cache ali reescrevia a conversa cacheada inteira a cada nova query. Como histórico eles são append-only, o prefixo do run anterior segue como leitura de cache, e ficam na frente do modelo durante todo o loop de tools.
  </Tab>
...
**Uma tool call que sobrevive à entrada é coberta por uma renovação, em todo provider que consegue.** Uma leitura de cache renova o timer da entrada de graça em qualquer cache de prompt. Uma suíte de testes de seis minutos dentro de um turno do coder expiraria uma entrada de 5 minutos antes da próxima requisição, e o prefixo inteiro seria reescrito: ao preço de escrita na Anthropic e no Bedrock, a preço cheio nos caches subconjunto da OpenAI e dos demais. Então 4,5 minutos depois de cada requisição, enquanto um run ainda está rodando, o ChatCLI reenvia a requisição anterior pedindo nenhum output (`max_tokens: 0` no wire da Anthropic, o único token que os wires Chat Completions e Bedrock Mantle aceitam) e paga o preço de leitura do prefixo, até doze vezes por trecho. O corpo é o mesmo que o turno enviou, thinking e effort incluídos, porque fazem parte da chave do cache. Cada renovação é contabilizada como qualquer requisição, então o `/cost` mostra quanto custou. Os adapters aderem por uma interface pequena; a API da Anthropic, o Chat Completions da OpenAI e o Bedrock Mantle renovam hoje, e um adapter que não consegue diz isso uma vez e não é chamado de novo.

**Onde a leitura é barata o bastante, a entrada é mantida quente também enquanto você está ocioso.** No Claude Fable 5.1 a leitura custa 2,5% do preço de input, o que vence pagar 2x em cada token escrito pela entrada de uma hora, a menos que as pausas se aproximem de uma hora. Então nesses modelos o REPL interativo continua renovando entre turnos, e a promoção para uma hora fica de fora. Nos demais modelos a leitura custa 10% do input ou mais e a aritmética favorece a hora, então eles mantêm a promoção por evidência. `CHATCLI_PROMPT_CACHE_KEEPALIVE` é `auto` (tool calls longas em todo lugar, ocioso pelo preço de leitura), `on` (ocioso em todo cliente que consegue renovar) ou `off`; o gateway nunca renova, porque lá um cliente serve todos os tenants.

**A decisão pertence à conversa.** O tempo de vida em que o `auto` se assentou faz parte de cada marcador de cache que a requisição carrega. Sob o gateway, onde um processo serve muitos principais, ele viaja com o conjunto de stores de cada tenant: o run de `/coder` de um tenant decide só pela conversa daquele tenant, e a conversa base recebe sua própria decisão de volta quando o tenant sai.

O ChatCLI agora espera pela resposta, em toda superfície. Agent e coder pediam a hora de antemão, apostando que pausam entre rodadas de tools; um run de duas requisições pagava a escrita dobrada por nada, então agora seguem a mesma evidência do chat. Quando a sessão vê o próprio prefixo expirar numa pausa que a hora teria coberto, ele pede a hora — **uma vez**, sem volta, nunca por cima de um `CHATCLI_PROMPT_CACHE_TTL` explícito e nunca num modelo que não suporta. Uma execução one-shot jamais chega à segunda observação, então permanece no TTL curto.
