...
<Tabs>
  <Tab title="Voyage (recomendado)">
    ...
    # Opcional: escolher modelo (default voyage-4, 1024-dim)
    export CHATCLI_EMBED_MODEL=voyage-4
    ```

    **Por quê Voyage:** é o provider **recomendado pela Anthropic** para workflows Claude, tem alta qualidade/$ em retrieval, e o modelo default (`voyage-4`, 1024-dim) é o tier general-purpose da família voyage-4 — mesmo preço do voyage-3, estritamente melhor. `voyage-4-large` (flagship de qualidade), `voyage-4-lite` (mais barato) e `voyage-code-4` (retrieval de código) compartilham o mesmo espaço de embedding.
  </Tab>
  <Tab title="OpenAI">
    ```bash
    export CHATCLI_QUALITY_HYDE_ENABLED=true
  ...
  <Tab title="Google (Gemini)">
    ...
    # Opcional: escolher modelo e dimensão truncada
    export CHATCLI_EMBED_MODEL=gemini-embedding-2
    export CHATCLI_EMBED_DIMENSIONS=1536  # 128-3072 via Matryoshka
    ```

    **Modelos:** `gemini-embedding-2` (default, 3072-dim nativo, GA, multimodal-ready, com free tier) ou o antigo `gemini-embedding-001` (superado; desliga em 14/05/2028). Batches vão via `:batchEmbedContents`; dimensões truncadas são L2-normalizadas localmente, então qualquer `CHATCLI_EMBED_DIMENSIONS` entre 128 e 3072 é seguro para similaridade de cosseno.
  </Tab>
  <Tab title="Bedrock (Titan + Cohere)">
    ...
    **Famílias suportadas:**
    ...
    | `cohere.embed-v4:0` | Cohere v4 (multi, ctx 128K) | 1536 (default) | Sim — uma chamada por batch |
    | `amazon.nova-2-multimodal-embeddings-v1:0` | Nova MME | 256, 384, 1024, 3072 (default 3072) | Não — paralelizado com pool de 8 workers |
...
## O tool @embed para o agent

O backend de embeddings também é exposto diretamente ao modelo como o tool **`@embed`** (modos agent/coder), do mesmo jeito que o `@image` expõe geração de imagem:

```text
<tool_call name="@embed" args='{"cmd":"similarity","args":{"a":"resetar uma senha","b":"recuperar acesso à conta"}}' />
```

| Subcomando | Propósito |
|---|---|
| `similarity {a, b}` | similaridade de cosseno entre dois textos, em [-1, 1] |
| `rank {query, candidates, k?}` | ordena textos candidatos por relevância semântica a uma query |
| `text {text\|texts, out?}` | embedda textos; com `out`, salva JSONL `{index, text, embedding}` para pipelines externos |
| `status` | mostra provider, modelo e dimensão |

Usa a mesma configuração `CHATCLI_EMBED_PROVIDER` descrita acima. Para **RAG persistente sobre documentos**, o modelo é direcionado ao `@context` (construir/atachar base de conhecimento) + `@knowledge` (consultar) — o `@embed` cobre as operações semânticas ad-hoc que não precisam de índice.
