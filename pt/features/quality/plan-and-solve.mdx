...
<Warning>**Por padrão, um trigger auto/always agora roteia para o [Task Graph](/pt/features/task-graph) verificado, não para o runner in-loop descrito aqui.** O runner legado do Plan-and-Solve não tem gates de validação nem reviewer independente; o task graph tem os dois. Esta página documenta o planner, a heurística de complexidade (ainda o trigger) e o opt-out `plan-solve`. Ver [Roteamento para o task graph](#roteamento-para-o-task-graph).</Warning>
...
## Roteamento para o task graph

A heurística de complexidade continua sendo o trigger determinístico — mas o que ela dispara mudou. Por padrão (`CHATCLI_QUALITY_PLAN_FIRST_STRATEGY=taskgraph`), um trigger auto/always **direciona o orquestrador para o [Task Graph](/pt/features/task-graph) verificado** em vez de rodar o runner legado in-loop:

- Uma dica de orquestração de uma linha é dobrada no turno do usuário, apontando o modelo para o `@taskgraph`: decompor o trabalho num DAG com contratos `validation` por task, e deixar o engine verificar cada uma com um reviewer independente antes de contar como done.
- O direcionamento é **não-coercivo**: a skill do task-graph ainda manda o modelo pular o grafo em trabalho genuinamente serial ou com menos de ~5 tasks independentes, então uma alteração profunda num arquivo só não é forçada a virar DAG.

| Prompt | Roteia para |
|---|---|
| Tarefa complexa, sem tool explícita (`ComplexityScore ≥ 6`) | **Task graph** (direcionado) |
| `@taskgraph` explícito / casa os triggers da skill | **Task graph** (plan-first cede) |
| `/plan <task>` ou `/plan preview <task>` | **Este planner** (preview / runner legado) |
| `CHATCLI_QUALITY_PLAN_FIRST_STRATEGY=plan-solve` | **Este runner legado** (opt-out) |
| Tarefa simples (score < threshold) | Tool calls diretas |

Use `plan-solve` para manter o comportamento anterior ao roteamento (o runner in-loop desta página); `/plan` explícito sempre usa o planner, independente da strategy.
...
| `CHATCLI_QUALITY_PLAN_FIRST_STRATEGY` | `taskgraph` | `taskgraph\|plan-solve` | Para onde um trigger auto/always roteia: o task graph verificado, ou o runner legado in-loop desta página |
