...
<Tip>Pra habilitar o embedding: `CHATCLI_QUALITY_REFINE_CONVERGENCE_EMBEDDING=1` + qualquer provider de embeddings (`CHATCLI_EMBED_PROVIDER=voyage` / `openai` / `bedrock`). O cascade reutiliza o mesmo provedor do HyDE — zero infra duplicada. Veja [RAG + HyDE](/pt/features/quality/rag-hyde) para detalhes por provider.</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="ReviewerAgent" icon="magnifying-glass-chart" href="/pt/features/customizable-agents">
