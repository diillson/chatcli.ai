...
<Tabs>
  ...
  <Tab title="Analisar dados CSV">
    ```bash
    cat relatorio.csv | chatcli --provider CLAUDEAI --model claude-sonnet-4-6 -p "Análise estes dados e me dê 3 insights principais."
    ```
  </Tab>
  <Tab title="Slash command num pipe">
    ```bash
    git diff | chatcli -p "/review"
    ```

    O corpo do pipe vira o `$ARGUMENTS` do [slash command](/pt/features/slash-commands) — e um comando `mode: coder` pode agir sobre ele com o coder engine completo, no meio da pipeline.
  </Tab>
</Tabs>
...
| `--provider <nome>` | Sobrescreve o provedor de LLM padrão (ex: `OPENAI`, `OPENAI_ASSISTANT`, `CLAUDEAI`, `BEDROCK`, `GOOGLEAI`, `XAI`, `ZAI`, `MINIMAX`, `MOONSHOT`, `STACKSPOT`, `OLLAMA`, `COPILOT`, `OPENROUTER`, `DEVIN`). |
...
Prompts one-shot também resolvem [templates de slash commands](/pt/features/slash-commands): `chatcli -p "/review-pr 1326 security"` expande o template de `.chatcli/commands` antes de qualquer execução. Pipes compõem naturalmente — `git diff | chatcli -p "/review"` alimenta o corpo do pipe no `$ARGUMENTS` do comando. Linhas de pré-execução seguem sua política de segurança: aprovação interativa só quando o stdin é um terminal de verdade; execuções via pipe resolvem pelas regras de policy e caem em deny fail-safe — uma regra `allow` explícita (ex.: `{ "pattern": "exec git log", "action": "allow" }` em `~/.chatcli/coder_policy.json`) deixa as linhas `!` de um comando rodarem totalmente unattended, destravando automações em cron/CI.
...
<Warning>Use `--agent-auto-exec` com cautela. Embora o validador de segurança bloqueie comandos perigosos conhecidos, sempre revise o comportamento em ambientes de produção.</Warning>

---

## Executando Tools Diretamente — `chatcli tool`

Às vezes você quer o output de uma tool built-in, não a opinião de um modelo sobre ele. Um `-p "@tool ..."` simples **executa a tool e depois envia o resultado pra LLM** como contexto — lento, caro em tokens e impraticável quando o output é grande. O subcomando `tool` executa a tool e imprime o output, **sem nenhuma chamada de LLM**:

```bash
# Lista todas as tools disponíveis pro subcomando
chatcli tool list

# Executa uma tool com flags normais (argv passa verbatim)
chatcli tool @docs-flatten --root ./docs --format jsonl --output corpus.jsonl

# Ou com um único envelope JSON
chatcli tool docs-flatten '{"root":"./docs","format":"jsonl","output":"corpus.jsonl"}'
```

- **Keyless** — sem provider configurado, sem problema: como as direct tools do servidor MCP, o subcomando só precisa da própria tool.
- **Mesma política de exposição** — o catálogo respeita `CHATCLI_MCP_TOOLS` (tools interativas são excluídas), exatamente como a superfície de tools do MCP.
- **Amigável a scripts** — output no stdout, erros no stderr com exit code 1. Perfeito para cron jobs, etapas de CI e pipes.

<Tip>Use `-p "@tool ..."` quando quiser que o modelo *raciocine sobre* o output da tool; use `chatcli tool` quando só quiser o output.</Tip>
