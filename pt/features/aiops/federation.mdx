...
O contador `chatcli_operator_federation_cross_cluster_issues_total` incrementa uma vez por correlação. Issues remotas que mudaram por baixo do update são puladas no conflito e apanhadas no próprio reconcile seguinte.
...
A detecção de cascata marca uma Issue de produção cujo recurso já falhou em staging: o mesmo problema está rolando pelos ambientes.

### Staging para Produção

A verificação precisa de pelo menos um registro conectado com `environment: staging` e um com `environment: prod`. Para a Issue sendo detectada, o controller lista as Issues de cada cluster de staging e procura uma num recurso com o **mesmo nome** cujo `detectedAt` seja **anterior** ao da Issue local. A primeira correspondência anota a Issue local:
...
| tier desconhecido | Qualquer | Manual com aprovação | Fallback seguro |
