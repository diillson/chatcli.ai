...
- `remediation_failed` — quando um plano termina Failed ou RolledBack (registrado uma vez, na transição)
- `approval_requested` — quando um plano fica em espera por um humano, por ApprovalPolicy, tier do cluster ou motor de decisão
- `approval_approved`, `approval_rejected`, `approval_expired` — quando o plano em espera sai de WaitingApproval, uma vez por decisão
- `notification_sent` — cada tentativa de entrega de uma NotificationPolicy ou canal de escalonamento, sucesso ou falha com o erro
- `slo_violation` — quando uma janela de burn rate dispara num ServiceLevelObjective
- `sla_breach` — quando um IncidentSLA registra uma violação de resposta ou resolução
...
Todo AuditEvent é criado com a annotation `platform.chatcli.io/immutable: "true"`. O operator não traz admission webhook, então a annotation é um marcador: imponha-a com uma regra de policy engine (Kyverno, Gatekeeper) que rejeite updates e deletes em recursos que a carregam.
...
As quatro ClusterRoles (`chatcli-role-viewer`, `chatcli-role-operator`, `chatcli-role-admin`, `chatcli-role-superadmin`) são **provisionadas pelo Helm chart ou pelo overlay kustomize**, nunca pelo operator em runtime (hardening H5: o operator não tem permissão para criar ClusterRoles). Elas dão acesso graduado aos CRDs da plataforma: viewers leem, operators também atualizam Issues, ApprovalRequests e PostMortems, admins gerenciam todos os recursos da plataforma.

Conceda um papel vinculando-o a um usuário ou grupo, como qualquer ClusterRole:
...
Revogar é apagar o binding. Mudanças de papel aparecem no audit log do Kubernetes; os AuditEvents do operator cobrem o que os controllers fazem, não quem recebeu qual papel.

<Note>
  A API REST e o dashboard usam um modelo próprio, API keys com `viewer`, `operator` ou `admin`, descrito em [Web Dashboard](/pt/features/aiops/web-dashboard). As ClusterRoles acima são para pessoas e ferramentas que chegam aos CRDs via `kubectl`.
</Note>
