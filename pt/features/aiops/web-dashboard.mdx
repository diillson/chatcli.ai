...
<AccordionGroup>
  <Accordion title="1. Overview">
    ...
    **Componentes:**
    ...
    | **Incidentes Recentes** | Os incidentes mais recentes com estado, severidade e timestamp, mais uma barra de filtros espelhando a aba Incidentes: severidade, estado, tipo de recurso, simulações de chaos e namespace, com botão Limpar. O filtro é client-side sobre os incidentes já carregados, reaplicado a cada auto-refresh, e a seleção fica guardada no `localStorage` (`chatcli_dash_recentFilters`). Todo cabeçalho de coluna ordena o painel (nome, severidade, estado, recurso, namespace, idade); a severidade ordena por peso, não pelo rótulo, e a ordenação é mantida por painel entre refreshes |
    | **Atividade de Remediação** | Os últimos incidentes com tentativas de remediação, em linha do tempo. Um botão alterna entre mais recentes primeiro (padrão) e mais antigos primeiro; a escolha fica guardada no `localStorage` (`chatcli_dash_remediationSort`) |
  ...
  <Accordion title="2. Incidents">
    ...
    **Badges de estado:**
    ...
    | **Contained** | **Roxo (#C79BFF) com animação pulse** — chama atenção visualmente porque há ação humana pendente |
  ...
  <Accordion title="5. AI Insights">
    ...
    **Funcionalidades:**
    ...
    | **Confiança** | Score de confiança colorido: verde (&ge;85%), amarelo (70-84%), vermelho (&lt;70%) |
