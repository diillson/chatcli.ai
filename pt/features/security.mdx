...
Toda linha da trilha de auditoria de LLM (`CHATCLI_AUDIT_LOG_PATH`) carrega `seq`, `prev_hash` e `hash` — `hash = SHA-256(prev_hash ‖ JSON canônico da entrada)`. Uma linha editada, removida ou reordenada quebra a cadeia dali em diante; um processo reiniciado continua a cadeia a partir do fim do arquivo. `/config security verify-audit [caminho]` re-hasheia a trilha e informa a primeira linha quebrada (entradas anteriores à cadeia contam como legadas).
