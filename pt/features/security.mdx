...
| 14 | **Rate Limiting** | Token bucket por sujeito autenticado no servidor gRPC — `sub` do JWT ou principal do certificado; por endereço para chamadores anônimos | <Icon icon="check" /> Ativo |
...
| 28 | **Bind Address** | Bind em `localhost` por padrão (não expõe na rede); um bind alcançável recusa subir sem credencial (token, segredo HS256, chave pública RS256 ou CA de cliente) | <Icon icon="check" /> Ativo |
...
| 35 | **mTLS** | O servidor exige e verifica certificado de cliente, via `CHATCLI_SERVER_TLS_CLIENT_CA`, e identifica o chamador por ele (`CHATCLI_MTLS_ROLE`) | <Icon icon="gear" /> Opt-in |
...
O TLS mútuo tem duas metades, e as duas são necessárias. No servidor, um bundle de CA de cliente torna obrigatório um certificado de cliente verificado:
...
chatcli connect servidor:50051 --tls --ca-cert ca.pem
```
...
O servidor implementa rate limiting com **token bucket** por cliente. O limitador roda **depois** da autenticação e usa como chave o sujeito do chamador — o claim `sub` do JWT, ou o principal do certificado sob mTLS — em vez do endereço que ele divide com todo outro tenant atrás do mesmo NAT ou ingress. Chamadores anônimos (deployment com token compartilhado ou em loopback) caem para o endereço. Rajadas não autenticadas são contidas à parte pelo limitador de falhas de autenticação por endereço.
...
<Tabs>
  <Tab title="17 tipos permitidos">
    ...
    ```
    <Note>O padrão é mais largo que um único tipo de workload de propósito: remediação que não pode tocar um Service ou um HPA é remediação que escala para um humano em trabalho de rotina.</Note>
    <Warning>**`CHATCLI_ALLOWED_RESOURCE_TYPES` soma a essa lista; não substitui.** Definir uma lista curta não restringe nada — os dezessete padrões continuam, e suas entradas entram por cima. Os kinds casam pela grafia exata (`Deployment`, não `deployments`), então um plural minúsculo não acrescenta nada. Para restringir de verdade o que o operator pode tocar, restrinja o RBAC dele: a Role do chart é namespace-scoped por padrão, e o operator só age no que a ServiceAccount concede.</Warning>
  </Tab>
...
| `CHATCLI_MTLS_ROLE` | Papel concedido a um chamador identificado só pelo certificado de cliente (sem bearer token): `viewer`, `user` ou `admin`. Valor não reconhecido vira somente-leitura. | `user` |
...
| `CHATCLI_KEYCHAIN_BACKEND` | Onde mora a chave de credenciais: `auto`, `file`, `keychain`. Um binário de teste Go nunca alcança o keychain do sistema em `auto` — só um `keychain` explícito — então rodar a suíte não pede permissão nem reescreve a chave do desenvolvedor | `auto` |
