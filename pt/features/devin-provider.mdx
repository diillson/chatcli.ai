...
- **Custo é derivado, não faturado** — cada turno lê o uso real de tokens de volta do export ATIF do CLI, e a tarifa é a que o CLI lista para a sua conta (`cost_summary`), então o `/cost` reporta o que o modelo roteado custa na tarifa por token da Cognition. ACUs não são modelados; a camada de pricing nunca cobra um `claude-*`/`gpt-*` roteado como se fosse a API direta, e um build sem `--export` degrada para a estimativa chars/4.
