...
<Tabs>
  <Tab title="Automático">
    ...
    1. **Nivel 1 — Trimming** (quase sem perda): remove tags `<plan>`, compacta XML, elimina duplicatas
...
### `/rewind compact` — desfazer uma compactação

Toda reescrita do histórico (auto-compact no chat, agent/coder e one-shot, `/compact`, recovery de overflow) guarda o histórico que substituiu. `/rewind compact` o traz de volta:
...
```text
↩ Compactação desfeita: 9 → 41 mensagens restauradas de memory. /rewind traz a visão compactada de volta.
```

- A visão compactada vira checkpoint antes, então `/rewind` reverte o undo.
- Depois de retomar (`/session load`), o snapshot é reconstruído a partir do [journal de transcript](/pt/features/session-management#journal-de-transcript): eventos `rewrite` registram os hashes ordenados do histórico que substituíram, e o journal guarda toda mensagem já vista. Um journal podado (além de `CHATCLI_SESSION_TTL`) significa nada a desfazer.
- O histórico restaurado é registrado no journal como nova reescrita, e a telemetria de cache de prompt conta um rebuild esperado.

### Limites

- Máximo de **20 checkpoints** são mantidos (os mais antigos são descartados)
- Checkpoints persistem com a sessão salva como listas de hash de mensagens e são reconstruídos do journal de transcript no load (descartados quando o journal não tem mais suas mensagens)
