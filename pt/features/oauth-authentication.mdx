...
<Warning>
A troca e o refresh de tokens usam um `http.Client` puro (sem transporte customizado) com header `User-Agent: claude-cli/2.1.259 (external, cli)` para evitar fingerprinting TLS pelo Cloudflare — e a versão precisa acompanhar um release atual do Claude Code: a API recusa os modelos mais novos para versões antigas (`claude_code_version_too_old`). Ela vive numa única constante (`auth.ClaudeCodeVersion`) compartilhada por login, refresh, messages e models. Usar um HTTP client com `LoggingTransport` causa rejeição pela CDN da Anthropic.
</Warning>
