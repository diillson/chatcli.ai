...
<Warning>
A troca e o refresh de tokens usam um `http.Client` puro (sem transporte customizado) com header `User-Agent: claude-cli/2.1.281 (external, cli)` para evitar fingerprinting TLS pelo Cloudflare — e a versão precisa acompanhar um release atual do Claude Code: a API recusa os modelos mais novos para versões antigas (`claude_code_version_too_old`; o Opus 5.5 exige 2.1.280+). O piso vive numa única constante (`auth.ClaudeCodeVersion`) compartilhada por login, refresh, messages e models; quando a API pede um release mais novo, o ChatCLI lê o release exigido do erro, passa a apresentá-lo no processo e reenvia a mesma requisição — `CHATCLI_CLAUDE_CODE_VERSION` fixa um release manualmente. Usar um HTTP client com `LoggingTransport` causa rejeição pela CDN da Anthropic.
</Warning>
...
O tempo de vida é definido pelo provedor que emitiu o token: o campo
`expiry` é verificado antes de cada requisição e um token expirado dispara
o fluxo de refresh automaticamente. O ChatCLI não impõe um teto próprio —
um ambiente com exigência de rotação a aplica no provedor de identidade,
ou revogando a credencial e rodando `/auth login` de novo.
