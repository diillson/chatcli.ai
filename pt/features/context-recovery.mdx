...
Com a [camada de compressão](/pt/features/context-compression) ativa, cada mensagem é **arquivada verbatim no store CCR antes do primeiro corte** e o conteúdo encolhido carrega um marcador `<<ccr:KEY>>` — o modelo recupera o original a qualquer momento com `@recall`. A truncagem nuclear (nível 3 da escada de recovery) adicionalmente limita cada mensagem não-system mantida a 4.000 chars.
...
- **Hooks veem toda compactação.** `PreCompact`/`PostCompact` disparam com `trigger` `auto`, `manual` ou `recovery` ([hooks](/pt/features/hooks-system#eventos-disponíveis)).
- **É contabilizado.** `/cost` mostra o número de compactações, quantas caíram no Nível 3 e quanto o sumarizador custou; os contadores persistem com a sessão ([cost tracking](/pt/features/cost-tracking)).

O `/compact <instrução>` guiado segue as mesmas regras: usa a rota de sumarizador configurada (`CHATCLI_COMPACT_SUMMARIZER_PROVIDER/MODEL`) quando existe, dá ao resumo sua própria janela de 10 minutos em vez do antigo prazo de 60 segundos do comando, mantém mensagens `PreserveVerbatim`, arquiva o segmento e roda o gate de qualidade.
...
|:--------------------|:----------|:--------|
| `CHATCLI_CONTEXT_WINDOW` | Override **global** da janela de contexto (em tokens), para qualquer provider/modelo. Tem precedência sobre o catálogo. Use quando a janela real do seu gateway/agente difere da assumida pelo ChatCLI — o orçamento de compactação deriva desse valor. | _(auto via catálogo)_ |
...
| `CHATCLI_COMPACT_MODEL` | `PROVIDER:modelo` dedicado ao resumo estruturado de Nível 2 (mais barato/rápido que o modelo da sessão) | _(cliente da sessão)_ |
...
<CardGroup cols={2}>
  <Card title="Tool Result Budget" icon="database" href="/pt/features/tool-result-management">
    O orcamento de resultados e a primeira linha de defesa. A recuperação ativa quando o orcamento não foi suficiente.
  </Card>
  <Card title="Microcompactacao" icon="compress" href="/pt/features/tool-result-management">
  ...
  <Card title="Cost Tracking" icon="money-bill-trend-up" href="/pt/features/cost-tracking">
