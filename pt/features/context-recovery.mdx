...
<Tip>
Duas features estruturais encolhem esse piso: as descrições de ferramentas MCP são limitadas a uma linha no índice do system prompt (schema completo via `@tools describe`), e os corpos de skills injetadas respeitam [`CHATCLI_SKILL_INJECT_BUDGET`](/pt/features/skill-registry#budget-de-injeção-de-skills). Veja [Integração MCP](/pt/features/mcp-integration) e [Skill Registry](/pt/features/skill-registry).
</Tip>
