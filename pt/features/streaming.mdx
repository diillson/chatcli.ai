...
O `chatcli server` faz streaming do mesmo jeito: `StreamPrompt` repassa cada fragmento conforme o provider emite (sem fatiar depois uma resposta bufferizada) e a última mensagem traz o usage e o stop reason. O cliente remoto por trás do `chatcli connect` implementa `StreamingClient`, então uma CLI conectada faz streaming como uma local e o `/cost` dela precifica os tokens informados. Veja [modo servidor](/pt/features/server-mode#streaming).
...
| **Moonshot (Kimi)** | Sim | Streaming compatível com OpenAI |
...
<CardGroup cols={2}>
  ...
  <Card title="Provider Fallback" icon="arrow-right-arrow-left" href="/pt/features/provider-fallback">
  ...
  <Card title="UI de Progresso" icon="spinner" href="/pt/features/agent-progress-ui">
