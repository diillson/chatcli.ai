...
| `outline` | Esqueleto de símbolos de um arquivo (declarações + linhas) | `--file` (obrig.) |
| `map` | Mapa estrutural do repositório com budget, arquivos ranqueados por estrutura | `--dir`, `--budget` |
...
| `checkpoint` | Snapshots shadow-git do workspace (automáticos antes de mutações) | `--list`, `--create`, `--restore`, `--label` |
...
<AccordionGroup>
  ...
  <Accordion title="outline -- Esqueleto de Símbolos do Arquivo" icon="list-tree">
    ...
    | Flag | Tipo | Obrigatório | Default | Descrição |
    |---|---|---|---|---|
    | `--file` | string | **Sim** | -- | Caminho do arquivo a ser esquematizado |
  ...
  <Accordion title="map -- Mapa Estrutural do Repositório" icon="map">
    ...
    | Flag | Tipo | Obrigatório | Default | Descrição |
    |---|---|---|---|---|
    | `--dir` | string | Não | `"."` | Diretório raiz a mapear |
    | `--budget` | int | Não | 8000 | Máximo de caracteres de estrutura a emitir |
    ...
    <Tabs>
      <Tab title="JSON">
        ```xml
        <!-- Mapear o projeto inteiro -->
        <tool_call name="@coder" args='{"cmd":"map","args":{"dir":"."}}'/>

        <!-- Mapear uma subárvore com budget maior -->
  ...
  <Accordion title="checkpoint -- Snapshots Shadow-git" icon="clock-rotate-left">

    Snapshots do workspace num **repositório git sombra**, uma rede de segurança mais forte que o rollback por `.bak` — cobre edições multi-arquivo e efeitos colaterais do `exec`, não apenas um arquivo.

    Um checkpoint é tirado **automaticamente antes de todo subcomando mutante** (`write`, `patch`, `multipatch`, `exec`). O snapshot vive num `GIT_DIR` separado sob `~/.chatcli/checkpoints/<hash>` com o seu workspace como work tree, então o **seu próprio `.git` nunca é tocado** e o `.gitignore` do projeto é respeitado.

    ### Flags

    | Flag | Tipo | Obrigatório | Default | Descrição |
    |---|---|---|---|---|
    | `--list` | bool | Não | (padrão) | Lista os snapshots, mais recentes primeiro |
    | `--create` | bool | Não | `false` | Grava um snapshot agora |
    | `--restore` | string | Não | -- | Rebobina a árvore de trabalho para o snapshot com este hash |
    | `--label` | string | Não | `manual` | Rótulo para um snapshot `--create` |
    ...
    <Tabs>
      <Tab title="JSON">
        ...
        <!-- Gravar um agora -->
        <tool_call name="@coder" args='{"cmd":"checkpoint","args":{"create":true,"label":"antes do refactor"}}'/>

        <!-- Rebobinar para um snapshot -->
      ...
      <Tab title="CLI-style">
        ```
        checkpoint --list
        checkpoint --create --label "antes do refactor"
    ...
    <Note>
    O `--restore` nunca apaga arquivos adicionados **depois** do snapshot; ele só rebobina o que o snapshot rastreava. Passa pelo [gate de segurança](/pt/features/coder-security); `--list` e `--create` são somente leitura.
    </Note>

    <Info>
    Auto-snapshots são **best-effort** (falha de checkpoint nunca bloqueia a edição), **throttled** a no máximo um por 15 segundos (uma rajada de writes gera um checkpoint), um **no-op quando o `git` não está instalado**, e desligados por completo com `CHATCLI_CODER_CHECKPOINTS=off`.

    Eles também são **limitados**: workspaces amplos como o seu diretório home (ou qualquer diretório que o contenha) nunca recebem snapshot automático — hashear tanto disco congelaria todos os comandos — e cada snapshot roda sob um deadline rígido (10s automático, 60s para um `checkpoint create` explícito, sobrescrevível em segundos via `CHATCLI_CODER_CHECKPOINT_TIMEOUT`). Falhas repetidas de snapshot fazem backoff exponencial e, após três seguidas, desativam snapshots automáticos pelo resto da sessão com um aviso de uma linha.
    </Info>
