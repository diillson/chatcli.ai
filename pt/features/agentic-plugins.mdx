...
A maioria das ferramentas de IA para linha de comando funciona como **assistentes**: você pergunta, elas respondem. O ChatCLI vai alem, transformando a IA em um **agente autônomo** que não apenas responde, mas **age**.
...
<Steps>
  <Step title="Monitora o diretório">
...
Plugins Instalados (12):
  ...
  @browser        - Controla um Chrome local (DevTools)     [builtin]
  @forge          - PRs, issues, CI via gh/glab             [builtin]
  @view           - Anexa uma imagem local à conversa       [builtin]
  @proc           - Processos em background e PTY interativo [builtin]
...
|---|---|---|
| `@coder` | Engenharia completa (read, write, patch, multipatch, **outline/map**, exec, test, git, **checkpoint**) | [Plugin @coder](/pt/features/coder-plugin) |
| `@taskgraph` | Executa um plano multi-task aprovado como um DAG verificado: workers paralelos por task, gates de validação executados pelo próprio engine e veredito de reviewer independente antes de qualquer coisa contar como done — mais uma dashboard live no browser | [Task Graph](/pt/features/task-graph) |
| `@browser` | Controla um Chrome local de verdade pelo protocolo DevTools: open, snapshot (elementos interativos numerados), click, type, eval, screenshot, console e captura de rede — o loop de verificação para trabalho web | [Automação de Navegador](/pt/features/browser-automation) |
| `@forge` | Pull requests, issues e CI pelo `gh`/`glab` autenticado do próprio usuário (keyless): lista/vê/diff de PRs, acompanha os checks, lê logs de runs que falharam e — atrás de confirmação — cria PRs e comenta | [Git Forge](/pt/features/git-forge) |
| `@view` | Anexa uma imagem local (screenshot, mock, diagrama) à conversa pelo pipeline de visão para o modelo **olhar** para ela no meio da tarefa | [Entrada de Imagem](/pt/features/vision-input) |
| `@read`, `@search`, `@tree` | **Tools atômicos** narrow para read-only ops | [Tools Atômicos](/pt/features/atomic-tools) |
| `@todo` | Plano de tarefas do agente (TodoWrite parity) | [Tools Atômicos](/pt/features/atomic-tools) |
| `@websearch`, `@webfetch` | Acesso a web (DuckDuckGo/SearxNG + HTTP GET) | [Web Tools](/pt/features/web-tools) |
| `@scheduler` | Scheduler durável (schedule, wait, query, cancel) | [Scheduler](/pt/features/scheduler) |
| `@proc` | Processos em background: start/status/logs/stop de servidores e watchers que precisam continuar rodando enquanto o agente testa contra eles (árvore de processos morre com a sessão). Suporta **sessões PTY interativas** — `start {pty:true}` roda um programa num pseudo-terminal real (REPLs, debuggers, ssh) e `stdin {id,text}` digita nele (Unix-first) | — |
| `@http` | Cliente HTTP estruturado para testar APIs: método/headers/body explícitos, status/latência/body limitado de volta; par natural do `@proc` | — |
| `@api-explorer` | Reconhecimento de API de ponta a ponta só com a URL base: descobre a spec OpenAPI/Swagger (paths conhecidos, HTML de Swagger-UI/ReDoc, /.well-known), resolve modelos, audita segurança e faz introspection de GraphQL | [API Explorer](/pt/features/api-explorer) |
| `@lsp` | Navegação semântica de código via language servers: diagnostics, definition, references, symbols, hover | [LSP](/pt/features/lsp-diagnostics) |
| `@tools` | Catálogo de tools: expande a definição completa de qualquer tool do índice deferred sob demanda — builtins e tools MCP (`mcp_*`, com JSON Schema e servidor de origem) | — |
| `@mcp-login` | Autoriza um servidor MCP remoto protegido por OAuth (login/status/logout) para suas tools `mcp_*` ficarem usáveis — o agente a chama quando uma tool reporta "authorization required" | [Integração MCP](/pt/features/mcp-integration#autorização-oauth-21-servidores-remotos) |
| `@model` | Roteamento de modelo/provider pela própria IA: lista modelos com tier de preço, custo por 1M de tokens, context window e capabilities; troca o restante da tarefa para um modelo mais adequado; ou delega uma sub-tarefa a um mais barato | [Roteamento de Modelo](/pt/features/model-routing) |
| `@dash` | A dash ao vivo como tool: sobe e devolve o endereço, para, lê o grafo reduzido ou os eventos mais recentes deste ou de todos os processos chatcli, marca uma fase na linha do tempo | [Dash ao Vivo](/pt/features/live-dashboard#dirigindo-pelo-agent) |
| `@park` | Pausa o agente para retomar (delay, until, for_url, for_cmd); inspeciona e envia diretivas a agents estacionados (list, note) | [Agent Park](/pt/features/agent-park) |
| `@compress`, `@recall` | Compressão reversível sob demanda + recuperação do original (CCR) | [Compressão de Contexto](/pt/features/context-compression) |
...
<Tip>Quando `CHATCLI_AGENT_PARALLEL_MODE=true`, o LLM orquestrador pode despachar 12 agents especialistas (FileAgent, CoderAgent, ShellAgent, GitAgent, SearchAgent, PlannerAgent, ReviewerAgent, TesterAgent, RefactorAgent, DiagnosticsAgent, FormatterAgent, DepsAgent) em paralelo. Veja a [documentação completa](/pt/features/multi-agent-orchestration).</Tip>
...
<Steps>
  ...
  <Step title="Execução (Invocação do Plugin)">
...
<Note>
**Segurança do loop (doom-loop guard).** Um guarda de loop de ferramentas observa o ciclo ReAct em busca de padrões travados. Além de *falhas* repetidas da mesma ferramenta, ele agora também detecta a mesma chamada **bem-sucedida** repetida — intercalada ou não — sem nenhuma mudança de estado no meio (ex.: reler o mesmo arquivo várias vezes): o resultado não pode mudar, então ele orienta o modelo a reutilizar a saída anterior em vez de queimar turnos. Qualquer ferramenta bem-sucedida com efeitos colaterais (um write, um patch, um exec) reseta o rastreamento, então uma releitura legítima depois de uma edição nunca dispara o alerta.
</Note>
...
<Steps>
  ...
  <Step title="Comunicação via I/O Padrão">
    | Canal | Uso | Descrição |
    |-------|-----|-----------|
...
<Steps>
  ...
  <Step title="Mover para o diretório de plugins">
    ```bash
...
<CardGroup cols={2}>
  <Card title="Validação de Entrada" icon="shield-check">
...
O modo [/coder](/pt/core-concepts/coder-mode) e especializado em engenharia de software e utiliza o plugin [@coder](/pt/features/coder-plugin) para executar suas ações. O `@coder` e um **plugin builtin** -- já vem embutido no ChatCLI e funciona sem instalação.
...
Veja mais em [Modo Coder](/pt/core-concepts/coder-mode) e [Plugin @coder](/pt/features/coder-plugin).
