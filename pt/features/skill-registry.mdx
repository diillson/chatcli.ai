...
### Medir o Que uma Skill Faz (`/skill stats`)

Todo run de agent e coder é creditado às skills injetadas nele — fixadas, auto-ativadas por trigger ou criadas pelo motor de self-evolve — com seus turnos, tool calls e custo, ao lado de uma baseline de todos os runs. `/skill stats` põe os dois lado a lado, por run, para que uma skill seja julgada pelo que muda e não por quantas vezes dispara: um percentual negativo significa que os runs que a usaram foram mais curtos ou mais baratos que a baseline; uma skill aprendida que fica positiva é candidata a poda. O ledger fica em `~/.chatcli/skills/.skill-stats.json` e é neutro de provider: turnos e tool calls vêm do registro de runs, custo do tracker, que todos os providers alimentam. Cada linha também diz se a skill foi **usada**, não só ativada: para uma skill que declara `allowed-tools`, um run conta como uso quando uma dessas tools executou depois da injeção (`tools usadas em 3 de 4 runs`); uma skill que não declara tools mostra `uso de tools não declarado`, porque chutar seria pior que não saber.

```bash
/skill stats           # Todas as skills contra a baseline, mais usadas primeiro
/skill stats <nome>    # Uma skill
```
...
|------|--------------|--------|------------------------|
| **Auto-activation** | `triggers:` (substring contígua, ou todas as palavras de um trigger multi-palavra presentes como tokens inteiros em qualquer ordem) ou `paths:` (glob contra paths do input) batem | per-turn (re-avalia toda mensagem) | `triggers:` ou `paths:` (sem `disable-model-invocation: true`) |
...
Detalhes de cada modo, frontmatter e exemplos: veja [Frontmatter avançado em Customizable Agents](/pt/features/customizable-agents#padroes-avancados-de-frontmatter).
...
<Info>
**Por que isso existe:** um único prompt pode auto-ativar uma rajada de skills (ex.: nove skills de ServiceNow casando com uma pergunta), somando dezenas de KB a **cada requisição subsequente**. Atrás de um proxy/WAF corporativo com limite de tamanho de corpo, só isso já empurra a sessão para além do limiar de rejeição — veja [Context Recovery](/pt/features/context-recovery). O default é generoso: sessões típicas inlinam todas as skills ativadas sem corte.
</Info>
...
```text
  ...
  --------------------------------------------------
    ...
    /skill stats [name]                  Effectiveness of each skill against the baseline of every run
...
- [**Customizable Agents**](/pt/features/customizable-agents) — Como criar e usar agentes com skills
- [**Referência de Comandos**](/pt/reference/command-reference) — Lista completa de todos os comandos
- [**Configuração (.env)**](/pt/reference/configuration) — Variáveis de ambiente disponíveis
