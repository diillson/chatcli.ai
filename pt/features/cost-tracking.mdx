...
| Devin CLI | — | O binário não reporta uso; tokens são estimados, custo é $0 (o gasto vive na assinatura Cognition) |
...
- **Agent / Coder** — cada turno ReAct, mais os **workers/subagents despachados** (o cliente de cada worker é decorado com um tally de uso; a soma é registrada sob o provider+modelo do próprio worker)
- **Painéis MoA** — caminhos native-tools, XML e plain, atribuídos por participante
- **One-shot (`-p`)**, turnos **RPC/Gateway/ACP** e **execuções agendadas** (o scheduler recebe de volta números reais de tokens e custo)
...
### Anthropic (cache write = 1,25× input, cache read = 10% do input)
...
| claude-opus (3 / 4.0 / 4.1 legado) | $15.00 | $75.00 |
| claude-sonnet (todos) | $3.00 | $15.00 |
...
| Modelo | Input | Output |
|:-------|:------|:-------|
| gpt-5.6 (Sol / alias de família) | $5.00 | $30.00 |
...
| gemini-3.1-pro / gemini-3 | $2.00 | $12.00 |
...
| Modelo | Input | Output |
|:-------|:------|:-------|
| glm-5.2 / glm-5.1 | $1.40 | $4.40 |
| glm-5-turbo / glm-5v-turbo | $1.20 | $4.00 |
| glm-5 | $1.00 | $3.20 |
| glm-4.x e outros ids Z.AI | $0.50 | $0.50 |
...
| kimi / moonshot (genérico, incl. k2.7-code, k2.6) | $0.95 | $4.00 |
...
| Ollama, StackSpot, Devin | $0 — não tarifados do ponto de vista do ChatCLI |
...
- **Anthropic / Bedrock Anthropic** reportam tokens de cache *ao lado* de `input_tokens` (aditivos): write cobrado a 1,25× do input, read a 10%.
- **OpenAI, Gemini, DeepSeek, Moonshot** reportam cache reads como *subconjunto* da contagem de prompt: a fatia cacheada é recortada do input e cobrada uma única vez na taxa com desconto — nunca preço cheio mais desconto.
...
O aviso é **proativo**: chat, agent e coder o imprimem no turno em que a sessão cruza um nível — você não precisa lembrar de rodar `/cost`. Sem `CHATCLI_BUDGET_HARD_STOP`, exceder o orçamento nunca interrompe a sessão; com ele, novos turnos (chat, loop do agent, RPC/gateway) são bloqueados até o limite subir ou o `/cost reset` abrir um novo período.
