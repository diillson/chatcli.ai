...
| **Chat** | ✅ (exceção controlada) | Chat é tool-less por design; abre exceção **apenas** para `ask_user`, a [consulta read-only à knowledge base](/pt/features/knowledge-base), o [`@graphview`](/pt/features/graphview), a [memória/perfil](/pt/features/bootstrap-memory) e o [`context_pull`](/pt/features/prefix-curation) — nunca para tools de execução |
...
O modo chat é **tool-less por design** — ele nunca executa tools. As exceções sancionadas são cinco, todas inofensivas por construção: o `ask_user` (interativa), a [consulta read-only à knowledge base](/pt/features/knowledge-base) (`/config chat knowledge`), o [`@graphview`](/pt/features/graphview) (renderização única, `/config chat graphview`), o [`context_pull`](/pt/features/prefix-curation) (recuperação read-only do material curado para fora do prefixo, `/config chat pull`) e a [memória](/pt/features/bootstrap-memory) (`/config chat memory` — persiste fatos duráveis que você revela no mesmo turno **e** carrega o caminho de pull que permite ao chat honrar o modo de memória `index`: digest compacto, recall sob demanda, blocos proativos de auto-recall/session-recall/card do grafo). Quando habilitadas, o chat oferece **exclusivamente** essas ferramentas (nenhuma de leitura de arquivos/escrita/execução).
