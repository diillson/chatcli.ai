...
<Info>O dispatch multi-agent é o motor; a camada [Agent Squad](/pt/features/agent-squad) constrói um time completo em cima dele: observabilidade de runs ao vivo (`/agents`), board de trabalho compartilhado (`/board`), mensagens agent-a-agent (`/mail`) e um playbook de entrega autônoma.</Info>
...
## Navegação de Contexto pelos Workers

Workers não ficam cegos às superfícies de contexto da sessão — eles navegam conteúdo truncado e comprimido do mesmo jeito que o orquestrador:

- **Recall sobre compressão**: outputs grandes de tools dentro do loop do worker são comprimidos reversivelmente (CCR). Todo worker carrega a tool universal `recall_output`, então qualquer marcador `<<ccr:KEY>>` que ele encontrar pode ser expandido de volta ao conteúdo original.
- **Truncagem recuperável**: resultados acima do limite inline são salvos em arquivos `result_*.txt` no workspace da sessão, e o prompt do worker o ensina a ler esses arquivos com `read` (usando `start`/`end`) em vez de adivinhar. O **output final** do worker segue o mesmo contrato — ao exceder o orçamento de ~30 KB, o texto completo é persistido e a referência viaja de volta ao orquestrador.
- **Recall proativo**: cada task despachada é confrontada com a memória persistente e as sessões salvas; blocos `[MEMORY AUTO-RECALL]` / `[SESSION RECALL]` relevantes são injetados automaticamente no contexto inicial do worker.
- **Context tools opt-in**: personas que declaram `Memory`, `Session` ou `Knowledge` no frontmatter `tools` ganham tools read-only de busca/leitura sobre essas camadas — veja [Agents Customizáveis](/pt/features/customizable-agents).
- **Microcompact no loop**: runs longos de worker compactam progressivamente tool results antigos (com CCR, recuperável), o mesmo mecanismo do orquestrador.
- **Prompt especialista preservado**: em modo native function-calling o prompt especialista do agent — incluindo personas customizadas — é mantido, aposentando apenas a sintaxe XML de tools.

Os resultados do squad injetados de volta no histórico do orquestrador são envelhecidos pelo mesmo motor de microcompact, então um dispatch grande de múltiplos workers não fica mais incompactável na janela de contexto.
...
As tool calls dos workers chegam ao motor de policy na mesma forma canônica `@coder <subcomando>` do modo sequencial, então regras escritas uma vez valem em todo lugar. O `/policy mode auto` também cobre workers ("ask" auto-aprova), com operações safety-immune ainda pedindo confirmação. Tools puramente in-process — squad mail, `recall_output` e as tools read-only de Memory/Session/Knowledge — não têm efeito em filesystem, shell ou rede e pulam o check de policy.
