...
<Tabs>
  ...
  <Tab title="AWS Bedrock (catálogo completo)">
    ...
    <Note>
    **Nova geração = IDs dateless.** Fable 5, Opus 5, Sonnet 5, Opus 4.8 e Opus 4.7 **não têm IDs ARN-versionados** no Bedrock — os IDs são `anthropic.claude-fable-5`, `anthropic.claude-opus-5`, `anthropic.claude-sonnet-5`, `anthropic.claude-opus-4-8` e `anthropic.claude-opus-4-7`. Opus 4.8/4.7 são invocados pelo inference profile `global.` (o ID puro não é invocável on-demand no `InvokeModel`). Os IDs datados antigos (ex.: `global.anthropic.claude-opus-4-8-20260528-v1:0`) continuam resolvendo como aliases. **Opus 5, Sonnet 5 e Fable 5 são servidos pelo endpoint Messages** (`bedrock-mantle.{região}.api.aws/anthropic/v1/messages`) — o ChatCLI detecta e roteia automaticamente (SigV4 `bedrock-mantle` ou `AWS_BEARER_TOKEN_BEDROCK`) e, se a chamada Mantle falhar, refaz a mesma requisição pelo runtime InvokeModel legado sob o inference profile `global.`; veja [AWS Bedrock](/pt/features/bedrock).
    </Note>
  ...
  <Tab title="Google (Gemini)">
    ...
    | `gemini-3` | `gemini-3-pro`, `gemini-3-pro-preview` | 1M tokens | 65K tokens | 👁 Vision, 🔧 Tools, 📋 JSON, 💻 Code Exec |
    ...
    | `gemini-2.0-flash` | — | 1M tokens | 8K tokens | 👁 Vision, 🔧 Tools, 📋 JSON |
    | `gemini-2.0-flash-lite` | — | 1M tokens | 8K tokens | — |

    <Info>
    Toda a geração Gemini 3.x compartilha a janela de entrada de 1.048.576 tokens com teto de saída de 65.536 tokens (`gemini-3.7-flash` é o workhorse GA atual, ago/2026). O Gemini 2.5 Flash Lite também suporta **Multimodal Live** para interações em tempo real; modelos com JSON Mode podem retornar saída estruturada via `response_mime_type`.
    </Info>
  </Tab>
  <Tab title="xAI (Grok)">
    ...
    | `grok-4.5` | `grok-4.5-latest` | 500K tokens | — | 👁 Vision, 🔧 Tools, 📋 JSON |
    ...
    | `grok-build-0.1` | `grok-build` | 256K tokens | — | 👁 Vision, 🔧 Tools, 📋 JSON |
    ...
    <Note>
    Os modelos Grok usam a API compatível com OpenAI. A xAI não publica teto de output por modelo, então os limites são gerenciados pelo provedor. `grok-4.6` (12/ago/2026) é o flagship atual; a variante fast anunciada não tem model ID publicado e por isso não está no catálogo.
    </Note>
  </Tab>
  ...
  <Tab title="Devin CLI (Cognition)">
    ...
    | Google | `gemini-3.6-flash` · `gemini-3.5-flash` · `gemini-3.1-pro` · `gemini-3-flash` |
    | Outros | `glm-5.2` · `kimi-k3` / `k2.7` / `k2.6` · `deepseek-v4-pro` |
