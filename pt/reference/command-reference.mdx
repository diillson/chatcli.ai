...
| `/context attach <nome>` | Anexa contexto à sessão atual (`--rag [K]` retrieval semântico, `--full`/`-f` conteúdo inteiro, `--chunk(s)`, `--priority`; contextos grandes auto-promovem para RAG com embeddings ativos) |
...
| `/context status` | Inspector ao vivo: do que o último system prompt montado é feito (seção a seção, com marcas de prefixo cacheado), o histórico da conversa por papel, estimativas de tokens pela razão chars/token aprendida, parcela projetada da janela do modelo e onde o auto-compact dispara |
...
| `/autocompact [60% \| 0.6 \| 150k \| default]` | Mostra ou define, para esta sessão, o quão cheia a janela fica antes do auto-compact disparar (percentual, fração ou contagem de tokens; `default` restaura o padrão do modo: 75% no chat, 60% no coder) |
