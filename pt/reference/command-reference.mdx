...
| `/config retention` | Política de ciclo de vida de todo store em disco numa só visão: sessões de máquina, journals de transcript, park snapshots e snapshots de custo (todos pelo `CHATCLI_SESSION_TTL`, varridos no boot), TTL e limite do arquivo CCR, execuções de task graph, notas diárias e limite de fatos da memória, purge de ociosos do hub |
