...
| `LLM_PROVIDER` | Define o provedor de IA padrão a ser usado. Valores válidos: `OPENAI`, `OPENAI_ASSISTANT`, `CLAUDEAI`, `BEDROCK`, `GOOGLEAI`, `XAI`, `ZAI`, `MINIMAX`, `MOONSHOT`, `OPENROUTER`, `STACKSPOT`, `OLLAMA`, `COPILOT`, `DEVIN`. | `"OPENAI"` |
...
| `CHATCLI_THEME` | Tema de cores da interface inteira (chat, cards, markdown, spinners). **12 temas:** `dark`, `light`, `grafite` + 9 da comunidade. Troca em runtime via `/config ui theme`. Veja a seção [Tema de Cores](#tema-de-cores). | `"dark"` |
...
São **12 temas**: `dark`, `light` e `grafite` (variantes calibradas do ChatCLI) + nove paletas da comunidade — `dracula`, `nord`, `tokyo-night`, `solarized-dark`, `solarized-light`, `gruvbox`, `catppuccin-mocha`, `monokai`, `one-dark`. Previews em cores reais de cada um estão no [Sistema de Tema](/pt/features/ui-theme).
...
<Tip>
Autocomplete: `/config ui theme <TAB>` (ou `/config theme <TAB>`) oferece os 12 temas.
</Tip>
...
| `OPENAI_MODEL` | O modelo a ser usado. Ex: `gpt-5.6-sol`, `gpt-5.6-luna`, `gpt-5.4`, `gpt-4o`. | Não |
| `OPENAI_ASSISTANT_MODEL` | O modelo a ser usado especificamente para a API de Assistentes. Nota: a OpenAI desligou o endpoint da Assistants API em 26/ago/2026 — prefira o provider `OPENAI` (Responses API). | Não |
| `OPENAI_USE_RESPONSES` | Define `true` para forçar a API `v1/responses` em todos os modelos. Definir `false` não força `v1/chat/completions`: sessões OAuth e modelos cuja entrada no catálogo prefere a Responses API (ex.: `gpt-5.4`) continuam usando-a. | Não |
| `OPENAI_API_URL` | Endpoint customizado de Chat Completions. Deve ser a URL completa terminando em `/chat/completions`; a URL de listagem `/models` é derivada dela. Com host customizado a listagem de modelos não é filtrada por família, e `OPENAI_API_KEY` continua sendo enviada como Bearer token (configure-a com a key do gateway). | Não |
| `OPENAI_RESPONSES_API_URL` | Endpoint customizado da Responses API. Um host custom em `OPENAI_API_URL` força a superfície de chat completions (a preferência do catálogo pela Responses API só vale no host oficial), então isto só é necessário ao optar explicitamente com `OPENAI_USE_RESPONSES=true` em um gateway que exponha a Responses API. | Não |
...
| `ANTHROPIC_SPEED` | Defina `fast` para opt-in no fast mode do Opus 5.5 / Opus 4.8 (research preview, pricing premium). | Não
...
| `XAI_MODEL` | O modelo a ser usado. Ex: `grok-4.3` (padrão), `grok-4.6`, `grok-4.5`, `grok-build-0.1`. | Não |
...
| `ZAI_MODEL` | O modelo a ser usado. Ex: `glm-5.3`, `glm-5.3-flash`, `glm-5`, `glm-4.7`. | Não |
| `ZAI_MAX_TOKENS` | Define o máximo de tokens para a resposta. | Não |
| `ZAI_USE_CODING_PLAN` | `true` troca para o endpoint do GLM Coding Plan (assinatura) — mesma API key, uso coberto pelo plano e reportado a $0 pelo `/cost`. Uma `ZAI_API_URL` explícita vence o toggle. | Não |
| `ZAI_THINKING` | Modo thinking do GLM: `enabled` ou `disabled` (sem definir, mantém o default do backend). | Não |
...
| `MOONSHOT_MODEL` | Modelo a usar (`kimi-k3`, `kimi-k2.7-code`, `kimi-k2.7-code-highspeed`, `kimi-k2.6`). | Não |
...
| `MOONSHOT_API_URL` | Endpoint customizado. Padrão: `https://api.moonshot.ai/v1/chat/completions`. Com host customizado, a listagem de modelos não é filtrada por família. | Não |
...
| `OPENROUTER_FALLBACK_MODELS` | Lista de modelos de fallback separados por vírgula. O OpenRouter tenta automaticamente o próximo modelo se o principal falhar. Ex: `anthropic/claude-sonnet-5,google/gemini-2.5-flash`. | Não |
...
| `COPILOT_MODEL` | O modelo a ser usado. Ex: `gpt-4o`, `claude-sonnet-4`, `gemini-2.5-flash`. | Não |
...
| `BEDROCK_ANTHROPIC_ENDPOINT` | Roteamento do wire Claude: `auto` (default, por catálogo com fallback Mantle→InvokeModel), `mantle` (pinado, sem fallback) ou `invoke` (só InvokeModel legado). | Não |
...
### Segurança de Execução do Coder

| Variável | Descrição | Padrão |
| :--- | :--- | :--- |
| `CHATCLI_CODER_AUTODIAG` | Após um `write`/`patch`/`multipatch` bem-sucedido do `@coder`, injeta os diagnósticos LSP dos arquivos tocados no resultado da ferramenta como bloco `[DIAGNOSTICS]` (silencioso em arquivos limpos). Aceita `off` para desativar. | `on` |
| `CHATCLI_CODER_CHECKPOINTS` | Snapshots automáticos do workspace via shadow-git antes de todo subcomando mutante do `@coder`; rebobine com `@coder checkpoint --restore <hash>`. Nunca toca o `.git` do seu projeto. Aceita `off` para desativar. | `on` |
| `CHATCLI_CODER_CHECKPOINT_TIMEOUT` | Deadline rígido de um snapshot de checkpoint, em segundos inteiros (limitado a 10 minutos). Workspaces amplos como o seu diretório home nunca recebem snapshot automático, e falhas repetidas desativam snapshots na sessão. | `10` (auto) / `60` (manual) |
| `CHATCLI_CODER_SANDBOX` | Confinamento em nível de SO para o `@coder exec`: `off` · `workspace` · `strict` (sem rede) · `docker`/`podman`/`container`. sandbox-exec nativo (macOS) / bwrap (Linux) / container (qualquer SO, Windows incluso). | `off` |
| `CHATCLI_CODER_SANDBOX_IMAGE` | Imagem de container para o backend de sandbox Docker/Podman. | `alpine:3` |
...
| `CHATCLI_FALLBACK_MODEL_<PROVIDER>` | Modelo específico por provedor na cadeia. Ex.: `CHATCLI_FALLBACK_MODEL_CLAUDEAI=claude-sonnet-4-6`. | (modelo padrão) |
...
| `CHATCLI_MEMORY_MODE` | Modo de injeção de memória em todos os modos (chat, agent, coder): `index` (pull, digest + ferramenta de recall), `full` (push completo por turno) ou `off`. O chat honra `index` via a exceção da ferramenta de memória; com `CHATCLI_CHAT_MEMORY=off` cai em `full`. | `index` |
| `CHATCLI_MEMORY_AUTORECALL` | Bloco proativo `[MEMORY AUTO-RECALL]` no modo `index` (top-3 fatos + linha `Related (graph):`). | `true` |
| `CHATCLI_MEMORY_GRAPH` | Cache persistido do grafo de conhecimento (`graph.json`) + expansão de vizinhança no recall; `off` restaura derivação por chamada, sem expansão. | `true` |
| `CHATCLI_GRAPH_INDEX` | Injeção por turno do card map-of-content do grafo (pull via `@memory map`/`neighbors` sempre disponível). | `true` |
| `CHATCLI_CHAT_MEMORY` | Exceção da ferramenta de memória no chat: persistência de fatos/perfil no mesmo turno e o caminho de pull do modo `index` no chat. | `true` |
...
| Variável | Descrição | Padrão |
| :--- | :--- | :--- |
| `CHATCLI_AUTO_UPDATE` | Política de auto-update: `auto` aplica em staging silencioso em background, `notify` mostra o aviso na tela de boas-vindas e na própria sessão, `off` desliga ambos. Veja [Auto-Update](/pt/features/auto-update). | `notify` |
| `CHATCLI_DISABLE_VERSION_CHECK` | Desabilita a verificação automática de versão no startup (implica auto-update `off`). Útil para ambientes air-gapped ou CI/CD. | `false` |
...
| `CHATCLI_ENCRYPTION_KEY` | Criptografia em repouso opt-in para sessões salvas, autosaves, espelhos de sessão MCP e park snapshots (AES-256-GCM). | `""` |
...
| `CHATCLI_AUDIT_LOG_PATH` | Caminho absoluto da trilha de auditoria encadeada por hash (requisições ao LLM em toda superfície e operações gRPC compartilham a mesma); vazio = sem trilha. |
