...
|   |       |-- retriever.go   # Smart retrieval por keywords + expansão do grafo
|   |       |-- graph_cache.go # Cache persistido do grafo (graph.json)
...
|   |-- handler_session.go     # RPCs de prompt e sessão
|   |-- llm_route.go           # Roteamento: fallback chain, max_tokens, refresh de credencial, retry de recusa, usage
|   |-- handler_remote.go      # RPCs de descoberta de recursos
|   |-- handler_analysis.go    # RPCs AIOps
|   |-- handler_hub.go         # RPCs do hub de conversas
|   |-- pipeline.go            # RPCs de pipeline (engine de turno hospedado no servidor)
|   +-- auth.go, auth_mtls.go  # Bearer/JWT e identidade por certificado
...
| `Model` | `string` | Modelo ativo (ex: `"claude-sonnet-4-6"`) |
...
Histórico excede budget de tokens
  ...
  |-- Remove blocos <plan> do assistente
