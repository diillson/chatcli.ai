...
Quando um servidor MCP emite **push notifications** (JSON-RPC messages sem `id`), o ChatCLI as encaminha pro [MCP Channels](/features/mcp-channels) ring — uma estrutura durável que alimenta o system prompt, o banner de inbox e o trigger engine.
...
Funciona com os três transports: `sse` (via stream), `http` (via listener GET) e `stdio` (via notifications no stdout). Veja [MCP Channels](/features/mcp-channels) pra detalhes do que acontece **depois** do filtro.
