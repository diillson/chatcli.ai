...
| `CHATCLI_CONTEXT_ENGINE` | `builtin` or `mcp:<server>`: an MCP server exposing `context_compact` produces the compaction summary, with fallback to the embedded summarizer (see [Context recovery → External context engine](/features/context-recovery#external-context-engine-mcp)). | `builtin` |
...
| `CHATCLI_ENCRYPTION_KEY` | Opt-in encryption at rest for saved sessions, autosaves, MCP session mirrors and park snapshots (AES-256-GCM, key derived with HKDF; plaintext files migrate on next save). See [Security](/features/security#encryption-at-rest-sessions-autosaves-park-snapshots). | `""` (off) |
