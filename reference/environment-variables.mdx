...
| `BEDROCK_ANTHROPIC_ENDPOINT` | Claude wire routing: `auto` (default) is catalog-driven — Opus 5, Sonnet 5 and Fable 5 go to the Messages endpoint automatically and fall back to legacy `InvokeModel` if the Mantle call fails; `mantle` pins the Messages endpoint (no fallback); `invoke` pins legacy `InvokeModel`. | `auto` |
