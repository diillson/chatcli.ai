...
| `CHATCLI_PROMPT_CACHE_TTL` | Prompt-cache lifetime: `5m`, `1h` or `auto` (`auto` = `1h` during agent/coder turns, `5m` in chat and one-shot). On Anthropic's direct API (API key, token and OAuth) it sets the marker TTL; with explicit Gemini caching it is the resource lifetime. The hour keeps the cached prefix warm through longer idle gaps at a higher write rate (2× input instead of 1.25×), so it pays off for sessions that pause between turns and costs more for rapid-fire ones. Applies to every marker in the request (system, tools and the rolling conversation breakpoint); Bedrock keeps the wire default. See [Cost tracking → Prompt caching](/features/cost-tracking#prompt-caching-by-provider). | `5m` |
| `CHATCLI_PROMPT_CACHE_KEEPALIVE` | Keeps the 5-minute prompt-cache entry warm while the interactive session is idle by re-sending the previous request with `max_tokens: 0` every 4.5 minutes (up to twelve times), instead of paying the hour-long TTL's 2× write. `auto` arms it on models whose cache reads cost at most 5% of the input price (Claude Fable 5.1), `on` on every client that can refresh (Anthropic direct API), `off` never. Refreshes are booked in `/cost`; the gateway never refreshes. See [Prefix curation](/features/prefix-curation). | `auto` |
