...
| `CHATCLI_MCP_PERMISSION_TIMEOUT` | Max wait for one elicitation approval form (Go duration or plain seconds). On expiry the action denies fail-safe and the model is told the request went unanswered; later prompts in the same run fail fast. `0`/`off` waits until the call dies. | `120s` |
