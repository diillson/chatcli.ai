...
| `CHATCLI_TURNUI` | Opt-in para a UI split-pane no `/coder` (status no topo, input fixo embaixo, histórico ↑/↓, navegação ←/→). Apenas `on` ativa; qualquer outra coisa mantém o renderer legado. Requer terminal compatível com DECSTBM + ESC 7/8 (xterm, iTerm2, Hyper, Terminal.app, Alacritty). Veja [features/split-pane-ui](/features/split-pane-ui). | `""` (desativado) |
