...
<Tabs>
  ...
  <Tab title="Google (Gemini)">
    ...
    <Info>
    The whole Gemini 3.x generation shares the 1,048,576-token input window with a 65,536-token output cap (`gemini-3.7-flash` is the current GA workhorse, Aug 2026). Gemini 2.5 Flash Lite also supports **Multimodal Live** for real-time interactions; models with JSON Mode can return structured output via `response_mime_type`.
    </Info>
  </Tab>
  <Tab title="xAI (Grok)">
    ...
    <Note>
    Grok models use the OpenAI-compatible API. xAI publishes no per-model output cap, so limits are managed by the provider. `grok-4.6` (Aug 12 2026) is the current flagship; the announced fast variant has no published model ID yet and is therefore not in the catalog.
    </Note>
  </Tab>
  ...
  <Tab title="Moonshot (Kimi)">
    ...
    **Thinking mode:** Set `MOONSHOT_THINKING=enabled|disabled|auto` to toggle between Thinking (explicit reasoning, default for K2.6/K2.5) and Instant (direct response, cheaper). The default `auto` lets the model choose; models without the `thinking` capability ignore the flag.
    ...
    <Tip>
    **Public pricing (Aug 2026):** kimi-k3 is $3.00/M input tokens (cache miss; cache hit $0.30/M) and $15.00/M output. kimi-k2.7-code and kimi-k2.6 are $0.95/M input (cache miss) and $4.00/M output; kimi-k2.7-code-highspeed is 2× that ($1.90/$8.00). Cache-hit input is cheaper on all of them, but the ChatCLI cost tracker bills the cache-miss price for a conservative accounting. **Sunsets announced by Moonshot:** kimi-k2.5 and the moonshot-v1 series retire on 2026-08-31 (kimi-latest deprecated since Jan 2026) — the entries stay in the catalog until then so pinned configs keep resolving.
    </Tip>
  </Tab>
  <Tab title="StackSpot">
  ...
  <Tab title="Devin CLI (Cognition)">
    ...
    | Others | `glm-5.2` · `kimi-k3` / `k2.7` / `k2.6` · `deepseek-v4-pro` |
