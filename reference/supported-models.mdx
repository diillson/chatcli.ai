...
<Tabs>
  ...
  <Tab title="AWS Bedrock (Claude)">
    ...
    **OpenAI GPT-OSS (open-weights)** — modelos OpenAI hospedados no Bedrock. Usam schema OpenAI Chat Completions (auto-detectado pelo prefixo `openai.*`, ou forçado via `BEDROCK_PROVIDER=openai`).
    ...
    <Info>
    A lista dinâmica (`/switch --model`) combina `bedrock:ListFoundationModels` e `bedrock:ListInferenceProfiles` com o catálogo estático acima, filtrando pelas famílias suportadas (`anthropic` e `openai`). Use o comando para ver exatamente quais modelos sua conta AWS pode invocar na região configurada.
    </Info>
  </Tab>
