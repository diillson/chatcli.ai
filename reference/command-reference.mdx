...
| `/context attach <name>` | Attach context to current session (`--rag [K]` semantic retrieval, `--full`/`-f` whole content, `--chunk(s)`, `--priority`; large contexts auto-upgrade to RAG when embeddings are on) |
...
| `/context status` | Live inspector: what the last assembled system prompt was made of (section by section, with cached-prefix marks), the conversation history by role, token estimates from the learned chars/token ratio, projected share of the model window and where auto-compact fires |
...
| `/autocompact [60% \| 0.6 \| 150k \| default]` | Show or set, for this session, how full the window gets before auto-compact fires (percent, fraction or token count; `default` restores the mode default: 75% in chat, 60% in coder) |
