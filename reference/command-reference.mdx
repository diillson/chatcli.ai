...
<Tabs>
  <Tab title="chatcli storage">
    ...
    Stores: `costs`, `checkpoints`, `sessions`, `transcripts`, `parked`, `memory-pending`, `taskgraph`, `ccr`, `hub`; every other store (`memory`, `skills`, `plugins`, `contexts`, `agents`, `commands`, `reflexion`, `scheduler`, `tokenizers`, `mcp`, `cache`, `logs`) is inventoried and never pruned. Rules: the session TTL (`CHATCLI_SESSION_TTL`) on last write for machine sessions, transcripts, parks, cost snapshots, pending memory and checkpoints; checkpoints whose workspace is a temp directory or no longer exists; cost snapshots started in a burst (four or more in one minute, at most two requests each — a test suite, never a person) on `prune` only; task-graph runs past 30 days; the CCR archive's TTL and size cap and the hub's idle TTL, swept when applied. Named sessions, distilled memory and user content are never touched. `--apply` without `prune` is refused.
  </Tab>
