...
| `/config retention` | Lifecycle policy of every on-disk store in one view: machine sessions, transcript journals, park snapshots and cost snapshots (all on `CHATCLI_SESSION_TTL`, swept at boot), CCR archive TTL and size cap, task-graph runs, memory daily notes and fact cap, hub idle purge |
