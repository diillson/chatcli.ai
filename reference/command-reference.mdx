...
| `/moa` | Mixture-of-Agents: vários modelos propõem em paralelo e um agregador sintetiza; cada participante recebe o briefing de um turno de chat + tools read-only (knowledge, recall de CCR, memória) (`CHATCLI_MOA_MODELS`) — ver [Mixture-of-Agents](/features/mixture-of-agents) |
...
| `/config selfevolve` | Auto-evolução de skills: modo (`CHATCLI_SELFEVOLVE_MODE` `auto`/`suggest`/`off`) + observabilidade (skills criadas pelo engine vs. total). Ver [Auto-autoria de Skills](/features/skill-authoring) |
...
| `@memory` | `remember`, `recall`, `forget`, `profile`, `neighbors`, `map` (grafo de conhecimento) | [Bootstrap e Memória](/features/bootstrap-memory) |
| `@skill` | `create`, `update`, `list`, `show`, `remove`, `restore`, `stats`, `export`, `import` | [Auto-autoria de Skills](/features/skill-authoring) |
| `@knowledge` | `search`, `get`, `toc` | [Knowledge Base](/features/knowledge-base) |

> Lista parcial — o catálogo completo (32 tools nativas) é auto-gerado: rode `/plugin list` no ChatCLI, ou peça `@tools list` a um agente. Destaques comentados em [Plugins Agênticos](/features/agentic-plugins).
