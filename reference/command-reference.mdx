...
| `/model <nome>` | Atalho para `/switch --model <nome>`; sem argumento lista os modelos |
...
| `/model <nome>` | **Atalho** de `/switch --model <nome>` (sem argumento, lista os modelos) |
...
/model                       # Lista modelos disponíveis (source: API ou catalog)
/max-tokens 4096             # Mesmo que /switch --max-tokens 4096
/max-tokens                  # Mostra o limite atual de max-tokens
```

<Tip>
Os atalhos `/model` e `/max-tokens` aparecem no menu de comandos (`<TAB>` após `/`) e têm autocomplete próprio — `/model <TAB>` sugere os modelos do provedor ativo e `/max-tokens <TAB>` oferece presets (`0`, `1024`, `4096`, `8192`, `16384`, `32768`).
</Tip>
