...
| `/clear` | Clear the conversation: the turns go to a checkpoint (`/rewind` restores them) while the session binding, attachments and memory stay |
| `/redraw` | Repaint the screen (prompt corrupted or visual artifacts; also `/reset`) |
...
| `/<command>` | Invoke a [slash-command template](/features/slash-commands) from `.chatcli/commands` (project or `~/.chatcli/commands`, with interop for 9 agent-CLI ecosystems — Claude Code, Devin, Windsurf, Cursor, opencode, Codex, Gemini, Qwen, Copilot): `/review-pr 1326`. `/config commands` lists the catalog |
| `/session` | Manage sessions (`save`, `load`, `attach`, `detach`, `status`, `list`, `delete`, `new`, `fork`, `search`) |
...
| `/update` | Update ChatCLI through the same channel it was installed with (Homebrew, go install or release binary); `/update check` only checks. Also available outside the REPL as the `chatcli update` subcommand |
| `/compact` | Compact conversation history (auto or guided) |
| `/autocompact` | Session auto-compaction threshold |
...
| `/cost [reset\|last\|sessions\|export]` | Session cost summary; reset the period, view the previous/recent snapshots, export JSON |
| `/storage [store]` / `/storage prune [store] [--apply]` | Inventory of every local store (files, size, policy, what the rules would remove now); `prune` is a dry run until `--apply` |
...
| `/policy` | Session security-policy mode (`status`, `mode auto`, `mode interactive`) — see [Session Policy Mode](/features/coder-security#session-policy-mode-policy) |
...
| `/agents` | Live agent runs as a tree (`list`, `show <id>`, `cancel <id>`) — see [Agent Squad](/features/agent-squad) |
| `/board` | Squad work board (`list`, `show`, `create`, `move`, `assign`, `note`, `archive`) — see [Agent Squad](/features/agent-squad) |
| `/taskgraph` | Task graph runs (`status`, `show`, `list`, `dash`, `prune`, `cancel`) — watch a DAG execution mid-run, see [Task Graph](/features/task-graph) |
| `/dash` | Live dashboard of the whole runtime (`open`, `url`, `status`, `off`): a real-time graph of agents, LLM, tools, skills, MCP, the seven patterns, background work and connections across every chatcli process on the machine. Works mid-run and over ACP; `chatcli dash` serves it from a terminal of its own. See [Live Dashboard](/features/live-dashboard) |
| `/mail` | Squad mail (`list`, `send <agent> <text>`, `pending`) — see [Agent Squad](/features/agent-squad) |
...
`chatcli acp` — runs ChatCLI over the **Agent Client Protocol** (stdio): chat/agent/coder session modes, structured tool-call and plan updates, slash commands from the IDE prompt, native permission dialogs and `session/load` restore (live state or the autosave mirror, with the conversation replayed), for JetBrains IDEs, Zed and any ACP client. See [ACP — ChatCLI inside your IDE](/features/acp).
...
| `/config security reseal` | Rewrite every store under the state root with the current encryption key (plaintext files get sealed; retired keys are migrated) |
| `/config security verify-audit [path]` | Re-hash the tamper-evident audit trail and report the first broken line, sealed count, rotation origin and torn tail |
...
| `/config providers` | Env vars for each provider (OpenAI, Claude, Bedrock, Copilot, Devin CLI, OpenRouter, ZAI, MiniMax, Moonshot, StackSpot, Ollama) + API key presence (no value leaked) |
...
| `/config managed` | Organization-managed defaults and locked policies in effect (file path, entries, precedence) |
| `/config retention` | Lifecycle policy of every on-disk store in one view: machine sessions, transcript journals, park snapshots, cost snapshots and coder checkpoints (all on `CHATCLI_SESSION_TTL`, swept at boot; a checkpoint whose workspace is gone goes regardless of age), CCR archive TTL and size cap, task-graph runs, memory daily notes and fact cap, hub idle purge |
| `/config memory` | Memory injection mode, auto-recall gates, knowledge-graph settings and live graph node/edge stats |
...
| `/config commands` | Slash-command catalog: every loaded template with its source, names refused for shadowing built-ins, scanned directories; `reload` forces a re-scan. See [Slash Commands](/features/slash-commands) |
...
| `/config auth` | OAuth status per provider (`anthropic`, `openai-codex`, `github-copilot`) with token expiry, keychain backend |
...
| --- | --- | --- |
| `@coder` | `read`, `write`, `patch`, `multipatch`, `tree`, `search`, `exec`, `test`, `rollback`, `git-status`, `git-diff`, `git-log`, `git-changed`, `git-branch`, `outline`, `map`, `checkpoint`, `clean` | [Plugin @coder](/features/coder-plugin) |
| `@browser` | `open`, `snapshot`, `click`, `type`, `scroll`, `eval`, `screenshot`, `console`, `network`, `back`, `status`, `close` | [Browser Automation](/features/browser-automation) |
| `@forge` | `pr-list`, `pr-view`, `pr-diff`, `pr-checks`, `pr-create`, `pr-comment`, `issue-list`, `issue-view`, `issue-comment`, `ci-status`, `ci-logs`, `detect` | [Git Forge](/features/git-forge) |
| `@view` | `view` (attach a local image to the conversation via the vision pipeline) | [Image Input (Vision)](/features/vision-input) |
| `@proc` | `start` (with `pty:true`), `stdin`, `status`, `logs`, `stop`, `remove`, `list` | [Agentic Plugins](/features/agentic-plugins) |
...
| `@agents` | `list`, `show`, `cancel` | [Agent Squad](/features/agent-squad) |
| `@board` | `create`, `list`, `show`, `move`, `assign`, `note`, `link`, `archive` | [Agent Squad](/features/agent-squad) |
| `@mail` | `send`, `inbox`, `history` | [Agent Squad](/features/agent-squad) |
...
| `@commands` | `list`, `get` — the model discovers and expands slash-command templates | [Slash Commands](/features/slash-commands) |
...
| Command | Description |
| --- | --- |
| `/session save <name>` | Save the current conversation (also binds it — turns are written through from then on) |
| `/session load <name>` | Load a saved conversation (also binds it, and rotates the shared hub thread) |
| `/session attach <name>` | Bind to a named session — loads it if it exists, creates it if not; every turn syncs both ways with other surfaces (see [Cross-surface continuity](/features/session-management#cross-surface-continuity)) |
| `/session detach` | Keep the conversation, drop the binding — turns stop writing through |
| `/session status` | Show whether (and to which session) the conversation is bound |
| `/session list` | List all sessions |
| `/session delete <name>` | Delete a session |
| `/session new` | Start a clean session (unbinds and rotates the shared hub thread, like `/newsession`) |
| `/session fork <name>` | Fork current session into an independent copy |
| `/session export <md\|jsonl> [path]` | Export the full transcript journal as Markdown or JSON Lines |
| `/session transcript <search\|show\|export\|stats>` | BM25 search, replay, export or stats over the transcript journal |
...
| `/context attach <name>` | Attach context to current session (`--rag [K]` semantic retrieval, `--full`/`-f` whole content, `--chunk(s)`, `--priority`, `--weight <n>` corpus weight when merging knowledge hits; large contexts auto-upgrade to RAG when embeddings are on) |
...
| `/context refresh <name>` | Re-index from the recorded source paths; only changed files are re-read and only changed passages re-embedded; unchanged corpora leave caches untouched |
| `/context watch <name> [off]` / `watch list` / `unwatch <name>` | Filesystem watch (fsnotify) that refreshes the context automatically after edits settle (1.5 s debounce); outcome printed at the next prompt |
...
| `/context status` | Live inspector: what the last assembled system prompt was made of (section by section, with cached-prefix marks), the conversation history by role, token estimates from the learned chars/token ratio, projected share of the model window and where auto-compact fires | The totals now include the **prefix budget** (half the window, floor 24K chars): how much of it the last prompt used and which sections were folded for budget (skills → knowledge digests → attachments).
...
<Tabs>
  <Tab title="chatcli storage">
    The `/storage` command without a REPL, for cron jobs and scripts. Boots no provider.

    | Form | Description |
    | --- | --- |
    | `chatcli storage [store]` | Inventory: files, size, policy and what the rules would remove now, per store |
    | `chatcli storage prune [store]` | Dry run: what the rules would remove, with the reason per item group |
    | `chatcli storage prune [store] --apply` | Remove it |
    | `--json` | Machine-readable inventory or outcome |

    Stores: `costs`, `checkpoints`, `sessions`, `transcripts`, `parked`, `memory-pending`, `taskgraph`, `pulse`, `ccr`, `hub`; every other store (`memory`, `skills`, `plugins`, `contexts`, `agents`, `commands`, `reflexion`, `scheduler`, `tokenizers`, `mcp`, `cache`, `logs`) is inventoried and never pruned. Rules: the session TTL (`CHATCLI_SESSION_TTL`) on last write for machine sessions, transcripts, parks, cost snapshots, pending memory and checkpoints; checkpoints whose workspace is a temp directory or no longer exists; cost snapshots started in a burst (four or more in one minute, at most two requests each — a test suite, never a person) on `prune` only; task-graph runs past 30 days; live-dashboard spools (`pulse`) of processes that are dead and were last heard from more than 24 hours ago, never one that is still recording; the CCR archive's TTL and size cap and the hub's idle TTL, swept when applied. Named sessions, distilled memory and user content are never touched. `--apply` without `prune` is refused.
  </Tab>
  ...
  <Tab title="chatcli update">
    One-shot self-update for scripts and automation — the same flow as `/update`, without entering the REPL. Updates through the detected install channel (Homebrew, go install or verified binary self-replace); manual channels (Docker, source build) get instructions instead.

    | Argument | Description |
    | --- | --- |
    | *(none)* | Check and apply through the detected channel |
    | `check` | Report only — never applies |

    Exit codes: `0` updated or already on the latest version (including `check`), `1` check/apply failure or a manual channel with a pending update, `2` invalid usage. See [Auto-Update](/features/auto-update) for channel detection and the verified self-replace pipeline.
  </Tab>
</Tabs>
...
| `/memory export [path]` | Export facts, episodes, topics, projects and profile as JSONL (sealed with `CHATCLI_ENCRYPTION_KEY` when set) |
| `/memory import <path>` | Merge a memory export; nothing is deleted |
| `/memory recall <query>` | Dry-run auto-recall with the reason behind each fact |
| `/memory why` | Why the last turn recalled what it recalled |
...
| `/autocompact [60% \| 0.6 \| 150k \| default]` | Show or set, for this session, how full the window gets before auto-compact fires (percent, fraction or token count; `default` restores the mode default: 75% in chat, 60% in coder) | Precedence: this override > the catalog's per-model threshold > the mode default (0.75 chat, 0.60 agent/coder).
| `/rewind` | Display checkpoint menu for restoration |
| `/rewind compact` | Undo the latest compaction (auto, `/compact` or recovery); journal-backed after a resume |
