...
|   |       |-- retriever.go   # Smart keyword-based retrieval + graph expansion
|   |       |-- graph_cache.go # Persisted knowledge-graph cache (graph.json)
...
|   |-- handler_session.go     # Prompt and session RPCs
|   |-- llm_route.go           # Request routing: fallback chain, max_tokens, credential refresh, refusal retry, usage
|   |-- handler_remote.go      # Resource discovery RPCs
|   |-- handler_analysis.go    # AIOps RPCs
|   |-- handler_hub.go         # Conversation hub RPCs
|   |-- pipeline.go            # Pipeline RPCs (server-hosted turn engine)
|   +-- auth.go, auth_mtls.go  # Bearer/JWT and certificate identity
...
| `Model` | `string` | Active model (e.g., `"claude-sonnet-4-6"`) |
...
History exceeds token budget
  ...
  |-- Remove <plan> blocks from assistant
