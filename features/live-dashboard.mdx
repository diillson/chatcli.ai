...
<Frame caption="One terminal window per process: here the REPL running a coder with its reviewer worker, and the ACP agent inside an IDE answering over JSON-RPC on another provider. Agents form a tree by their real parent; everything they use is a hub with counters, latency and state; the tiles above sum what the page shows. (Synthetic data.)">
  <img src="/images/live-dashboard.png" alt="ChatCLI live dashboard: two process windows with a graph of agents, LLM, tools, skills, MCP servers, harness patterns, background work and connections, and the event feed" />
</Frame>
...
<Frame caption="Live: edges pulse and the feed tails as things happen; `/` searches the graph (here for the shell tool), `e` narrows the feed to errors. (Synthetic data.)">
  <img src="/images/live-dashboard.gif" alt="Animated ChatCLI live dashboard: two processes reporting, particles flowing along edges, a search for the shell tool and the feed switched to errors only" />
</Frame>
