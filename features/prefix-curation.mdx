...
<Tabs>
  ...
  <Tab title="Per-turn suffix">
    ...
    In chat the suffix travels as one flagged `[TURN CONTEXT]` user-role message right before your turn. In agent and coder mode the blocks that belong to a whole run — the query-driven retrieval and the skills block — travel as flagged history messages appended once per run (`[RUN CONTEXT]` and the skills message), never inside the system message: the system array sits ahead of every message, so even an uncached block there rewrote the entire cached conversation on each new query. As history they are append-only, the previous run's prefix stays a cache read, and they stay in front of the model for the whole tool loop.
  </Tab>
...
ChatCLI now waits for the answer, on every surface. When a session watches its own prefix expire during a gap that an hour would have covered, it asks for the hour — **once**, one-way, never over an explicit `CHATCLI_PROMPT_CACHE_TTL`, and never on a model that cannot carry it. A one-shot run can never reach that second observation, so it stays on the short lifetime. Agent and coder runs used to ask for the hour up front, on the bet that they pause between tool rounds; a two-request run paid the doubled write for nothing, so they now follow the same evidence as chat.

`/cost` shows the lifetime in effect on the cache line.

**A tool call that outlives the entry is covered by a refresh, on every provider that can.** A cache read refreshes the entry's timer for free on every prompt cache. A test suite that runs six minutes inside a coder turn would expire a 5-minute entry before the next request, and the whole prefix would be rewritten — at the write price on Anthropic and Bedrock, at full price on the subset caches of OpenAI and the others. So 4.5 minutes after each request, while a run is still going, ChatCLI re-sends the previous request asking for no output (`max_tokens: 0` on Anthropic's wire, the one token the Chat Completions and Bedrock Mantle wires accept) and pays the read price of the prefix instead, up to twelve times per stretch. The body is the same one the turn sent, thinking and effort included, because they are part of the cache key. Each refresh is booked like any request, so `/cost` shows what it cost. Adapters opt in through a small interface; the Anthropic API, OpenAI Chat Completions and Bedrock Mantle refresh today, and an adapter that cannot says so once and is not asked again.

**Where a read is cheap enough, the entry is kept warm while you are idle too.** On Claude Fable 5.1 a read is 2.5% of the input price, which beats paying 2x on every written token for the hour-long entry unless pauses run toward an hour. So on such models the interactive REPL keeps refreshing between turns as well, and the hour-long promotion stands down. On the other models a read is 10% of input or more and the arithmetic favors the hour, so they keep the promotion above. `CHATCLI_PROMPT_CACHE_KEEPALIVE` is `auto` (long tool calls everywhere, idle by read price), `on` (idle on every client that can refresh) or `off`; the gateway never refreshes, because one client serves every tenant there.

**The decision belongs to the conversation.** The lifetime "auto" settled on is part of every cache marker the request carries. Under the gateway, where one process serves many principals, it travels with each tenant's store set: a tenant's `/coder` run decides for that tenant's conversation only, and the base conversation gets its own decision back when the tenant leaves.
