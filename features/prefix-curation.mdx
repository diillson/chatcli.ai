...
**Where a read is cheap enough, the entry is kept warm instead.** A cache read refreshes the entry's timer for free, so re-sending the previous request with `max_tokens: 0` shortly before the 5-minute entry expires keeps it alive for the price of one read and no output. On Claude Fable 5.1 a read is 2.5% of the input price, which beats paying 2x on every written token for the hour-long entry unless pauses run toward an hour. So on such models the interactive REPL refreshes the entry every 4.5 minutes while you are idle, up to twelve times, and the hour-long promotion stands down. Each refresh is booked like any request, so `/cost` shows what it cost. On the other Claude models a read is 10% of input and the arithmetic favors the hour, so they keep the promotion above. `CHATCLI_PROMPT_CACHE_KEEPALIVE` is `auto` (by read price), `on` (every client that can refresh) or `off`; the gateway never refreshes, because one client serves every tenant there.
