...
On macOS/Linux you also **see what you type**: the line being composed renders live under the panel (or next to the turn spinner) as `❯ your text▌`, with backspace working — ChatCLI owns the line editing while the loop runs, so the spinner no longer eats your keystrokes. (Windows keeps the classic behavior: the line is delivered on Enter.)
