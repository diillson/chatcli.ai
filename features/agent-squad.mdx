...
<Note>For an **approved plan of 5+ genuinely independent tasks**, there is a stricter layer above the playbook: the [Task Graph](/features/task-graph) — a persisted DAG where the engine runs each task's validation commands itself and an independent reviewer issues the verdict before anything counts as done.</Note>

<Note>A dispatched worker can be granted session plugins for its task — `<agent_call agent="coder" task="..." tools="@browser,@websearch" />` — gated by the same security policy. See [Worker capabilities](/features/task-graph#worker-capabilities).</Note>
...
The terminal is not locked while a squad (or any `/coder` run) is working: type `/agents`, `/board`, `/mail`, `/jobs`, `/taskgraph` or [`/dash`](/features/live-dashboard) at any moment and the command executes right away — the live progress panel pauses, the output prints, the panel resumes.
