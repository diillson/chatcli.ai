...
<Note>
O hub é uma **ponte do momento**, não memória de longo prazo. Ele mantém a conversa **atual** sincronizada entre canais, com banco **limitado** (poda automática). Memória persistente entre projetos/sessões continua sendo o [sistema de memória](/features/bootstrap-memory) (`/memory`) e o [`/session save`](/features/session-management).
</Note>
...
| [Memória](/features/bootstrap-memory) (`/memory`) | fatos/aprendizados de longo prazo | permanente |
| [Sessões](/features/session-management) (`/session save`) | snapshot explícito de uma conversa | arquivo JSON |
...
- [Chat Gateway](/features/chat-gateway) — expõe o ChatCLI nos canais de mensagem
- [Servidor Remoto (/connect)](/features/remote-connect) — conectar o CLI a um hub remoto
- [Variáveis de Ambiente](/reference/environment-variables#conversation-hub-continuidade-cross-channel)
