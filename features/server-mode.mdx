...
| `CHATCLI_BIND_ADDRESS` | Endereço de bind do servidor. Alterado de `0.0.0.0` para `127.0.0.1` por segurança. | `127.0.0.1` |

<Warning>
O gRPC reflection agora requer **duas condições**: a flag `--grpc-reflection` **E** a variável `CHATCLI_GRPC_REFLECTION=true`. Isso evita habilitação acidental em produção. Veja a [documentação de segurança](/features/security) para todas as medidas de hardening.
</Warning>

<Note>
O endereço de bind padrão foi alterado de `0.0.0.0` para `127.0.0.1`. Para expor o servidor na rede, defina `CHATCLI_BIND_ADDRESS=0.0.0.0` explicitamente.
</Note>
