...
<Steps>
  ...
  <Step title="Com fallback de provedores (opcional)">
    ```bash
    chatcli server --fallback-providers OPENAI,CLAUDEAI,GOOGLEAI,OPENROUTER,ZAI,MINIMAX,COPILOT
    ```
  </Step>
...
<Tabs>
  ...
  <Tab title="TLS (HTTPS)">
    ...
    **mTLS (Mutual TLS):** Para autenticação mútua, configure tambem o certificado do cliente:
