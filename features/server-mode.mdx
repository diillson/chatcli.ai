...
The `GetAlerts`, `AnalyzeIssue` and `AgenticStep` RPCs are used by the [AIOps Operator](/features/k8s-operator) to feed the autonomous remediation pipeline. `AnalyzeIssue` and `AgenticStep` responses also carry the `usage`, `provider` and `model` fields described above.
