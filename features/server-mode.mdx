...
<CardGroup cols={2}>
  ...
  <Card title="Performance" icon="bolt">
    Communication via gRPC with TLS support and real provider streaming
  </Card>
</CardGroup>
...
| `--token` | Authentication token. Required unless the server binds loopback — see below | `""` | `CHATCLI_SERVER_TOKEN` |
...
Every surface — the REPL, one-shot runs, the gateway, the MCP/ACP server and this gRPC server — can push its session counters to an OpenTelemetry collector over OTLP/HTTP (JSON encoding, no SDK dependency). Configuration is the standard OTel environment contract only:
...
Per-model points carry the OpenTelemetry GenAI attribute names (`gen_ai.provider.name`, `gen_ai.request.model`); embedding tokens/cost and the daily spend/limit are exported too. `chatcli.session.cost` emits one series per session only when `OTEL_RESOURCE_ATTRIBUTES` contains `chatcli.session=attr` (unbounded cardinality is opt-in). Cumulative sums pushed every interval (and once more at shutdown), with `service.name`, `chatcli.surface` and `chatcli.tenant` on the resource: `chatcli.llm.tokens` (`kind` = input/output/cache_read/cache_write/reasoning, `provider`, `model`), `chatcli.llm.cost` (USD per provider/model), `chatcli.context.compactions` (`level` = summary/truncation) and `chatcli.context.compaction_cost`, `chatcli.cache.requests` (`outcome` = total/miss/expected_rebuild) and `chatcli.cache.storage_cost`, `chatcli.session.cost`. `/config server` shows the endpoint, the push count and the last error.
...
| `CHATCLI_SERVER_TLS_CLIENT_CA` | CA bundle client certificates are verified against (`--tls-client-ca`); enables mutual TLS and identifies callers by certificate. Requires `--tls-cert`/`--tls-key`. | `""` |
| `CHATCLI_MTLS_ROLE` | Role granted to a caller identified by its client certificate alone (no bearer token): `viewer`, `user` or `admin`. An unrecognized value resolves to read-only. | `user` |

<Warning>
gRPC reflection now requires **two conditions**: the `--grpc-reflection` flag **AND** the `CHATCLI_GRPC_REFLECTION=true` variable. This prevents accidental exposure in production. See the [security documentation](/features/security) for all hardening measures.
</Warning>

<Note>
The default bind address is `127.0.0.1` (secure for local use). In Kubernetes, the server auto-detects the environment via `KUBERNETES_SERVICE_HOST` and automatically binds to `0.0.0.0`. An explicit `CHATCLI_BIND_ADDRESS` value always takes precedence.
</Note>

<Warning>
**A reachable server needs a credential.** Authentication is optional only
on loopback, where the trust boundary is the machine. On any other bind
address — including the `0.0.0.0` Kubernetes uses — a server with no
credential would admit every caller as an administrator, so it **refuses
to start** and names what is missing.

Provide one of:

- `--token` / `CHATCLI_SERVER_TOKEN` — one shared credential;
- `CHATCLI_JWT_SECRET` (HS256) or `CHATCLI_JWT_PUBLIC_KEY` (RS256) —
  per-user JWTs carrying identity and role. Tokens must carry an `exp`
  claim; one without an expiry is rejected;
- `--tls-client-ca` / `CHATCLI_SERVER_TLS_CLIENT_CA` — mutual TLS: every
  caller presents a certificate the CA signed and is identified by it
  (see the TLS tab below);
- `CHATCLI_BIND_ADDRESS=127.0.0.1` — no credential, reachable only from
  the machine itself.

Running in Docker, the published port only reaches a server bound to the
container's interface, so `CHATCLI_BIND_ADDRESS=0.0.0.0` and a token go
together there.
</Warning>
...
<Tabs>
  <Tab title="No Authentication">
    On loopback, the server does not require authentication and any local client can connect (a reachable bind refuses to start without a credential — see the warning above):
  ...
  <Tab title="TLS (HTTPS)">
    ...
    **mTLS (Mutual TLS):** For mutual authentication, the server verifies client certificates against a CA bundle and the client presents its certificate:

    ```bash
    # Server: require a certificate signed by ca.pem (env: CHATCLI_SERVER_TLS_CLIENT_CA)
    chatcli server --tls-cert cert.pem --tls-key key.pem --tls-client-ca ca.pem

    # Client: present its certificate
    ...
    With `--tls-client-ca`, a caller that sends no bearer token is identified by its verified certificate: the principal is `mtls:<CN>` (the first URI SAN, then the first DNS SAN, when the CN is empty) and its role comes from `CHATCLI_MTLS_ROLE` (`viewer`, `user` or `admin`; default `user`; an unrecognized value resolves to read-only). A bearer token, when present, still wins, since it carries the role its issuer chose. The Helm chart exposes this as `security.mtlsRole`.
...
| `StreamAlerts` | Pushes K8s Watcher alerts as they are raised, with heartbeats (server streaming; used by the Operator) |
| `AnalyzeIssue` | Sends Issue context to the LLM and returns analysis + suggested actions |
| `AgenticStep` | Runs one turn of the AI-driven remediation loop (used by the Operator) |
| `ResolveActiveConversation` | Returns the principal's current conversation, creating one on first contact (conversation hub) |
| `NewConversation` | Rotates the principal's active conversation to a fresh thread; every channel follows |
| `AppendEvent` | Appends one dialogue turn to a conversation's shared log |
| `ReadConversation` | Returns the events after a given sequence number (hydration/resume) |
| `SubscribeConversation` | Live-tails a conversation: backlog, then events as any channel appends them (server streaming) |
| `SetBinding` | Maps a channel identity (platform, user id) to a principal |
| `ListBindings` | Lists channel-to-principal bindings |
| `ChatTurn` | Full-pipeline chat turn on the server (memory, contexts, skills, knowledge, compaction); opt-in, see [Pipeline RPCs](#pipeline-rpcs) |
| `RunCoder` | Runs the coder loop on the server with its tools (server streaming: transcript lines, then the final answer) |
| `RunAgent` | Runs the agent loop on the server (same contract) |
| `ListPipelineTools` | Lists the tools the server-hosted engine exposes |
| `RunPipelineTool` | Invokes one of those tools |
...
`StreamPrompt` streams the text as the provider produces it: every chunk is a fragment the provider emitted, not a slice of a finished reply. The final message (`done: true`) carries the `usage` the provider reported and the `stop_reason`. When the selected route cannot stream, the reply is completed in one call and delivered as a single chunk, so clients never need a second code path.

### Reply Attribution and Token Usage

`SendPrompt`, `AnalyzeIssue` and `AgenticStep` responses, and the final `StreamPrompt` message, carry a `TokenUsage` message and a `stop_reason`, and name the `provider` and `model` that actually answered (the fallback chain may answer with a different provider than the one configured):
...
message TokenUsage {
  ...
  bool estimated = 6;   // provider returned no usage; counts derive from character length
  double cost_usd = 7;  // priced by the server with the same tables /cost uses
  bool cost_known = 8;  // false when the model has no published price (cost_usd is 0)
}
```

`stop_reason` is what the provider reported (`end_turn`, `max_tokens`, `refusal`, ...). `chatcli connect` feeds this usage to its local cost tracker, so `/cost` over a remote connection prices the real tokens instead of a character estimate.

### Request Routing

Every prompt RPC (`SendPrompt`, `StreamPrompt`, `InteractiveSession`, `AnalyzeIssue`, `AgenticStep`) resolves its LLM client through the same rules:

- A caller-forwarded credential (`--llm-key`, `--use-local-auth`, StackSpot fields) or an explicit provider/model override gets a dedicated client.
- A request that names no provider or model and forwards no credential goes through the configured **fallback chain** (`CHATCLI_FALLBACK_*`) when there is one, otherwise the server's default client. The response names the chain entry that answered.
- `max_tokens` is the request value when set, else the provider's `*_MAX_TOKENS` environment override (`OPENAI_MAX_TOKENS`, `ANTHROPIC_MAX_TOKENS`, `GOOGLEAI_MAX_TOKENS`, `BEDROCK_MAX_TOKENS`, ...), else the catalog ceiling for the model.
- An expired **server** credential (provider 401/403) triggers one provider refresh and a retry. Refreshes are throttled to one per 30 seconds process-wide, so a burst of 401s under load rebuilds the providers once. Caller-forwarded credentials are never refreshed: the remote client owns them.
- A reply stopped by a safety classifier (`stop_reason: refusal`) is resent once on the sibling model of the same provider, as the agent loop does. On a stream this only happens while no text has been shown yet.

Requests are not serialized: every RPC keeps running concurrently on its own client instance.

### Pipeline RPCs

`SendPrompt` is a model proxy: when you use `chatcli connect`, the enrichment pipeline (memory, `/context` attachments, skills, knowledge retrieval, compaction) runs in **your** CLI and the server only talks to the model. Thin clients, the operator and automation that want the **server** to own the whole turn use the pipeline RPCs, the same engine the stdio MCP and ACP servers expose:

```bash
CHATCLI_SERVER_PIPELINE=true chatcli server --port 50051 --token my-token
```

| RPC | What runs on the server | Role |
|---|---|---|
| `ChatTurn` | one chat turn with the full enrichment pipeline (`plain: true` asks for the bare passthrough); optional per-turn provider/model | any authenticated caller |
...
Sessions are namespaced by the authenticated principal (`<subject>/<session>`; the default session when none is given), so two callers never share a conversation by picking the same id. `GetServerInfo` reports `pipeline_enabled`.

<Warning>
The engine hosts one ChatCLI inside the server process: its workers (memory, MCP servers, scheduler) run there and its turns are **serialized**; concurrent callers queue, bounded by their own deadline. It is exclusive with the co-located gateway (`CHATCLI_GATEWAY_IN_SERVER`), which builds its own engine; with both set the pipeline stays off and the reason is logged. Prefer a dedicated Instance (or replica set) for pipeline traffic.
</Warning>
...
The `GetAlerts`, `StreamAlerts`, `AnalyzeIssue` and `AgenticStep` RPCs are used by the [AIOps Operator](/features/k8s-operator) to feed the autonomous remediation pipeline. `AnalyzeIssue` and `AgenticStep` responses also carry the `usage`, `provider` and `model` fields described above.
...
Pushes alerts the moment the watcher raises them, so the operator no longer has to poll. The stream is server-side streaming over the same authenticated connection:
...
message StreamAlertsResponse {
  ...
  bool heartbeat = 2;       // Liveness tick every 15s while the watcher is quiet
}
```

- With `include_current` the stream opens with the alerts `GetAlerts` would return, then only new ones follow. The watcher's own dedup (same type and object within the window) means each alert is pushed once.
- Heartbeats every 15 seconds let a client tell a quiet watcher from a dead connection. Without a watcher configured the stream carries heartbeats only, matching `GetAlerts` returning an empty list.
- Every subscriber owns a bounded buffer. A subscriber that falls behind is dropped with `ABORTED`: reopen the stream with `include_current` to resync. `GetAlerts` stays available for one-shot reads and for clients that prefer polling.
...
The server implements per-client rate limiting using a token bucket to protect against abuse. The limiter runs **after** authentication and keys on the caller's subject — the JWT `sub` or the certificate principal — so tenants behind the same NAT or ingress do not share a bucket; anonymous callers are keyed by address. Unauthenticated floods are bounded separately by the per-address authentication failure limiter.
...
CHATCLI_JWT_SECRET=my-secret-key            # HS256, or:
# CHATCLI_JWT_PUBLIC_KEY=/path/to/jwt-public.pem   # RS256
...
# mTLS (server side)
CHATCLI_SERVER_TLS_CLIENT_CA=/path/to/ca.pem
CHATCLI_MTLS_ROLE=user   # role for callers identified by certificate alone

# mTLS (client side, chatcli connect)
