...
O embedado é totalmente self-contained, mas rasteriza com uma **fonte embarcada** e sem cairo/pango — fontes como `Helvetica`/`Menlo` caem num fallback e diagramas com muito texto ficam mais simples. O `dot` do sistema usa **fontconfig + as fontes do SO + cairo**, gerando texto mais nítido e layout melhor. No modo `auto`, se o render pelo `dot` do sistema falhar, há **fallback transparente** para o embedado.
