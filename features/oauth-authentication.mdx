...
<Warning>
The token exchange uses a plain `http.Client` (no custom transport) to avoid TLS fingerprinting issues with Cloudflare. The `User-Agent: claude-cli/2.1.281 (external, cli)` header is required, and the version must track a current Claude Code release: the API refuses the newest models to older versions (`claude_code_version_too_old`; Opus 5.5 requires 2.1.280+). The floor lives in one constant (`auth.ClaudeCodeVersion`) shared by login, refresh, messages and models; when the API asks for a newer release, ChatCLI reads the required release from the error, presents it for the rest of the process and resends the same request — `CHATCLI_CLAUDE_CODE_VERSION` pins a release by hand. Using an HTTP client with `LoggingTransport` or a custom transport causes rejection by Anthropic's CDN.
</Warning>
...
Token lifetime is set by the provider that issued it: the `expiry` field is
checked before each request and an expired token triggers the refresh flow
automatically. ChatCLI does not impose a ceiling of its own, so an
environment with a rotation requirement enforces it at the identity
provider, or by revoking the credential and running `/auth login` again.
