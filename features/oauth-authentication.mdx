...
<Warning>
The token exchange uses a plain `http.Client` (no custom transport) to avoid TLS fingerprinting issues with Cloudflare. The `User-Agent: claude-cli/2.1.259 (external, cli)` header is required, and the version must track a current Claude Code release: the API refuses the newest models to older versions (`claude_code_version_too_old`). It lives in one constant (`auth.ClaudeCodeVersion`) shared by login, refresh, messages and models. Using an HTTP client with `LoggingTransport` or a custom transport causes rejection by Anthropic's CDN.
</Warning>
