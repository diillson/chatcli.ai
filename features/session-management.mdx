...
<Info>
O ChatCLI usa um **historico unificado** — um único array de mensagens compartilhado entre todos os modos (chat, agent, coder). Ao salvar uma sessao, todo o contexto e preservado independente do modo em que foi gerado. Use `/compact` para reduzir o tamanho e `/rewind` para voltar a pontos anteriores.
</Info>
...
/session save minha-sessao-segura
/session load minha-sessao-segura
```
