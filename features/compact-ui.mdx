...
<Tip>
**`CHATCLI_CODER_UI` e [`CHATCLI_TURNUI`](/features/split-pane-ui) sao ortogonais.** `CHATCLI_CODER_UI` controla o **estilo dos cards/inline** do agente; `CHATCLI_TURNUI` controla a **arquitetura do terminal** (status fixo no topo, input fixo embaixo, scroll region). Combine `CHATCLI_CODER_UI=compact CHATCLI_TURNUI=on` para a experiência mais minimalista possível, similar ao Claude Code.
</Tip>
