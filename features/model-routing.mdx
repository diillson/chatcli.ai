...
| `frontier` | Architectural reasoning, hard debugging | Fable 5.1, Opus 5, GPT-5.6 Sol, o3 |
| `unmetered` | Local backends or unknown pricing | Ollama models, Devin before the account listing has run (afterwards Devin models tier by the rate the CLI lists) |
...
2. It prints **what the classifier flagged** — the `stop_details` category (`cyber`, `bio`, `reasoning_extraction`, …) and the explanation when Anthropic gives one — and resends that turn on the model the provider **recommends** for that category when the refusal names one (`stop_details.recommended_model`); otherwise on a **sibling model of the same provider**, as a one-turn route override: Fable → Opus 5, Opus (5.5 included) → Sonnet 5, Sonnet → Haiku 4.5. The next turn is back on your model. A `reasoning_extraction` category means the prompt asks for the internal reasoning in the text — remove the instruction rather than switch models.
...
The switch is visible where every route change is: the session card and the feed of the [live dashboard](/features/live-dashboard) show `via refusal fallback`, and cost is attributed to the model that actually served the turn.

| `CHATCLI_AGENT_REFUSAL_FALLBACK` | What happens |
|---|---|
| `auto` (default) | the model the provider recommended in the refusal, when the catalog has it; else the sibling above |
| `PROVIDER:model` | that model, as given |
| `off` | a plain resend on the same model, at most twice per run; a third refusal ends the run with the cause |
...
| `CHATCLI_AGENT_REFUSAL_FALLBACK` | Where a turn the safety classifier refused is resent: `auto` (sibling model of the same provider), `PROVIDER:model`, or `off`. See above. | `auto` |
