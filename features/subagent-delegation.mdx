...
The output is truncated by the same [tool result budget](/features/tool-result-management) that applies to every other tool — so even a chatty subagent has a bounded effect on the parent's context. Truncation is **recoverable**: content past the inline limit is persisted to a `result_*.txt` overflow file (referenced inline) and reversibly compressed output carries `<<ccr:KEY>>` markers the parent — or the subagent itself — can expand with the `recall_output` tool.
...
| Max inline output returned to parent | ~30 KB (full text persisted to an overflow file, reference kept inline) | `MaxWorkerOutputBytes` (internal const) |
