...
<Tabs>
  <Tab title="Automatic">
    ...
    1. **Level 1 — Trimming** (near-lossless): strips `<plan>` tags, compacts XML, removes duplicates
...
### `/rewind compact` — undo a compaction

Every rewrite of the history (auto-compact in chat, agent/coder and one-shot, `/compact`, overflow recovery) keeps the history it replaced. `/rewind compact` puts it back:
...
```text
↩ Compaction undone: 9 → 41 messages restored from memory. /rewind brings the compacted view back.
```

- The compacted view is checkpointed first, so `/rewind` reverses the undo.
- After a resume (`/session load`), the snapshot is rebuilt from the [transcript journal](/features/session-management#transcript-journal): `rewrite` events record the ordered hashes of the history they replaced, and the journal holds every message ever seen. A pruned journal (past `CHATCLI_SESSION_TTL`) means nothing to undo.
- The restored history is journaled again as a rewrite, and the prompt-cache telemetry counts one expected rebuild.

### Limits

- A maximum of **20 checkpoints** are kept (oldest ones are discarded)
- Checkpoints persist with a saved session as message-hash lists and are rebuilt from the transcript journal on load (dropped when the journal no longer has their messages)
