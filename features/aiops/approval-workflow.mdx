...
| `autoApproveConditions` | AutoApproveConditions | Para auto | Condicoes para auto-aprovação |
...
<Tabs>
  <Tab title="auto">
    **Auto-approve**: O sistema aprova automaticamente se **todas** as condicoes de `autoApproveConditions` forem atendidas. Caso contrario, escala para `manual`.
    ...
    |----------|------|-----------|
    | `minConfidence` | float64 | Confianca minima da análise de IA (0.0-1.0) |
    | `maxSeverity` | string | Severidade máxima para auto-approve |
    | `historicalSuccessRate` | float64 | Taxa minima de sucesso histórico para este tipo de ação |
  ...
  <Tab title="manual">
    **Manual**: Requer aprovação explicita de pelo menos `requiredApprovers` humanos. O RemediationPlan fica pausado até a aprovação ou timeout.
  ...
  <Tab title="quorum">
    **Quorum**: Requer aprovação de `requiredApprovers` pessoas. Garante que a aprovação não depende de um único individuo.
...
| `blackoutDates` | []BlackoutDate | Não | Datas especificas com congelamento total |
...
| Campo | Tipo | Descrição |
|-------|------|-----------|
| `aiConfidence` | float64 | Nível de confianca da análise da IA (0.0-1.0) |
...
| `lastSimilarResolution` | string | Descrição da ultima resolucao similar |
...
sequenceDiagram
    ...
    alt Nenhuma regra faz match
        APC-->>RR: Sem aprovação necessaria
...
<CardGroup cols={2}>
  <Card title="Notificações e Escalacao" icon="bell" href="/features/aiops/notifications">
