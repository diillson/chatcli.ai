...
Quando o Issue pai está em estado `Contained`, o PostMortem é gerado com:

- `spec.requiresHumanAction: true`
- `spec.requiredAction`: descrição concreta do que precisa ser feito (ex: "restore deployment replicas after fixing root cause")
- O `PostMortemReconciler` **se recusa** a transitar para `Closed` enquanto a anotação `aiops.chatcli.io/human-action-acknowledged` não for `true`
...
Veja [Chaos Engineering](/features/aiops/chaos-engineering) para detalhes da CR e do controller.
