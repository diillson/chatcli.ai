...
- `remediation_failed` — when a plan ends Failed or RolledBack (recorded once, on the transition)
- `approval_requested` — when a plan is parked for a human, by an ApprovalPolicy, the cluster tier or the decision engine
- `approval_approved`, `approval_rejected`, `approval_expired` — when the parked plan leaves WaitingApproval, once per decision
- `notification_sent` — every delivery attempt of a NotificationPolicy or escalation channel, success or failure with the error
- `slo_violation` — when a burn-rate window fires on a ServiceLevelObjective
- `sla_breach` — when an IncidentSLA records a response or resolution violation
...
Every AuditEvent is created with the annotation `platform.chatcli.io/immutable: "true"`. The operator ships no admission webhook, so the annotation is a marker: enforce it with a policy engine rule (Kyverno, Gatekeeper) that rejects updates and deletes on resources carrying it.
...
The four ClusterRoles (`chatcli-role-viewer`, `chatcli-role-operator`, `chatcli-role-admin`, `chatcli-role-superadmin`) are **provisioned by the Helm chart or the kustomize overlay**, never by the operator at runtime (H5 hardening: the operator holds no permission to create ClusterRoles). They grant graded access to the platform CRDs: viewers read, operators also update Issues, ApprovalRequests and PostMortems, admins manage every platform resource.

Grant a role by binding it to a user or group the way you bind any ClusterRole:
...
Revoking is deleting the binding. Role changes are visible in the Kubernetes audit log; the operator's own AuditEvents cover what the controllers do, not who was granted what.

<Note>
  The REST API and the dashboard use their own role model, API keys with `viewer`, `operator` or `admin`, described in [Web Dashboard](/features/aiops/web-dashboard). The ClusterRoles above are for people and tooling that reach the CRDs through `kubectl`.
</Note>
