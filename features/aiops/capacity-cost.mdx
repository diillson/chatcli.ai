...
description: "Capacity forecasting with linear regression, noise reduction, and LLM cost tracking per incident."
icon: "chart-line"
---

The ChatCLI AIOps platform includes three advanced subsystems that work together to optimize operations: **Capacity Planner** (resource exhaustion forecasting), **Noise Reducer** (intelligent alert suppression), and **Cost Tracker** (LLM spend per incident, served over the REST API).
...
Every analysis (`AnalyzeIssue`) and every agentic remediation step (`AgenticStep`) is booked from the **token usage the server reports** on the reply, attributed to the provider and model that actually answered (the server's fallback chain may route elsewhere). Analyses used to be estimated as characters divided by four and agentic steps were not booked at all; only a server without usage fields still triggers the character estimate.

The price per token is resolved in this order:

1. an explicit entry for the provider in the `chatcli-cost-config` ConfigMap (the cluster operator's word);
2. the shared pricing engine (`llm/pricing`), the same per-model tables, overrides and subscription rules the CLI's `/cost` uses, so a ledger here and a session there agree on the price of the same call;
3. a per-provider default for a model the engine does not know.

The server also reports `cost_usd` / `cost_known` on the reply itself (see [server mode](/features/server-mode#reply-attribution-and-token-usage)).

### Cost Configuration

Prices are overridable per provider via the `chatcli-cost-config` ConfigMap in the Instance's namespace, key `pricing`, one entry per provider name as it appears on the Instance (`CLAUDEAI`, `OPENAI`, ...):
...
<Note>
  Without the ConfigMap, or for a provider it does not list, the pricing engine's per-model rates apply. The ConfigMap is read on every booking, so an update is picked up on the next call.
</Note>
...
| `llmCosts.estimatedCostUSD` | `float64` | Spend priced by the pricing engine or the ConfigMap override |
| `llmCosts.provider` / `llmCosts.model` | `string` | Provider and model that answered |
...
Aggregation over a window, served by the REST API:

| Field | Type | Description |
|-------|------|-------------|
| `periodStart` / `periodEnd` | `time` | Window boundaries |
| `totalLLMCost` | `float64` | Sum of every incident's `estimatedCostUSD` |
| `incidentCount` | `int` | Incidents with a ledger entry |
...
| `chatcli-cost-ledger` | Cost records per incident (IncidentCost in JSON), one ConfigMap per namespace. | Indefinite (never pruned by the operator) |
| `chatcli-cost-config` | Per-provider pricing override (`pricing` key). | Indefinite (user-managed) |

<Warning>
  ConfigMaps have a 1MB limit in Kubernetes, roughly 3,000 ledger entries. The operator does not compact the ledger: on a namespace with that volume, prune old keys periodically or the bookings start failing with a size error.
</Warning>
...
<CardGroup cols={2}>
  <Card title="REST API" icon="code" href="/reference/api-reference">
    `/api/v1/analytics/cost` serves the ledger summary; `/api/v1/analytics/capacity` and `/api/v1/analytics/summary` expose capacity and incident data.
  </Card>
  <Card title="Web Dashboard" icon="chart-mixed" href="/features/aiops/web-dashboard">
    The Overview view displays capacity warnings and remediation effectiveness in real time.
  </Card>
  <Card title="Grafana" icon="chart-area" href="/features/aiops/web-dashboard#grafana-dashboards">
    The `remediation-stats.json` dashboard covers remediation success by strategy.
  </Card>
