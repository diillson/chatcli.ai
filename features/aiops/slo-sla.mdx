...
| `description` | string | Não | Descrição legivel do SLO |
| `indicator` | SLOIndicator | **Sim** | Definicao do indicador de nível de servico (SLI) |
| `target` | SLOTarget | **Sim** | Meta e janela temporal |
| `burnRateAlerts` | []BurnRateWindow | Não | Configuração de alertas multi-window |
| `alertPolicy` | SLOAlertPolicy | Não | Politica geral de alertas |
...
Quando configurados, o sistema envia notificações ao atingir cada threshold:
...
| `businessHours` | BusinessHoursSpec | Não | Configuração de horario comercial |
| `violationPolicy` | ViolationPolicySpec | Não | Ações em caso de violacao |
...
| `maxResolutionTime` | duration | **Sim** | Tempo máximo para resolucao completa |
