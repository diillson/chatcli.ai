...
<CardGroup cols={3}>
  <Card title="Notifications" icon="bell" href="/features/aiops/notifications">
    NotificationPolicy & EscalationPolicy
  </Card>
  <Card title="SLO & SLA" icon="bullseye" href="/features/aiops/slo-sla">
    ServiceLevelObjective & IncidentSLA
  </Card>
  <Card title="Approvals" icon="check-double" href="/features/aiops/approval-workflow">
    ApprovalPolicy & ApprovalRequest
  </Card>
  <Card title="Multi-Cluster" icon="globe" href="/features/aiops/federation">
    ClusterRegistration & Federation
  </Card>
  <Card title="Audit" icon="scroll" href="/features/aiops/audit-compliance">
...
sequenceDiagram
    ...
    Note over WB,Server: Server without StreamAlerts: GetAlerts polled every 30s
...
| `poll()` | Queries GetAlerts once (fallback for a server without StreamAlerts, or `CHATCLI_OPERATOR_ALERT_TRANSPORT=poll`) |
...
<AccordionGroup>
  <Accordion title="handleDetected()">
    1. Sets `detectedAt` and `maxRemediationAttempts` (default: 5, configurable via Instance `aiops.maxRemediationAttempts`)
...
| Advanced | `ExecDiagnostic` | Runs a command from a read-only allowlist inside a pod | `command` (see [allowlist](/features/k8s-operator#execdiagnostic-allowlist)), `pod`, `container` |
...
The server pushes K8s Watcher alerts over `StreamAlerts` (server streaming with heartbeats; see [Server Mode](/features/server-mode#streamalerts)) and still exposes them for one-shot reads via gRPC:
