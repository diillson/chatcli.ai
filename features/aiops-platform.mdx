...
sequenceDiagram
    ...
    alt Modo standard (Runbook)
        RR->>K8s: Executa ações (19 tipos: Scale/Restart/Rollback/HelmRollback/ArgoSync/AdjustHPA/...)
    else Modo agêntico
...
**Ações Suportadas (19 tipos):**

| Categoria | Tipo | O que Faz | Parâmetros Chave |
|-----------|------|-----------|-----------------|
| Workload | `ScaleDeployment` | Ajusta réplicas | `replicas` |
| Workload | `RestartDeployment` | Rollout restart | — |
| Workload | `RollbackDeployment` | Rollback para revisão anterior/saudável/específica | `toRevision` |
| Workload | `PatchConfig` | Atualiza ConfigMap | `configmap`, `key=value` |
| Workload | `AdjustResources` | Ajusta CPU/memória | `memory_limit`, `cpu_limit`, etc. |
| Workload | `DeletePod` | Remove pod mais doente | `pod` (auto-seleciona) |
| Workload | `RestartStatefulSetPod` | Restart StatefulSet pod | `pod` (ou rolling restart) |
| GitOps | `HelmRollback` | Rollback de Helm release | `revision` |
| GitOps | `ArgoSyncApp` | Trigger sync ArgoCD | `revision` |
| Autoscaling | `AdjustHPA` | Modifica HPA min/max/target | `minReplicas`, `maxReplicas` |
| Infra | `CordonNode` | Marca node unschedulable | `node` |
| Infra | `DrainNode` | Evict pods do node | `node` |
| Storage | `ResizePVC` | Expande PVC | `pvc`, `size` |
| Security | `RotateSecret` | Atualiza Secret | `secret`, `key=value` |
| Networking | `UpdateIngress` | Modifica Ingress | `ingress`, `backendService` |
| Networking | `PatchNetworkPolicy` | Abre porta em NetworkPolicy | `networkPolicy`, `allowPort` |
| Advanced | `ApplyManifest` | Aplica manifesto de ConfigMap | `configmap`, `key` |
| Advanced | `ExecDiagnostic` | Comando diagnóstico whitelisted | `command` |
| — | `Custom` | **Bloqueado** — requer aprovação | — |

<Warning>
**Safety Checks (pré-execução):** Scale to 0 replicas is blocked. AdjustResources limit cannot be less than request. DeletePod refuses if only 1 pod exists. Custom actions are blocked. Blast radius prediction checks PDB violations, resource quotas, and affected services before execution.

**Rollback Automático (pós-falha):** Antes de qualquer ação, um `ResourceSnapshot` estruturado captura o estado completo do recurso (replicas, images, CPU/memory, HPA). Se uma ação falha ou a verificação de saúde expira (90s), o `RollbackEngine` restaura automaticamente o recurso ao estado pré-remediação. O plano transiciona para `RolledBack` e `PostFailureHealthy` confirma se o recurso está saudável. Funciona para Deployments, StatefulSets, DaemonSets, Nodes e HPAs.
</Warning>
...
O operator possui 110 testes (146 com subtests) cobrindo todos os componentes:
