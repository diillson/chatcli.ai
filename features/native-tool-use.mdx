...
O ChatCLI suporta **chamadas de ferramentas via API nativa** para OpenAI, Anthropic, ZAI (Zhipu AI) e MiniMax, substituindo a abordagem de XML embutido no prompt por chamadas estruturadas via API. Isso melhora a precisao, reduz o consumo de tokens e habilita otimizações de cache.
...
| Compatibilidade | Qualquer provedor | OpenAI, Anthropic, ZAI e MiniMax (outros continuam com XML) |
...
| **MiniMax** | Sim | Sim | Suporte completo a `/coder` e multi-agente |
