...
Truncated results are saved as temporary files **inside the [Session Workspace](/features/session-workspace)**, instead of the legacy global `/tmp/chatcli-tool-results/`:

```text
$TMPDIR/chatcli-agent-<random>/tool-results/
  budget_tc_1_3f9a1c02d7e4b5a6.txt    # Full result from tool call 1
  budget_tc_2_8b04de11c6f2a9d3.txt    # Full result from tool call 2
...
With the [compression layer](/features/context-compression) active, both levels are **lossless**: the original result is archived in the CCR store before the cut and the preview/summary carries a `<<ccr:KEY>>` marker (preserved across levels) that the model expands with `@recall`.
...
| `CHATCLI_HISTORY_REWRITE_PRESSURE` | Share of the compaction budget the history must reach before the age-based rewrites run against a warm prefix cache (`0` = every turn, as before) | 0.75 |
...
Every rewrite of an old message invalidates the provider's cached prefix from that message onward: on Anthropic and Bedrock the whole tail is billed again as a cache write (1.25x or 2x the input price), on OpenAI, Gemini, xAI, Kimi and the other providers with a prompt cache it is billed at full price instead of the cached discount. Shaving a few hundred tokens off a result that is already served from cache does not pay for that.

So microcompaction, repeated-read dedup and skill aging are scheduled against the cache, on every provider:

- While the prefix cache is **warm** and the history is under `CHATCLI_HISTORY_REWRITE_PRESSURE` of the compaction budget, the passes wait. The old results keep riding as cheap cache reads.
- Once the history reaches that share, the passes run: shrinking it now is what keeps the far more expensive summarizing compaction away.
- With no warm prefix to protect (a provider that reports no cache, a cache that expired, the first request) the passes run every turn as they always did.

The same gate applies to squad, taskgraph and delegate workers. Set `CHATCLI_HISTORY_REWRITE_PRESSURE=0` to restore the age-only behavior.
...
| `CHATCLI_HISTORY_REWRITE_PRESSURE` | Share of the compaction budget before the rewrites run against a warm cache (`0` = every turn) | 0.75 |
...
<CardGroup cols={2}>
  ...
  <Card title="Context Recovery" icon="life-ring" href="/features/context-recovery">
