...
**Generalization.** A run is about an entity — one team, one service, one repository — and a skill learned from it must be about the competence. The self-evolve prompt asks for `team-match-info`, not `flamengo-match-info`, with the entity as an input in the body and generic words in the triggers. And before a new skill is written, its keywords (name, description, triggers) are compared with every installed skill's; a candidate that shares at least half of its keywords with an existing skill (three words or more) is folded into that skill as an evolution instead of authored beside it. The notice says so: `folded into an existing skill: corinthians-match-info → team-match-info`.
