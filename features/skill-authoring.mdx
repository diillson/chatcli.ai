...
<Info>
**`@memory` vs `@skill`**: `@memory` guarda **fatos** ("o user prefere tabs"); `@skill` guarda **procedimento/conhecimento reutilizável com triggers** ("como deployar este projeto"). Fato curto → memória; "como fazer X" → skill. E para criar skills **a partir de documentação**, anexe o corpus como [knowledge base](/features/knowledge-base): o agente pesquisa com `@knowledge` e escreve a skill com `@skill`.
</Info>
