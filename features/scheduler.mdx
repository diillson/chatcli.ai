...
`/config security` só **lê** os valores; ele não edita rules. Os caminhos práticos hoje:

1. **Pelo prompt interativo do `/coder`** — quando você roda algo no modo coder e o prompt pergunta "Allow once / Allow always / Deny forever", escolher **Allow always** cria uma rule `allow` persistida em `~/.chatcli/coder_policy.json`. **Deny forever** cria uma `deny`. É o caminho mais idiomático: você roda a ação uma vez interativamente, aprova, e daí em diante o scheduler reconhece.

2. **Editar o JSON direto** — seja global (`~/.chatcli/coder_policy.json`) ou local por-projeto (`<root>/coder_policy.json`):
   ```json
   {
     "rules": [
   ...
   ```
   A pattern usa prefix match sobre o comando completo como o PolicyManager vê: `@coder exec <cmd>`. Deny tem prioridade sobre allow. Reinicie a sessão ou espere o próximo fire — o scheduler reloada o arquivo a cada `RunShell`.

<Info>
Um comando `/coder policy {list,add,remove}` para editar rules sem abrir JSON ainda não existe. No escopo desta feature, a rotina prevista é: se você quer agendar um `my-tool --backup` fora da allowlist, ou (a) você roda `/coder` uma vez interativamente pra aprovar, ou (b) usa `--i-know` no `/schedule` como pré-aprovação persistida no próprio job, ou (c) edita o JSON.
</Info>
