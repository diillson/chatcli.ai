...
<Note>
The gateway consults this memory too: the daemon's persona calls `@memory recall` before answering "I don't know" to personal questions. See [Chat Gateway](/features/chat-gateway).
</Note>
...
| `/memory import <path>` | Merge an export into this machine's memory: facts keep their ids (known ones keep the stronger metadata), episodes dedup, topics/projects/profile merge field by field; nothing is deleted |
...
Automatic detection doesn't always catch everything, so you can **edit memory explicitly**. Beyond name/role/expertise/company/location, the profile covers **certifications, skills, goals, interests, directives, stances, milestones and environment (env_*)**.
...
<CardGroup cols={2}>
  ...
  <Card title="Sessions" icon="floppy-disk" href="/features/session-management">
...
The embedded memory stays the default. An organization that already runs a memory service plugs it in through MCP with no ChatCLI code: `CHATCLI_MEMORY_PROVIDER=mcp:<server>` where `<server>` is a configured MCP server exposing two tools:
