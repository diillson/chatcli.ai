...
Para desenvolvedores de plugins e extensoes:

```go
ms := workspace.NewMemoryStore(dir, logger)

// Memoria de longo prazo
ms.WriteLongTerm("# Key Facts\n- Go project")   // sobrescreve tudo
ms.AppendLongTerm("\n- Uses gRPC for remote access") // adiciona ao final
content := ms.ReadLongTerm()                     // le conteudo atual

// Notas diarias
ms.WriteDailyNote("Implementei feature X")       // appenda com timestamp HH:MM
path := ms.TodayNotePath()                       // caminho do arquivo de hoje
notes := ms.GetRecentDailyNotes(3)               // ultimas 3 notas

// Contexto completo para system prompt
ctx := ms.GetMemoryContext()
// Retorna secao formatada com "Long-term Memory" + "Recent Daily Notes"
```
...
O `ContextBuilder` combina bootstrap + memoria em um unico prefixo de system prompt, injetado automaticamente em toda interacao:

```go
cb := workspace.NewContextBuilder(bootstrapLoader, memoryStore)

// Conteudo estatico (bootstrap files) — injetado como prefixo do system prompt
systemPrefix := cb.BuildSystemPromptPrefix()

// Conteudo dinamico (memoria, notas diarias) — injetado apos o system prompt principal
dynamicCtx := cb.BuildDynamicContext()
```

### Injecao Automatica

O contexto do workspace e injetado em dois pontos:

1. **Chat mode** (`processLLMRequest`): antes de enviar ao LLM, o `BuildSystemPromptPrefix()` e prepended ao system prompt e `BuildDynamicContext()` e appended
2. **Agent/Coder mode** (`agent_mode.go`): o mesmo contexto e injetado na instrucao de sistema do ReAct loop

O resultado e cacheado e invalidado automaticamente quando qualquer arquivo source muda (verificacao por mtime).
