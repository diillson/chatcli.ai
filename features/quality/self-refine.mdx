...
<Tip>To enable embedding: `CHATCLI_QUALITY_REFINE_CONVERGENCE_EMBEDDING=1` + any embeddings provider (`CHATCLI_EMBED_PROVIDER=voyage` / `openai` / `bedrock`). The cascade reuses the same provider as HyDE — zero duplicate infra. See [RAG + HyDE](/features/quality/rag-hyde) for per-provider details.</Tip>
