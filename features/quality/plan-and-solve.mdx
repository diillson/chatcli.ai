...
The complexity heuristic is still the deterministic trigger — but what it triggers changed. By default (`CHATCLI_QUALITY_PLAN_FIRST_STRATEGY=taskgraph`) a firing auto/always trigger **steers the orchestrator to the verified [Task Graph](/features/task-graph)** instead of running the legacy in-loop runner:
...
| Prompt | Routes to |
|---|---|
| Complex task, no explicit tool named (`ComplexityScore ≥ 6`) | **Task graph** (steered) |
| Explicit `@taskgraph` / matches the task-graph skill triggers | **Task graph** (plan-first defers) |
| `/plan <task>` or `/plan preview <task>` | **This planner** (preview / legacy runner) |
| `CHATCLI_QUALITY_PLAN_FIRST_STRATEGY=plan-solve` | **This legacy runner** (opt-out) |
| Simple task (score < threshold) | Direct tool calls |
...
| `CHATCLI_QUALITY_PLAN_FIRST_STRATEGY` | `taskgraph` | `taskgraph\|plan-solve` | What an auto/always trigger routes to: the verified task graph, or this legacy in-loop runner |
...
<CardGroup cols={2}>
  ...
  <Card title="PlannerAgent (pre-PR)" icon="person-walking-luggage" href="/features/customizable-agents">
