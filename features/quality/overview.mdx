...
<Tip>Self-Refine, CoVe and Reflexion work silently. To see each pattern fire **and what it concluded** (`rewrote draft`, `found discrepancy`, `lesson saved`, …), open the [Live Dashboard](/features/live-dashboard#the-seven-patterns-with-outcomes) with `/dash`.</Tip>
...
<Tip>`CHATCLI_EMBED_PROVIDER` accepts `voyage`, `openai`, `bedrock`, or `null`. For Bedrock, ChatCLI reuses the same AWS chain as the chat (IAM, profile, SSO) — no extra API key. See [RAG + HyDE](/features/quality/rag-hyde) for per-provider details.</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Configure HyDE with vectors" icon="rocket" href="/features/quality/rag-hyde#phase-3b-vector-embeddings">
    Enable embeddings (Voyage, OpenAI, or Bedrock Titan/Cohere) for semantic retrieval.
  </Card>
  <Card title="Slash reference" icon="terminal" href="/features/quality/configuration#slash-commands">
    `/thinking`, `/plan`, `/refine`, `/verify`, `/reflect`.
  </Card>
  <Card title="Full env var list" icon="list" href="/features/quality/configuration#env-vars-full-reference">
