...
description: "Duas camadas de retrieval sobre memory.Fact: Phase 3a gera uma hipótese LLM e extrai keywords para expandir a query; Phase 3b embedding vetores (Voyage/OpenAI) + busca cosseno pure-Go. Lazy backfill, JSON persistido, zero CGO."
icon: "database"
---
...
| `CHATCLI_EMBED_PROVIDER` | — | `voyage\|openai\|null` — **única fonte de verdade** para selecionar o provider |
| `CHATCLI_EMBED_MODEL` | provider default | Ex: `voyage-3`, `text-embedding-3-small`, `text-embedding-3-large` |
| `CHATCLI_EMBED_DIMENSIONS` | nativa do modelo | Só para OpenAI; sete apenas para truncar via Matryoshka |
...
<Warning>**Privacy**: a query do usuário é enviada ao provider de embedding. Para workloads sensíveis, considere self-host de um embedding model (futuro: roadmap de provider Ollama-embedding).</Warning>
