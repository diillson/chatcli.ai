...
Details: [#7 Reasoning Backbone](/features/quality/reasoning-backbone#thinking-the-slash).
...
Details: [#2 Plan-and-Solve](/features/quality/plan-and-solve#plan-manual-invocation).
...
Details: [#5 Self-Refine](/features/quality/self-refine#refine-session-toggle).
...
Details: [#6 CoVe](/features/quality/cove#verify-session-toggle).
...
Full details: [#3 Reflexion → Durable Queue](/features/quality/reflexion#durable-queue-wal-worker-pool-dlq).
...
| Env var | Default | Values | Effect |
|---|---|---|---|
| `CHATCLI_EMBED_PROVIDER` | `null` | `voyage` / `openai` / `google` / `bedrock` / `null` | Embedding backend picker |
| `CHATCLI_EMBED_MODEL` | provider default | string | Voyage: `voyage-4` (also `voyage-4-large`/`voyage-4-lite`/`voyage-code-4`). OpenAI: `text-embedding-3-small` / `-large`. Google: `gemini-embedding-2`. Bedrock: `amazon.titan-embed-text-v2:0` (default), `amazon.titan-embed-text-v1`, `cohere.embed-english-v3`, `cohere.embed-multilingual-v3`, `cohere.embed-v4:0`, `amazon.nova-2-multimodal-embeddings-v1:0`. |
| `CHATCLI_EMBED_DIMENSIONS` | model native | int | OpenAI: truncate via Matryoshka (`text-embedding-3-small`=1536, `-large`=3072 native). Bedrock Titan v2: accepts 256 / 512 / 1024; Nova MME: 256 / 384 / 1024 / 3072 (others rejected). Bedrock Titan v1 / Cohere v3: fixed dim, ignored. Cohere v4: 1536 default. |
...
| `BEDROCK_REGION` / `AWS_REGION` / `AWS_PROFILE` / AWS credentials | per chat Bedrock | — | Required for `provider=bedrock` — reuses the same chain as the [AWS Bedrock](/features/bedrock) feature (IAM role, SSO, assume-role, profile). |

<Tip>
**Bedrock embeddings** support Titan (single text per call — parallelized internally with an 8-worker pool) and Cohere v3 (native batch). Family dispatch is automatic from the model id prefix. See [RAG + HyDE](/features/quality/rag-hyde) for the full architecture.
</Tip>
...
<CardGroup cols={2}>
  <Card title="Seven patterns overview" icon="sparkles" href="/features/quality/overview">
    Back to the hub.
  </Card>
  <Card title="Environment Variables (full)" icon="key" href="/reference/environment-variables">
    The whole ChatCLI env var matrix.
  </Card>
  <Card title="Command Reference" icon="terminal" href="/reference/command-reference">
    Every slash.
  </Card>
  <Card title="/config (hierarchical)" icon="gears" href="/features/config-migration">
