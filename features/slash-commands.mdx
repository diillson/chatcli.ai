...
3. On unattended surfaces (gateway, MCP, ACP, scheduler — and piped one-shot runs, where stdin cannot answer a prompt) an "ask" resolves through `/policy automode` — or **fails safe to deny**. A denied line is replaced by an explicit marker so the model knows the output is missing, never silently empty. An explicit **allow** rule needs no automode: it runs headless (see the pipelines section below).
...
| One-shot | `chatcli -p "/review-pr 1326"` expands before routing; piped stdin becomes part of `$ARGUMENTS`; coder-mode commands run through the coder engine |
...
## Pipelines & headless automation

Slash commands compose with shell pipes: piped stdin is appended to the prompt before resolution, so a **bare invocation** works and the whole pipe body lands in `$ARGUMENTS`:

```bash
git diff | chatcli -p "/review"                # pipe body → $ARGUMENTS
kubectl logs deploy/api | chatcli -p "/triage" # works with any command
```

If the command is `mode: coder`, the auto-route fires here too — the model doesn't just read the pipe, it can **act on it** with the full coder engine (edit files, run tests) before the pipeline moves on.

### Pre-execution without prompts

A piped run has no terminal to answer an approval prompt, so `!` lines resolve through your [security policy](/features/coder-security) alone. To automate a command headless — cron, CI, scripts — add an explicit **allow** rule for exactly what it runs to `~/.chatcli/coder_policy.json`:
...
With that rule, the standup example below runs end-to-end in a pipeline with zero interaction:

```bash
chatcli --no-anim -p "/standup 3" >> standup-notes.md
```

The gate's guarantees don't bend for automation: **deny always beats allow**, safety-immune commands (`rm -rf`, `sudo`, …) still require an interactive human, and anything not matched by a rule fails safe to deny. Scope patterns tightly — `exec git log` rather than `exec git` — so an allow rule covers only what the command actually needs. `/policy automode on` is the blunter alternative (auto-approves every non-immune "ask" for the session); prefer narrow allow rules for unattended pipelines.
