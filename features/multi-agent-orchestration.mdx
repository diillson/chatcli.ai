...
<Info>Multi-agent dispatch is the engine; the [Agent Squad](/features/agent-squad) layer builds a full team on top of it: live run observability (`/agents`), a shared work board (`/board`), agent-to-agent mail (`/mail`) and an autonomous delivery playbook.</Info>
...
Workers are not blind to the session's context surfaces — they navigate truncated and compressed content the same way the orchestrator does:

- **Recall over compression**: large tool outputs inside a worker's loop are reversibly compressed (CCR). Every worker carries a universal `recall_output` tool, so any `<<ccr:KEY>>` marker it encounters can be expanded back to the original content.
- **Recoverable truncation**: results past the inline limit are saved to `result_*.txt` files inside the session workspace, and the worker's prompt teaches it to `read` those files (with `start`/`end` ranges) instead of guessing. The worker's **final output** follows the same contract — when it exceeds the ~30 KB budget, the full text is persisted and the reference travels back to the orchestrator.
- **Proactive recall**: each dispatched task is matched against persistent memory and saved sessions; relevant `[MEMORY AUTO-RECALL]` / `[SESSION RECALL]` blocks are injected into the worker's initial context automatically.
- **Opt-in context tools**: personas that declare `Memory`, `Session`, or `Knowledge` in their `tools` frontmatter get read-only search/read tools over those layers — see [Customizable Agents](/features/customizable-agents).
- **In-loop microcompact**: long worker runs progressively compact old tool results (CCR-backed, recoverable), the same mechanism the orchestrator uses.
- **Specialist prompt preserved**: in native function-calling mode the agent's specialist prompt — including custom personas — is kept, with only the XML tool syntax retired.

Squad results injected back into the orchestrator's history are aged by the same microcompact engine, so a large multi-worker dispatch no longer sits uncompactable in the context window.
...
Worker tool calls are presented to the policy engine in the same canonical `@coder <subcommand>` form as sequential mode, so rules written once apply everywhere. `/policy mode auto` also covers workers ("ask" auto-approves), with safety-immune operations still prompting. Purely in-process tools — squad mail, `recall_output`, and the read-only Memory/Session/Knowledge tools — have no filesystem, shell, or network side effects and skip the policy check.
