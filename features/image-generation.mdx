...
5. `GOOGLEAI_API_KEY`/`GEMINI_API_KEY` → **native Google** (Gemini image / "Nano Banana" `:generateContent`, generate **and** edit; default `gemini-3.1-flash-image` — the Imagen `:predict` family was shut down by Google on 2026-08-17).
6. `XAI_API_KEY` → **native xAI Grok Imagine** (OpenAI shape, omits `size`; default `grok-imagine-image-2.0`). **Generate only.**
7. `CHATCLI_IMAGE_PROVIDER=bedrock` → **AWS Bedrock** (Stability / Nova Canvas via `InvokeModel`, reusing the chat provider's credential chain).
...
| **Google** (Gemini image) | ✅ | ✅ | `:generateContent` with inline image |
| **Bedrock** (Stability / Nova) | ✅ | ✅ | Stability `image-to-image` / Nova `IMAGE_VARIATION` |
| xAI (Grok Imagine) | ✅ | ❌ | API only generates |
...
```bash
export CHATCLI_IMAGE_PROVIDER=xai            # generate with Grok Imagine
...
/model-image grok-imagine-image-2.0   # set the image model (applies to gen and edit)
...
/config image model gpt-5.6-sol       # set the model
