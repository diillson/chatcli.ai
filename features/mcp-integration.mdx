...
sidebarTitle: "Connect MCP Servers"
description: "Integrate external tools via Model Context Protocol with support for stdio, SSE and Streamable HTTP transports, plus OAuth 2.1 authorization for remote servers."
icon: "link"
---
...
#### What a server inherits

A stdio server starts with the parent environment (so `npx`, `uvx` and `docker` find their binaries and caches) plus its `env` map, whose values expand `${VAR}` against the parent. ChatCLI's own secrets never cross that boundary: `CHATCLI_ENCRYPTION_KEY` (and `_PREVIOUS`), `CHATCLI_JWT_SECRET`, `CHATCLI_AUTH_DIR`, `CHATCLI_AUDIT_LOG_PATH` and `CHATCLI_MANAGED_CONFIG` are removed before the spawn. A server that genuinely needs one of them opts in by naming it in its `env` (`"CHATCLI_AUDIT_LOG_PATH": "${CHATCLI_AUDIT_LOG_PATH}"`). Executable plugins get the same scrubbed environment. Provider API keys are not filtered — servers and launchers legitimately use them.
...
Refresh tokens eventually die server-side (AWS sign-in returns `TOKEN_EXPIRED`; standard OAuth servers return `invalid_grant`). ChatCLI treats these as **terminal**: automatic renewal stops immediately instead of retrying forever, the failure is latched so tool calls fail fast without extra round-trips, and the server is flagged `🔒 authorization required` — even mid-session, long after the startup handshake. Tool calls surface the actionable `@mcp-login` hint (the agent can re-authorize itself, opening your browser), and a `/mcp login <name>` recovers manually. Transient failures (network blips, 5xx) keep the normal retry behavior.
...
<Tip>
Useful combo for diagnosing connection issues: `/mcp status` (state), then `/mcp logs <name>` (the why). No need to restart with `LOG_LEVEL=debug`.
</Tip>
