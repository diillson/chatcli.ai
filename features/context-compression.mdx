...
Imagens são encolhidas antes de irem para modelos com visão — veja a página de [Entrada de Visão](/features/vision-input). Resumo: downscale da maior aresta para 1568px (o que os provedores já fazem server-side, então é token-equivalente) + re-encode JPEG de fotos, preservando transparência (PNG), sem nunca inflar o payload. Keyless, puro-Go.
