...
**Proactive auto-recall** (`CHATCLI_MEMORY_AUTORECALL`, on by default) adds a tiny hint-driven top-3-facts block per turn — but it rides in the *uncached trailing* region next to the wall-clock context, never in the stable digest, so the cached prefix stays byte-identical and none of the savings above are given back. See [Bootstrap and Memory](/features/bootstrap-memory) for details.
