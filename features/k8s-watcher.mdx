...
<Tabs>
  ...
  <Tab title="Multiple Resources (YAML)">
    ```bash
...
<Tabs>
  ...
  <Tab title="Multi-namespace (ClusterRoleBinding + shared ClusterRole)">
    When targets are in different namespaces, the Operator creates only a per-Instance `ClusterRoleBinding` pointing at the shared `chatcli-watcher` ClusterRole, which is pre-provisioned by the operator Helm chart (H5 hardening — see [k8s-operator#tls-and-rbac](/features/k8s-operator#tls-and-rbac)). The `chatcli-watcher` rules match the Role above, applied cluster-wide.
  </Tab>
</Tabs>
...
K8s Watcher alerts automatically feed into the Operator's **AIOps pipeline**. The Operator receives alerts over the `StreamAlerts` RPC the moment the watcher raises them (polling `GetAlerts` only for a server without the stream) and creates Anomaly CRs that are correlated into Issues, analyzed by AI, and automatically remediated.
...
See the [full AIOps Platform documentation](/features/aiops/notifications) for details.
</Note>
