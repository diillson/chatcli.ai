...
Every line of the LLM audit trail (`CHATCLI_AUDIT_LOG_PATH`) carries `seq`, `prev_hash` and `hash` — `hash = SHA-256(prev_hash ‖ canonical JSON of the entry)`. An edited, removed or reordered line breaks the chain from that point on; a restarted process continues the chain from the file's tail. `/config security verify-audit [path]` re-hashes the trail and reports the first broken line (entries written before the chain existed are counted as legacy).
