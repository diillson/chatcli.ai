...
Installed Plugins (12):
  ...
  @browser        - Drive a local Chrome (DevTools protocol)  [builtin]
  @forge          - PRs, issues, CI via gh/glab               [builtin]
  @view           - Attach a local image to the conversation  [builtin]
  @proc           - Background & interactive PTY processes     [builtin]
...
| Plugin | Domain | Docs |
|---|---|---|
| `@coder` | Full engineering (read, write, patch, multipatch, **outline/map**, exec, test, git, **checkpoint**) | [Plugin @coder](/features/coder-plugin) |
| `@taskgraph` | Execute an approved multi-task plan as a verified DAG: parallel workers per task, validation gates the engine runs itself, and an independent reviewer verdict before anything counts as done — plus a live browser dashboard | [Task Graph](/features/task-graph) |
| `@browser` | Drive a real local Chrome over the DevTools protocol: open, snapshot (numbered interactive elements), click, type, eval, screenshot, console and network capture — the verification loop for web work | [Browser Automation](/features/browser-automation) |
| `@forge` | Pull requests, issues and CI through the user's own authenticated `gh`/`glab` CLI (keyless): list/view/diff PRs, watch their checks, read failing run logs, and — behind confirmation — create PRs and comment | [Git Forge](/features/git-forge) |
| `@view` | Attach a local image (screenshot, mock, diagram) to the conversation through the vision pipeline so the model can **look** at it mid-task | [Vision Input](/features/vision-input) |
...
| `@proc` | Background processes: start/status/logs/stop for servers and watchers that must keep running while the agent tests against them (process tree dies with the session). Supports **interactive PTY sessions** — `start {pty:true}` runs a program on a real pseudo-terminal (REPLs, debuggers, ssh) and `stdin {id,text}` types into it (Unix-first) | — |
...
| `@dash` | The live dashboard as a tool: start it and hand the address over, stop it, read the reduced graph or the newest events of this or every chatcli process, mark a phase on the timeline | [Live Dashboard](/features/live-dashboard#driving-it-from-the-agent) |
| `@park` | Pause the agent for later resume (delay, until, for_url, for_cmd); inspect and message parked agents (list, note) | [Agent Park](/features/agent-park) |
...
<Note>
**Loop safety (doom-loop guard).** A tool-loop guard watches the ReAct cycle for stuck patterns. Beyond repeated *failures* of the same tool, it now also detects the same **successful** call repeated — interleaved or not — with no state change in between (e.g. re-reading the same file over and over): the result cannot differ, so it nudges the model to reuse the earlier output instead of burning turns. Any successful tool with side effects (a write, a patch, an exec) resets the tracking, so a legitimate re-read after an edit never trips it.
</Note>
