...
|---|---|---|
| `@coder` | Engenharia completa (read, write, patch, multipatch, exec, test, git) | [Plugin @coder](/features/coder-plugin) |
| `@read`, `@search`, `@tree` | **Tools atômicos** narrow para read-only ops | [Tools Atômicos](/features/atomic-tools) |
| `@todo` | Plano de tarefas do agente (TodoWrite parity) | [Tools Atômicos](/features/atomic-tools) |
| `@websearch`, `@webfetch` | Acesso a web (DuckDuckGo/SearxNG + HTTP GET) | [Web Tools](/features/web-tools) |
| `@scheduler` | Scheduler durável (schedule, wait, query, cancel) | [Scheduler](/features/scheduler) |
...
| `@lsp` | Navegação semântica de código via language servers: diagnostics, definition, references, symbols, hover | [LSP](/features/lsp-diagnostics) |
...
| `@park` | Pausa o agente para retomar (delay, until, for_url, for_cmd) | [Agent Park](/features/agent-park) |
| `@compress`, `@recall` | Compressão reversível sob demanda + recuperação do original (CCR) | [Compressão de Contexto](/features/context-compression) |
...
<Tip>Quando `CHATCLI_AGENT_PARALLEL_MODE=true`, o LLM orquestrador pode despachar 12 agents especialistas (FileAgent, CoderAgent, ShellAgent, GitAgent, SearchAgent, PlannerAgent, ReviewerAgent, TesterAgent, RefactorAgent, DiagnosticsAgent, FormatterAgent, DepsAgent) em paralelo. Veja a [documentação completa](/features/multi-agent-orchestration).</Tip>
