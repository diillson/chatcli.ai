...
A maioria das ferramentas de IA para linha de comando funciona como **assistentes**: voce pergunta, elas respondem. O ChatCLI vai alem, transformando a IA em um **agente autonomo** que nao apenas responde, mas **age**.
...
<Note>Esta nao e apenas uma funcionalidade -- e a fundacao para um novo modo de interagir com seu ambiente de desenvolvimento.</Note>
...
<Info>O agente pode usar plugins remotos da mesma forma que plugins locais -- a execucao e transparente. Ao desconectar, plugins remotos sao removidos automaticamente da listagem.</Info>
...
<Steps>
  ...
  <Step title="Execucao (Invocacao do Plugin)">
...
<Tabs>
  ...
  <Tab title="Ver detalhes">
    ...
    Plugin: @go-bench-gen
    Descricao: Gera arquivos de benchmark Go a partir de funcoes existentes
...
<Warning>Voce esta prestes a instalar codigo de terceiros que sera executado em sua maquina. Revise o codigo-fonte antes de prosseguir.</Warning>
...
<Steps>
  ...
  <Step title="Responder ao Contrato --metadata (Obrigatorio)">
  ...
  <Step title="Comunicacao via I/O Padrao">
    | Canal | Uso | Descricao |
    |-------|-----|-----------|
...
<Steps>
  ...
  <Step title="Dar permissao de execucao (CRITICO!)">
    ```bash
    chmod +x hello
    ```
  </Step>
  ...
  <Step title="Verificar instalacao">
    ```bash
    > /plugin list
...
O modo [/coder](/core-concepts/coder-mode) e especializado em engenharia de software e utiliza o plugin [@coder](/features/coder-plugin) para executar suas acoes. O `@coder` e um **plugin builtin** -- ja vem embutido no ChatCLI e funciona sem instalacao.
