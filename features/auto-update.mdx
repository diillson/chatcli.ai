...
| **go install** | No CI version stamp, but the Go module version is embedded in the binary | Runs `go install github.com/diillson/chatcli@vX.Y.Z`, pinned to the exact release the check detected (never the module proxy's `@latest`, which can lag behind GitHub) |
...
── ChatCLI · Update ──────────────────────────────────────────────
  ...
  Current: v1.169.0 · Latest: v1.192.0

── What's new in v1.192.0 ────────────────────────────────────────
  ...
  https://github.com/diillson/chatcli/releases/tag/v1.192.0

  Downloading v1.192.0 (chatcli-darwin-arm64)...
  ✓ Updated to v1.192.0 — restart chatcli to use the new version.
```
...
For scripts, CI and automation the same flow is available without entering the REPL:

```bash
chatcli update          # applies through the detected channel
chatcli update check    # reports only, never applies
```

Exit codes: `0` updated or already on the latest version (including `check`), `1` check/apply failure or a manual channel (Docker, source build) with a pending update, `2` invalid usage.
...
sudo curl -fsSL -o /usr/local/bin/chatcli https://github.com/diillson/chatcli/releases/download/v1.192.0/chatcli-darwin-arm64 && sudo chmod +x /usr/local/bin/chatcli
```
...
```text
  Version: 1.192.0 (commit: 52441581)
  ✓ Auto-updated to v1.192.0
```
...
```text
  ⬆ v1.192.0 available — run /update to upgrade
...
```text
  ⬆ ChatCLI v1.192.0 is available — run /update to upgrade.
```
...
🔄 Auto-update
   ...
   Current version                 1.192.0
   Latest release                  1.192.0
