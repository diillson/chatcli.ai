...
── ChatCLI · Update ──────────────────────────────────────────────
  ...
  Current: v1.169.0 · Latest: v1.173.0

── What's new in v1.173.0 ────────────────────────────────────────
  ...
  Downloading v1.173.0 (chatcli-darwin-arm64)...
  ✓ Updated to v1.173.0 — restart chatcli to use the new version.
```
...
```text
  Version: 1.173.0 (commit: 52441581)
  ✓ Auto-updated to v1.173.0
```

In `notify` mode (the default) the welcome screen shows the invitation instead:

```text
  ⬆ v1.173.0 available — run /update to upgrade
```

The banner never waits for a fresh check: it trusts the last known release data even past the daily re-check window. And when the boot-time refresh discovers a new release *after* the welcome screen already printed, ChatCLI announces it in the running session, at the next prompt turn — you don't have to restart (or run `/version`) to find out:

```text
  ⬆ ChatCLI v1.173.0 is available — run /update to upgrade.
```
...
🔄 Auto-update
   ...
   Current version                 1.173.0
   Latest release                  1.173.0
