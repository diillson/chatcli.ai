...
| `outline` | Symbol skeleton of a file (declarations + line numbers) | `--file` (req.) |
| `map` | Budgeted structural map of a whole tree, files ranked by structure | `--dir`, `--budget` |
...
| `checkpoint` | Shadow-git workspace snapshots (automatic before mutations) | `--list`, `--create`, `--restore`, `--label` |
...
<AccordionGroup>
  ...
  <Accordion title="read -- Read Files" icon="file-lines">
    ...
    | `--max-bytes` | int | No | 200000 | Maximum byte limit for output (the ceiling; a read that asks for no range is capped at 2,000 lines with a `--start` continuation hint, the way Claude Code pages reads) |
  ...
  <Accordion title="write -- Write Files" icon="pen">
    ...
    <Note>
    **Post-edit diagnostics.** After a successful `write` (also `patch` and `multipatch`), ChatCLI runs the language server over the touched files and appends any findings to the tool result as a `[DIAGNOSTICS]` block — so a broken edit is caught **immediately**, in the same turn, instead of turns later when a test fails. It is silent on clean files, capped at 5 files / 3000 characters, and disabled with `CHATCLI_CODER_AUTODIAG=off`. A language server must be available for the file's language (see [LSP Diagnostics](/features/lsp-diagnostics)).
    </Note>
  ...
  <Accordion title="outline -- File Symbol Skeleton" icon="list-tree">

    Renders a file's **symbol skeleton** — its declarations with line numbers — so you understand a file's shape without reading the whole thing. For Go files the signatures are **exact** (parsed with Go's `go/ast`: functions with their receiver, structs, interfaces, consts, vars); for Python, JS/TS/JSX, Java, Ruby, Rust, Kotlin, C# and PHP a pattern-based outline recognizes the common declaration forms. **Read-only.**

    ### Flags

    | Flag | Type | Required | Default | Description |
    |---|---|---|---|---|
    | `--file` | string | **Yes** | -- | Path of the file to outline |
    ...
    <Tabs>
      <Tab title="JSON">
        ```xml
        <tool_call name="@coder" args='{"cmd":"outline","args":{"file":"cli/cli.go"}}'/>
        ```
      </Tab>
      <Tab title="CLI-style">
        ```
    ...
    <Tip>
    Reach for `outline` before `read` when you only need to know *what* is in a file (which functions, types, methods) and *where* — it costs a fraction of the tokens of reading the whole file.
    </Tip>

  </Accordion>

  <Accordion title="map -- Repository Structure Map" icon="map">

    Renders a **character-budgeted structural map** of a whole tree: every source file is outlined, files are **ranked by how much structure they carry**, and their skeletons are printed until the budget is spent — with an explicit note when files are elided. It ignores `.git`, `node_modules`, `vendor`, `dist`, `build`, hidden files and `*_test.go`. **Read-only.** Use it to orient yourself in a large codebase without reading whole files.

    ### Flags

    | Flag | Type | Required | Default | Description |
    |---|---|---|---|---|
    | `--dir` | string | No | `"."` | Root directory to map |
    | `--budget` | int | No | 8000 | Maximum characters of structure to emit |
    ...
    <Tabs>
      <Tab title="JSON">
        ...
        <tool_call name="@coder" args='{"cmd":"map","args":{"dir":"."}}'/>

        <!-- Map a subtree with a larger budget -->
        <tool_call name="@coder" args='{"cmd":"map","args":{"dir":"./cli","budget":16000}}'/>
        ```
      </Tab>
      <Tab title="CLI-style">
        ```
        map --dir . --budget 8000
        map --dir ./cli --budget 16000
        ```
      </Tab>
    </Tabs>
  ...
  <Accordion title="checkpoint -- Shadow-git Snapshots" icon="clock-rotate-left">

    Workspace snapshots into a **shadow git repository**, a stronger safety net than the per-file `.bak` rollback — it covers multi-file edits and `exec` side effects, not just a single file.

    A checkpoint is taken **automatically before every mutating subcommand** (`write`, `patch`, `multipatch`, `exec`). The snapshot lives in a separate `GIT_DIR` under `~/.chatcli/checkpoints/<hash>` with your workspace as its work tree, so **your own `.git` is never touched** and your project's `.gitignore` is honored.

    ### Flags

    | Flag | Type | Required | Default | Description |
    |---|---|---|---|---|
    | `--list` | bool | No | (default) | List snapshots, newest first |
    | `--create` | bool | No | `false` | Record a snapshot now |
    | `--restore` | string | No | -- | Rewind the working tree to the snapshot with this hash |
    | `--label` | string | No | `manual` | Label for a `--create` snapshot |
    ...
    <Tabs>
      <Tab title="JSON">
        ```xml
        <!-- List snapshots -->
        <tool_call name="@coder" args='{"cmd":"checkpoint","args":{"list":true}}'/>

        <!-- Record one now -->
        <tool_call name="@coder" args='{"cmd":"checkpoint","args":{"create":true,"label":"before refactor"}}'/>

        <!-- Rewind to a snapshot -->
        <tool_call name="@coder" args='{"cmd":"checkpoint","args":{"restore":"a1b2c3d"}}'/>
        ```
      </Tab>
      <Tab title="CLI-style">
        ```
        checkpoint --list
        checkpoint --create --label "before refactor"
    ...
    <Note>
    `--restore` never deletes files added **after** the snapshot; it only rewinds what the snapshot tracked. It goes through the [security gate](/features/coder-security); `--list` and `--create` are read-only.
    </Note>

    <Info>
    Auto-snapshots are **best-effort** (a checkpoint failure never blocks the edit), **throttled** to at most one per 15 seconds (a burst of writes produces one checkpoint), a **no-op when `git` is not installed**, and switched off entirely with `CHATCLI_CODER_CHECKPOINTS=off`.

    They are also **bounded**: workspaces as broad as your home directory (or any directory containing it) are never auto-snapshotted — hashing that much of the disk would freeze every command — and each snapshot runs under a hard deadline (10s automatic, 60s for an explicit `checkpoint create`, overridable in seconds via `CHATCLI_CODER_CHECKPOINT_TIMEOUT`). Repeated snapshot failures back off exponentially and, after three in a row, disable automatic snapshots for the rest of the session with a one-line warning.
    </Info>
...
In `/coder` mode, the assistant responds with a `reasoning` block followed by a `tool_call`. The block is sized to the task: a single-step request (one lookup, one command, one answer) gets one line and no task list, while anything with two or more steps gets a short numbered plan with `[✓]` marks as it progresses. The rule lives in the stable system prompt on purpose — deciding it per query would change the prompt and rewrite the cached prefix. The same prompt binds claims to evidence: when tool results ground the answer, specific figures, dates, deadlines, rules and place names come only from that evidence, and whatever comes from the model's own knowledge is labeled unverified or an estimate in the sentence that uses it, never presented with the authority of a fetched source. Here is a complete engineering workflow:
