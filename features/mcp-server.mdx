...
| `manage_session` | Persist and restore the conversations behind `ask_chatcli`'s `session` parameter — save/load/attach/detach/status/list/delete/clear/active, plus **full-text `search`** across the saved-session store, **`fork`** to copy a saved session, and **`policy_mode`** to switch the session's [security-policy mode](/features/coder-security#session-policy-mode-policy) (`name: auto \| interactive \| status`). Shares the `/session` store with the interactive REPL; under `CHATCLI_HUB_ISOLATE=true` (multi-user gateway) `load`, `attach` and `delete` are refused on this surface because it carries no principal. `save` and `load` now **bind** the live session to the saved name, and `attach` binds even to a not-yet-existing one: every turn is written through to the file and writes made by other surfaces (REPL, gateway, another server) are adopted before each turn — [cross-surface continuity](/features/session-management#cross-surface-continuity); `detach` drops the binding, `status` reports it. Live sessions also **autosave by default** as rolling `mcp-<name>` mirrors after every turn (`CHATCLI_MCP_SESSION_AUTOSAVE`; unset follows `CHATCLI_SESSION_AUTOSAVE`), and `clear` saves one last time before discarding. |
