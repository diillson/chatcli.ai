...
| `coder_task` | The **full agent loop** on a task — recommended for anything that needs autonomous work. Reads/edits files, runs commands and uses every built-in tool, memory, knowledge and any MCP server ChatCLI is itself connected to. The `session` id scopes which `/context` attachments and knowledge bases the run sees. Accepts `provider` / `model` and `quality` toggles. |
| `agent_task` | The **command-oriented (ReAct) loop** that proposes shell commands step by step. Prefer `coder_task` for general work. Same routing, quality and session-scoping options. |
...
- **Permission dialogs** — dangerous commands raise `session/request_permission` (native Allow/Reject in the IDE) instead of blanket auto-approve/block.
