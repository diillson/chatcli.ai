...
Você pode adicionar uma policy local no diretorio do projeto:
...
<Info>Para a lista completa de protecoes de segurança do ChatCLI, veja a [documentacao de Segurança e Hardening](/features/security).</Info>
...
O `ParseToolCalls` aplica deduplicacao automatica: tool calls extraidas pelo parser JSON que já existam como parte de uma tool call XML são descartadas. A comparacao usa o nome da tool, os argumentos e o texto bruto para detectar duplicatas.
...
<Steps>
  ...
  <Step title="Exec Seguro">
    O `@coder exec` bloqueia padroes perigosos por padrão (50+ regras). Use `--allow-unsafe` apenas quando necessario.
  </Step>
</Steps>
...
<Note>Os prompts de segurança de multiplos workers são **serializados** -- apenas um prompt por vez e exibido, evitando sobreposicao visual. Regras criadas durante a sessao (via "Always" ou "Deny") são imediatamente visiveis para todos os workers subsequentes.</Note>

---

## UI do Modo Coder

Você pode controlar o estilo e o banner do `/coder` via variaveis de ambiente:
