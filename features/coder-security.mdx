...
|---------|-------------------------|
| **ACP** (IDE) | Raises the IDE's native dialog via `session/request_permission`; a denial blocks and the model replans |
| **MCP** with `elicitation` capability | Raises an `elicitation/create` approval form in the client |
