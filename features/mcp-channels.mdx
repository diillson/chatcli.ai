...
description: "Receba push messages de servidores MCP para notificações reativas de CI, alerts e eventos externos"
...
<Steps>
  <Step title="Conexão SSE">
    O ChatCLI mantem uma conexão SSE persistente com cada servidor MCP que suporta channels. A conexão e estabelecida automaticamente ao iniciar.
  </Step>
  ...
  <Step title="Reacao do agent">
    O agent pode reagir aos eventos injetados, tomando ações como investigar falhas, ler logs ou notificar o usuário.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  ...
  <Card title="Alertas de Monitoramento" icon="triangle-exclamation">
    Receba alertas de Prometheus, Grafana ou Datadog e deixe o agent analisar métricas e logs.
  </Card>
  <Card title="Eventos de Deploy" icon="rocket">
    Acompanhe deploys em staging e produção com notificações de status em tempo real.
  </Card>
