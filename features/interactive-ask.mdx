...
description: "A IA pergunta e você decide num menu navegável: 1 a 4 perguntas, single ou multi-select e opção livre, com as respostas voltando ao modelo no mesmo turno."
icon: "list-check"
---
...
| **Chat** | ✅ (exceção controlada) | Chat é tool-less por design; abre exceção **apenas** para `ask_user`, nunca para tools de execução |

---

## A interface

Cada chamada traz de 1 a 4 perguntas. Para cada pergunta:
...
O modo chat é **tool-less por design** — ele nunca executa tools. A única exceção sancionada é o `ask_user`: quando habilitado, o chat passa a oferecer **exclusivamente** essa ferramenta (nenhuma de leitura/escrita/execução).
