...
| **Chat** | ✅ (controlled exception) | Chat is tool-less by design; the exception opens **only** for `ask_user`, the [read-only knowledge base query](/features/knowledge-base), [`@graphview`](/features/graphview), [memory/profile](/features/bootstrap-memory) and [`context_pull`](/features/prefix-curation) — never for execution tools |
...
Chat mode is **tool-less by design** — it never executes tools. There are five sanctioned exceptions, all harmless by construction: `ask_user` (interactive), the [read-only knowledge base query](/features/knowledge-base) (`/config chat knowledge`), [`@graphview`](/features/graphview) (render-once, `/config chat graphview`), [`context_pull`](/features/prefix-curation) (read-only recovery of material curated out of the prefix, `/config chat pull`) and [memory](/features/bootstrap-memory) (`/config chat memory` — persists durable facts you reveal in the same turn **and** carries the pull path that lets chat honor `index` memory mode: compact digest, on-demand recall, proactive auto-recall/session-recall/graph-card blocks). When enabled, chat offers **exclusively** those tools (no file read/write/exec tools).
...
- [Native Tool Use](/features/native-tool-use) — how structured calls work
