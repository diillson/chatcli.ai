...
| **Chat** | ✅ (exceção controlada) | Chat é tool-less por design; abre exceção **apenas** para `ask_user`, a [consulta read-only à knowledge base](/features/knowledge-base), o [`@graphview`](/features/graphview) e a [memória/perfil](/features/bootstrap-memory) — nunca para tools de execução |
...
O modo chat é **tool-less por design** — ele nunca executa tools. As exceções sancionadas são quatro, todas inofensivas por construção: o `ask_user` (interativa), a [consulta read-only à knowledge base](/features/knowledge-base) (`/config chat knowledge`), o [`@graphview`](/features/graphview) (renderização única, `/config chat graphview`) e a [memória/perfil](/features/bootstrap-memory) (`/config chat memory` — persiste fatos duráveis que você revela, no mesmo turno). Quando habilitadas, o chat oferece **exclusivamente** essas ferramentas (nenhuma de leitura de arquivos/escrita/execução).
...
- [Tool Use Nativo](/features/native-tool-use) — como as chamadas estruturadas funcionam
- [Modo Agente](/core-concepts/agent-mode)
