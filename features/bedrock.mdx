...
- The catalog flags these models with the `bedrock_mantle_only` capability and the client routes the request through the Messages endpoint automatically — `/switch --model claude-opus-5` (or `claude-sonnet-5` / `claude-fable-5` / `claude-fable-5-1`) just works. Fable 5.1 (`anthropic.claude-fable-5-1`, Sep 1, 2026) is exposed through `us.`/`global.` profiles only and is priced like the first-party API ($10/$50, cache reads $0.25).
...
Anthropic models from the 4.x–4.6 era (4, 4.1, 4.5, 4.6) **do NOT accept direct on-demand invocation by base model ID** (the dateless new generation — Fable 5.1, Fable 5, Opus 5, Sonnet 5, Opus 4.8/4.7 — needs no profile; see the section above). Attempting this with an older-era model returns:
