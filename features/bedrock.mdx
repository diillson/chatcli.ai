...
- **Converse API (default)** — AWS's unified schema covering everything else: OpenAI GPT-5.6 (Sol/Terra/Luna — Converse-only on Bedrock), xAI Grok 4.6, Llama, Amazon Nova (incl. Nova 2 Lite), Mistral, Cohere, AI21 Jamba, DeepSeek, Stability, Writer Palmyra, Moonshot Kimi, MiniMax, Qwen, Z.AI/GLM, Google Gemma, NVIDIA Nemotron, TwelveLabs Pegasus, and any provider AWS onboards next.
...
<CardGroup cols={2}>
  ...
  <Card title="Native embeddings" icon="vector-square">
    Embeddings provider reuses the same AWS credential chain. Titan v1/v2 + Cohere v3. See [RAG + HyDE](/features/quality/rag-hyde).
  </Card>
</CardGroup>
...
- **Profile selection:** `BEDROCK_PROFILE` or `AWS_PROFILE` — from the process environment or the resolved environment file. `BEDROCK_PROFILE` wins and scopes the choice to ChatCLI, leaving `AWS_PROFILE` free for the rest of your shell; both are honored identically by chat, agent, model listing, embeddings and image generation.
...
- **`aws login` (AWS CLI v2 sign-in):** profile with `login_session` in `~/.aws/config` — these profiles carry no key material anywhere else
- **Assume-role / credential_process:** profiles with `role_arn` or `credential_process` in `~/.aws/config`
- **Login token cache:** files in `~/.aws/sso/cache/` or `~/.aws/login/cache/` (a prior `aws sso login` / `aws login`)
...
export BEDROCK_MODEL=claude-opus-4-8  # optional default model (catalog aliases work)
chatcli
```
...
**Important:** the AWS SDK **does not** know which profile is "logged in". You **must** indicate the profile via `BEDROCK_PROFILE`/`AWS_PROFILE` (env, environment file, or flag). If your SSO profile is named `default`, it is used automatically without it.
</Info>

<Warning>
**Running ChatCLI from an IDE or an MCP client?** Those spawn `chatcli acp` / `chatcli mcp-server` as a child process **without your shell profile**, so a profile exported from `.zshrc`/`.bashrc` — and `CHATCLI_DOTENV` itself — never reaches them. The environment file is then discovered as `./.env` → `~/.chatcli/.env` → `~/.env`; if none of those holds your profile, the SDK falls back to the `default` profile and Bedrock answers from the wrong account, or the credential chain exhausts into the misleading `no EC2 IMDS role found`.

Keep the profile in `~/.chatcli/.env` (or `~/.env`), or pin it in the client's `env` block. `/config` reports the *Effective AWS profile* and the variable it came from, and credential errors name the profile, the environment file in effect and the `aws sso login` command to refresh the session.
</Warning>
...
| :--- | :--- | :--- | :--- |
| `anthropic.*`, `global./us./eu./apac.anthropic.*` and any id containing `claude`/`fable` | **Anthropic Messages** | `anthropic_version`, `messages`, `system` (with `cache_control`) | Preserves cache breakpoints, extended-thinking budget, every Claude knob. Bare ids (e.g. `claude-fable-5`) are normalized to the invokable catalog ID — previously they fell through to Converse, which **drops cache markers** |
| `openai.gpt-oss-*`, `us.openai.gpt-oss-*`, etc. | **OpenAI Chat Completions** | `messages`, `max_completion_tokens` | Stable schema with broad coverage for GPT-OSS |
| `global./us./in.openai.gpt-5.6-sol/-terra/-luna` | **Converse API** (catalog capability `bedrock_converse_only`) | `messages`, `system`, `inferenceConfig` | GPT-5.6 on Bedrock speaks Converse/Responses/Chat Completions but **not `InvokeModel`**; the catalog flag wins over the `openai.` vendor sniff, so no env var is needed |
...
<Tip>
**Why do Anthropic and OpenAI stay out of Converse?** Anthropic's cache_control breakpoints and extended-thinking knobs map onto Converse with a different shape — we deliberately don't disturb the production-proven cache planner. OpenAI gpt-oss runs stable on direct `InvokeModel` and Converse coverage for those IDs varies by region. The exception is **GPT-5.6** (Sol/Terra/Luna), which has no `InvokeModel` surface on Bedrock at all — its catalog entries carry `bedrock_converse_only` and always go through Converse. Set `BEDROCK_PROVIDER=converse` if you want to experiment with everything on Converse.
</Tip>
...
The newest Claude generation on Bedrock (**Fable 5.1, Fable 5, Opus 5.5, Opus 5, Sonnet 5, Opus 4.8, Opus 4.7**) uses **dateless IDs** — `anthropic.claude-fable-5-1`, `anthropic.claude-fable-5`, `anthropic.claude-opus-5-5`, `anthropic.claude-opus-5`, `anthropic.claude-sonnet-5`, `anthropic.claude-opus-4-8`, `anthropic.claude-opus-4-7` — with **no** ARN-versioned IDs (`...-v1:0`). On the `InvokeModel` path (Opus 4.8/4.7) the bare dateless ID is **not on-demand invokable** — AWS answers "retry with the ID or ARN of an inference profile" — so ChatCLI invokes them through the **`global.` inference profile** (`global.anthropic.claude-opus-4-8`); the Messages-endpoint models (Opus 5, Sonnet 5, Fable 5, Fable 5.1) use the bare dateless ID.

**Claude Opus 5**, **Claude Sonnet 5**, **Claude Fable 5** and **Claude Fable 5.1** have one particularity: they are served **exclusively** by the *Claude in Amazon Bedrock* endpoint — the Messages API at `https://bedrock-mantle.{region}.api.aws/anthropic/v1/messages`. Opus 5 and Sonnet 5 do not exist on legacy `InvokeModel`, and Fable 5 / Fable 5.1 reject it with `400 ValidationException: data retention mode 'default' is not available for this model` (they require 30-day data retention, available only under the *Claude in Amazon Bedrock* agreement). ChatCLI handles this on its own:

- The catalog flags these models with the `bedrock_mantle_only` capability and the client routes the request through the Messages endpoint automatically — `/switch --model claude-opus-5` (or `claude-opus-5-5` / `claude-sonnet-5` / `claude-fable-5` / `claude-fable-5-1`) just works. Fable 5.1 (`anthropic.claude-fable-5-1`, Sep 1, 2026) is exposed through `us.`/`global.` profiles only and is priced like the first-party API ($10/$50, cache reads $0.25).
...
- **Automatic fallback to InvokeModel**: in the default routing (`BEDROCK_ANTHROPIC_ENDPOINT` unset or `auto`), if the Mantle call fails after its retries — regional outage, VPC without a `bedrock-mantle` interface endpoint, account-level restriction — ChatCLI re-sends the **same request** through the legacy `InvokeModel` runtime under the `global.` inference-profile spelling (`anthropic.claude-sonnet-5` → `global.anthropic.claude-sonnet-5`; IDs that already carry a profile prefix or ARNs pass through unchanged). The configured model is never mutated: the next call tries Mantle first again. A warning log names both endpoints when the fallback fires.
- **Operational overrides**: `BEDROCK_ANTHROPIC_ENDPOINT=mantle|invoke` pins every Claude model onto one wire — `mantle` disables the fallback (you asked for that surface explicitly), `invoke` never touches the Messages endpoint. `BEDROCK_MANTLE_BASE_URL` points the Mantle surface at VPC endpoints or proxies (it is a different host and service than `BEDROCK_BASE_URL`, which covers only the InvokeModel runtime). Corporate TLS (`CHATCLI_BEDROCK_CA_BUNDLE` etc.) is honored on both.

<Note>
Opus 4.8 and Opus 4.7 stay on `InvokeModel` by default (served by the same infrastructure as the Messages endpoint); set `BEDROCK_ANTHROPIC_ENDPOINT=mantle` to move them to the new endpoint too. Prerequisite for Opus 5/Sonnet 5/Fable 5/Fable 5.1: enable the model under **Model access** in the Bedrock console with a data retention mode selected.
</Note>
...
Anthropic models from the 4.x–4.6 era (4, 4.1, 4.5, 4.6) **do NOT accept direct on-demand invocation by base model ID** (the dateless new generation — Fable 5.1, Fable 5, Opus 5.5, Opus 5, Sonnet 5, Opus 4.8/4.7 — needs no profile; see the section above). Attempting this with an older-era model returns:
...
<Info>
ChatCLI already uses a global inference profile as the **default model** (`global.anthropic.claude-sonnet-4-6`). The Claude 3.x line is gone from Bedrock (Haiku 3 reaches EOL on Sep 10, 2026) and was removed from the catalog; Sonnet 4 (EOL Oct 14, 2026) and Opus 4.1 (EOL Jan 8, 2027) are **Legacy** but still served through their `global.`/`us.`/`eu.` profiles.
</Info>
...
```bash
export BEDROCK_BASE_URL=https://bedrock.internal.example.com
```
...
export AWS_ENDPOINT_URL_BEDROCK=https://bedrock.vpc.internal   # control plane (model listing)
```
...
| :--- | :--- | :--- |
| `BEDROCK_MODEL` | Default model when none is selected — same role as `COPILOT_MODEL`/`MOONSHOT_MODEL` on other providers; catalog aliases work (`claude-opus-4-8`) | `global.anthropic.claude-sonnet-4-6` |
...
| `BEDROCK_BASE_URL` | Custom base URL for the whole Bedrock surface — runtime and model listing (VPC endpoints, corporate DNS) — parity with Claude Code's `ANTHROPIC_BEDROCK_BASE_URL`; wins over `AWS_ENDPOINT_URL*` | — |
| `AWS_ENDPOINT_URL_BEDROCK_RUNTIME` | Override for Bedrock Runtime endpoint | — |
| `BEDROCK_CONTROL_BASE_URL` | Optional control-plane override (model listing) when it lives on a different host than `BEDROCK_BASE_URL` — VPC endpoints are per service; wins over `AWS_ENDPOINT_URL_BEDROCK` | inherits `BEDROCK_BASE_URL` |
...
| `BEDROCK_ANTHROPIC_ENDPOINT` | Claude wire routing: `auto` (default — catalog decides, with Mantle→InvokeModel fallback on failure), `mantle` (pins the Messages endpoint, no fallback) or `invoke` (legacy InvokeModel only) | `auto` |
...
All these vars surface in `/config providers` (chat) and `/config quality` (embeddings). See [Environment Variables](/reference/environment-variables) for the full reference.
...
<AccordionGroup>
  <Accordion title="bedrock: model X requires an inference profile">
    ChatCLI message when you select a base ID that requires an inference profile (Claude 4.x/4.5/4.6 and equivalents from other providers). The message **already suggests the fix**:

    ```text
    bedrock: model "anthropic.claude-sonnet-4-5-20250929-v1:0" requires an
    inference profile (the bare foundation ID is not invokable on-demand).
    Try selecting "global.anthropic.claude-sonnet-4-5-20250929-v1:0",
    "us.anthropic.claude-sonnet-4-5-20250929-v1:0",
    "eu.anthropic.claude-sonnet-4-5-20250929-v1:0", or
    "apac.anthropic.claude-sonnet-4-5-20250929-v1:0" instead, or run
...
| `cohere.embed-v4:0` (also `us.`/`eu.`/`global.` profiles) | Cohere v4 | 1536 (default) |
| `amazon.nova-2-multimodal-embeddings-v1:0` | Nova MME | 256 / 384 / 1024 / 3072 (default 3072) |

Reuses **the same credentials chain** as the chat client — `BEDROCK_REGION` / `AWS_REGION` / `AWS_PROFILE` / `~/.aws/credentials` etc. See [RAG + HyDE](/features/quality/rag-hyde) for the retrieval architecture.
