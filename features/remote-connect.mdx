...
| `--provider <name>` | Overrides the server's LLM provider: `OPENAI`, `OPENAI_ASSISTANT`, `CLAUDEAI`, `BEDROCK`, `GOOGLEAI`, `XAI`, `ZAI`, `MINIMAX`, `MOONSHOT`, `STACKSPOT`, `OLLAMA`, `COPILOT`, `OPENROUTER`, `DEVIN` (DEVIN is a local-CLI transport: it needs the Devin CLI on the server host and is not in the server image) | |
...
| `--max-tokens <int>` | Maximum tokens in response (unset: the server's provider `*_MAX_TOKENS` override, else the model's catalog ceiling) | |
...
- **Cost**: `/cost` prices the real token usage the server reports for each reply

---

## Streaming and Cost

Replies stream as the provider produces them: the server forwards each fragment over the `StreamPrompt` RPC and the client renders it as it arrives, exactly as a local provider would. The final message carries the token usage and stop reason the provider reported, so the local cost tracker prices real tokens — `/cost` over a remote connection shows the same numbers a local session would, not a character estimate. The reply's provider and model follow the server's attribution (the server's fallback chain may answer with another provider) unless you pinned them with `--provider`/`--model`.

<Note>
A request that names no provider or model and forwards no credential is served by the server's fallback chain when one is configured (`CHATCLI_FALLBACK_*`), otherwise by the server's default provider. `--max-tokens` overrides the server's choice; without it the server applies the provider's `*_MAX_TOKENS` override, else the model's catalog ceiling.
</Note>
