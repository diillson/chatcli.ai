...
<Frame caption="The page bound to a saved session: the transcript on the right, the saved sessions on the left, the mode, provider and model in the header, the session cost, the composer with attachments, voice and image generation, and the panel open on the skills. (Synthetic data.)">
  <img src="/images/web-ui.png" alt="ChatCLI web UI: a chat transcript bound to a saved session, with the sessions sidebar, mode tabs, provider and model selectors, cost, the composer and the panel open on the skills" />
</Frame>
...
<Frame caption="A tour of the page: the panel with status, the tool catalog with its filter, skills and commands; `/` and `@` open palettes in the composer; the mode tabs; the light theme. (Synthetic data.)">
  <img src="/images/web-ui.gif" alt="Animated tour of the ChatCLI web UI: opening the panel, filtering tools, browsing skills and commands, the slash and at palettes in the composer, switching mode and theme" />
</Frame>
