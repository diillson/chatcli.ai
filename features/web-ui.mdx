...
<Frame caption="The page bound to a saved session: the transcript on the right, the saved sessions on the left, the mode, provider and model in the header, the session cost, and the composer with attachments, voice and image generation. (Synthetic data.)">
  <img src="/images/web-ui.png" alt="ChatCLI web UI: a chat transcript bound to a saved session, with the sessions sidebar, mode tabs, provider and model selectors, cost and the composer" />
</Frame>
