...
A tool **`@context`** dá ao agente o mesmo poder de auto-serviço que ele já tem com [skills](/features/skill-authoring), mas para conhecimento:
...
<Tip>
O modo **`url`** do `@docs-flatten` é o que fecha o ciclo para docs que **só existem como site HTML** (sem repo de Markdown): um crawl raso, mesmo-domínio, reaproveitando o motor de fetch do [`@webfetch`](/features/web-tools) e emitindo o mesmo JSONL. Bounded por `maxPages`/`maxDepth` — sem truncamento silencioso.
</Tip>
