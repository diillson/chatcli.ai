...
<Info>You don't have to ask for it by name. A substantial task (complexity ≥ 6) is **auto-routed** here by default — see [Plan-and-Solve → Routing to the task graph](/features/quality/plan-and-solve#routing-to-the-task-graph). The routing steers, it doesn't force: genuinely serial or small work still runs directly.</Info>
...
- `tools` optionally grants the task's **executor** worker session plugins — `"tools": ["@browser", "@websearch"]`. The reviewer never receives them. See [Worker capabilities](#worker-capabilities) below.
...
<Info>This dashboard goes deep on **one task graph run**. To watch the whole runtime live (every agent, LLM request, tool, skill, MCP server, pattern and background job, across processes), use [`/dash`](/features/live-dashboard); task graph runs show up there too.</Info>
...
<Frame caption="A run in flight: the design task is done, the two implementation tasks finished (one after a retry), the verification tasks are running and the shipping task waits on them; the feed on the right is the append-only trail. (Synthetic data.)">
  <img src="/images/task-graph.png" alt="ChatCLI task graph dashboard: a DAG in phase swimlanes with done, running and pending tasks, dependency curves and the event feed" />
</Frame>

<Frame caption="The same run from start to finish: tasks light up as the ready set advances, a gate failure sends a task back for a second attempt, a click opens its evidence with both attempts, and the Results tab sums critical path, times and cost. (Synthetic data.)">
  <img src="/images/task-graph.gif" alt="Animated ChatCLI task graph dashboard: tasks moving from pending to running, reviewing and done, a retry after a failed gate, the evidence popover and the Results tab" />
</Frame>
...
Each executor and reviewer runs with the session's own context — proactive memory/session recall, CCR expansion of truncated results, and read-only memory/session/knowledge tools — so a worker picks up what the session already learned instead of starting blind.

Beyond that, two capabilities are opt-in per task:

- **Tool grants.** `"tools": ["@browser", "@websearch"]` on a task gives its **executor** those session plugins (builtins or `mcp_*`), gated by the same security policy — a `@browser click` prompts exactly as it would in `/coder`. The reviewer never gets tools (its job is inspection). `@browser` drives one shared page, so grant it to a **single** task at a time; parallel browser tasks would corrupt each other's element refs.
- **User skills.** A worker's task text is matched against your own [skills](/features/builtin-skills) the same way the orchestrator matches them: pinned skills always apply, trigger-matched ones are injected (capped and budgeted). Your curated knowledge reaches every executor.

When a run finishes, a compact **learning digest** feeds the session's long-term memory (facts, episodes, self-evolving skill candidates) and a Reflexion lesson is queued for each failed task — so the graph teaches the session, not just delivers.
