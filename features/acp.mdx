...
A dialog that never gets an answer cannot freeze the run: the wait is bounded by **`CHATCLI_MCP_PERMISSION_TIMEOUT`** (shared with the [MCP server](/features/mcp-server#approval-prompts-via-elicitation); a Go duration like `90s`/`10m` or plain seconds, default `600s`; `0`/`off` waits until the turn dies). On expiry the action **denies fail-safe**, the model is told the request went **unanswered** — not that you denied it — so it can continue and explain, and later dialogs in the same run fail fast instead of stalling again.
