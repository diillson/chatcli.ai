...
| AWS Bedrock — Claude (InvokeModel, Mantle) | Same markers, 5-minute wire default | ✅ |
| AWS Bedrock — Converse (Amazon Nova, Claude via Converse) | `cachePoint` after the system blocks and on the last user message; other Converse vendors reject the block and stay automatic | ✅ (`TokenUsage` cache read/write) |
...
| OpenRouter | `cache_control` on system and last user message for `anthropic/` and `google/` models; other vendors cache automatically | ✅ |
