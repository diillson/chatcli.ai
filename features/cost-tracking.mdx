...
| Google Gemini | Implicit caching (automatic on 2.5+, favours the stable-first prefix) | ✅ (`cachedContentTokenCount`) |
...
Explicit cache *resources* (Gemini `cachedContents`, Moonshot context caching) are a separate lifecycle with storage billing and are not used yet.
