...
O gateway é iniciado em foreground pelo comando `/gateway` e roda até `Ctrl+C`:

```text
/gateway start     # inicia os adaptadores configurados
/gateway status    # lista plataformas registradas e quais estão prontas (configuradas)
```

Cada adaptador **só liga quando suas variáveis obrigatórias estão presentes** — defina apenas os canais que quiser usar. Referência completa em [Variáveis de Ambiente → Chat Gateway](/reference/environment-variables#chat-gateway-telegram-slack-discord-whatsapp-webhook).
...
chatcli
> /gateway start
  OK Gateway rodando em: telegram. Pressione Ctrl+C para parar.
```
