...
every interface inside a cluster, so a spec with no `server.token` and no
`security.jwtSecretRef` would provision a server that refuses to start.
...
`extraEnv` as `CHATCLI_SERVER_TOKEN` or `CHATCLI_JWT_SECRET` counts too.
</Warning>
