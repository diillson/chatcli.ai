...
<Info>
For detailed documentation on each v2 CRD (NotificationPolicy, EscalationPolicy, SLO, SLA, ApprovalPolicy, ApprovalRequest, ClusterRegistration, AuditEvent, ChaosExperiment), see the [AIOps Platform sub-pages](/features/aiops/notifications).
</Info>
...
<Accordion title="Configurable values">
  ...
  | `prometheusUrl` | `""` | Prometheus URL for incident metrics collection (set it here, not on the server chart, where it is deprecated) |
  | `security.allowedDiagnosticCommands` | `""` | Extra read-only diagnostic commands for `ExecDiagnostic` (comma-separated, appended to the built-in allowlist); rendered as `CHATCLI_ALLOWED_DIAGNOSTIC_COMMANDS` on the operator pod |
...
graph TD
    subgraph "ChatCLI Server"
        W[K8s Watcher<br/>collectors] --> GA[StreamAlerts RPC<br/>GetAlerts fallback]
        AI_RPC[AnalyzeIssue RPC<br/>LLM call]
        AG_RPC[AgenticStep RPC<br/>observe-decide-act]
    end

    subgraph "Operator"
        WB[WatcherBridge<br/>open stream] -->|gRPC| GA
...
|-------|-----------|--------------|
| **1. Detection** | WatcherBridge | Receives alerts over the server's `StreamAlerts` stream (polls `GetAlerts` every 30s only for a server without it). Creates Anomaly CRs (dedup SHA256). Invalidates dedup when Issue reaches terminal state. |
...
<Warning>
**An Instance needs a credential.** The server an Instance provisions binds
every interface inside a cluster, so a spec with no `server.token`, no
`security.jwtSecretRef` and no `security.jwtPublicKeyRef` would provision a
server that refuses to start.
The operator does not provision the Deployment in that case — it raises a
condition on the Instance instead:
...
Bind loopback with `security.bindAddress: "127.0.0.1"` when the server is
only reached from inside its own pod. A credential passed through
`extraEnv` as `CHATCLI_SERVER_TOKEN`, `CHATCLI_JWT_SECRET`,
`CHATCLI_JWT_PUBLIC_KEY` or `CHATCLI_SERVER_TLS_CLIENT_CA` counts too.
</Warning>
...
| 4 | `spec.server.security.operatorClientCertSecretName` | its client certificate (`tls.crt` / `tls.key`), presented on every connection; with `spec.server.tls.clientCASecretName` on the server and no bearer credential, the certificate is the whole identity |
...
|-------|------|----------|-------------|
| `jwtSecretRef` | SecretKeyRef | No | HS256 signing secret (`CHATCLI_JWT_SECRET`); the operator also mints its own tokens from it |
| `jwtPublicKeyRef` | SecretKeyRef | No | RSA public key (PEM) the server verifies RS256 tokens with (`CHATCLI_JWT_PUBLIC_KEY`). The operator cannot sign with it: set `operatorTokenRef` too |
| `jwtIssuer` | string | No | Required `iss` claim (`CHATCLI_JWT_ISSUER`); stamped on the tokens the operator mints |
| `jwtAudience` | string | No | Required `aud` claim (`CHATCLI_JWT_AUDIENCE`); stamped on the tokens the operator mints |
| `operatorTokenRef` | SecretKeyRef | No | Credential the operator presents to this server: a JWT issued out of band, or the shared token |
| `mtlsRole` | string | No | Role for callers identified by their client certificate alone (`viewer`, `user`, `operator`, `admin`; `CHATCLI_MTLS_ROLE`, default `user`) |
| `operatorClientCertSecretName` | string | No | TLS Secret (`tls.crt`, `tls.key`) the operator presents as its client certificate when dialing this Instance |
| `rateLimitRps` / `rateLimitBurst` | int32 | No | Per-client rate limit (`CHATCLI_RATE_LIMIT_RPS` / `CHATCLI_RATE_LIMIT_BURST`) |
| `maxRecvMsgSize` / `maxSendMsgSize` / `maxConcurrentStreams` | int32 | No | gRPC message and stream limits |
| `bindAddress` | string | No | Server bind address (unset = every interface in Kubernetes) |

Mutual TLS on the server side lives under `spec.server.tls`: `clientCASecretName` names a Secret whose `ca.crt` is the CA client certificates are verified against (mounted at `/etc/chatcli/client-ca`, passed as `--tls-client-ca`); it requires `enabled: true` and `secretName`, and counts as a credential for the reachable-bind check.

### What is really running

Once the Deployment is ready the operator probes the server with its own transport and credential (`Health`, then `GetServerInfo`) and records the outcome, so `kubectl get instance` shows the version that answered and whether the AIOps pipeline can drive it:
...
| Condition | Meaning |
|---|---|
| `ServerReachable=True`, reason `Serving` | the server answered with its version, provider and model |
| `ServerReachable=False`, reason `ProbeFailed` | transport or credential failure; a message mentioning the credential check means the pods are up but the operator's credential was refused |
| `ServerReachable=False`, reason `NotServing` | the server answered `Health` with `NOT_SERVING` (draining) |
| `ServerReachable=Unknown` | no ready replica yet, nothing probed |

`status.serverVersion` keeps the last known version across a failed probe; `status.serverProbeTime` is when the operator last tried.

The AIOps pipeline books LLM spend from the tokens the server reports on `AnalyzeIssue` and `AgenticStep` (the operator used to estimate analyses from character counts and never booked agentic steps), attributed to the provider and model that actually answered, priced with the same tables as the CLI's `/cost`.

### Features, scheduling and ServiceAccount (typed)

What used to need `spec.extraEnv` now has typed fields:

| Field | Renders | Notes |
|---|---|---|
| `spec.features.memory` (`enabled`, `mode`) | `CHATCLI_MEMORY_ENABLED`, `CHATCLI_MEMORY_MODE` | `mode`: `index`, `pull` or `off` |
| `spec.features.knowledge` | `CHATCLI_CHAT_KNOWLEDGE` | nil keeps the server default |
...
| `spec.features.hub` | `CHATCLI_HUB_ENABLED` | conversation hub |
| `spec.features.caBundleSecretName` | mounts `ca.crt` at `/etc/chatcli/ca`, `CHATCLI_CA_BUNDLE` | corporate CA trusted by every outbound TLS connection |
| `spec.features.allowHTTPProviders` | `CHATCLI_ALLOW_HTTP_PROVIDERS` | off keeps the TLS-only posture |
| `spec.features.encryptionKeyRef` | `CHATCLI_ENCRYPTION_KEY` from the Secret | encryption at rest |
| `spec.features.logRotation` (`maxSizeMB`, `maxBackups`, `maxAgeDays`, `compress`) | `CHATCLI_LOG_*` | |
| `spec.pipeline.enabled` | `CHATCLI_SERVER_PIPELINE` | the [pipeline RPCs](/features/server-mode#pipeline-rpcs) |
| `spec.scheduling` (`nodeSelector`, `tolerations`, `affinity`, `imagePullSecrets`, `priorityClassName`, `podAnnotations`) | pod template | the operator's rollout hashes win over a clashing pod annotation |
| `spec.serviceAccount.annotations` | the managed ServiceAccount | IRSA / Workload Identity; annotations added by other tools are kept |

Only what is set is rendered, so the server defaults keep applying for everything else.
| `auditLogPath` | string | No | Audit log file path (`CHATCLI_AUDIT_LOG_PATH`) |
| `debug` | bool | No | Debug logging |
| `enableReflection` | bool | No | gRPC reflection (debugging only) |
...
| Field | Type | Required | Description |
|-------|------|:--------:|-------------|
| `name` | string | **Yes** | Provider name: OPENAI, OPENAI_ASSISTANT, CLAUDEAI, BEDROCK, GOOGLEAI, XAI, ZAI, MINIMAX, MOONSHOT, OPENROUTER, STACKSPOT, OLLAMA, COPILOT, DEVIN |
...
3. **Fallback** — `latest` when neither is present (e.g., `make deploy` without Helm).
...
| `ExecDiagnostic` | Executes a command from a read-only allowlist inside a pod | `command` (exact string — see [allowlist](#execdiagnostic-allowlist)) |
...
Extend via the `CHATCLI_ALLOWED_DIAGNOSTIC_COMMANDS` env var (comma-separated, read once at startup). The **operator** process reads it, not the server, so set it on the `chatcli-operator` chart with `security.allowedDiagnosticCommands` — an entry in an Instance's `spec.extraEnv` lands on the server pod and has no effect:
...
- **Stream first**: Keeps the server's `StreamAlerts` stream open and turns each alert into an Anomaly the moment it arrives. The stream opens with the alerts active right now, so nothing raised while the bridge was away is lost
- **Liveness**: The server heartbeats every 15 seconds; 60 seconds of silence reopens the stream. Reopens back off from 1s to 30s, and three silent attempts in a row drop the connection so the Instance is rediscovered (address or credentials may have changed)
- **Polling fallback**: A server without `StreamAlerts` (older release) answers `UNIMPLEMENTED`; the bridge then polls `GetAlerts` every 30 seconds and probes the stream again after 10 minutes, so a server upgrade is picked up without an operator restart. `CHATCLI_OPERATOR_ALERT_TRANSPORT=poll` (chart value `alertTransport`) forces polling
- **Resync**: If the server drops the bridge for falling behind (`ABORTED`), the stream is reopened at once with the current snapshot; dedup absorbs the repeats
...
# Unit tests (fake client, reconcilers driven by hand)
go test ./... -v

# Integration tests: the controllers run against a real API server
# (envtest: kube-apiserver + etcd) with the CRDs from config/crd/bases,
# wired exactly as main.go wires them, over an in-memory ChatCLI server.
# The binaries are fetched once into ./bin; without KUBEBUILDER_ASSETS the
# suite skips so a plain `go test ./...` stays green.
make test-integration

# Web dashboard preview: no cluster needed. Serves the embedded dashboard on
# http://127.0.0.1:8085/ over a fake client seeded with synthetic incidents,
# runbooks, SLOs and a pending approval. Log in with the API key "preview".
# Override the address with DASHPREVIEW_ADDR=127.0.0.1:9090.
make dash-preview
...
<Note>
**Two distinct Secrets in this project** — do not confuse it with the LLM provider keys consumed by the chatcli server (`chatcli-api-keys`, referenced via `Instance.spec.apiKeys.name`). See the comparison table in [Security — Operator Authentication](/features/security#operator-authentication).

The `chatcli-operator-secrets` Secret must live in the **same namespace as the operator pod** (the controller resolves it via the `POD_NAMESPACE` env var / ServiceAccount namespace file, falling back to `chatcli-system`). If you ran `helm install --namespace <X>`, create the Secret in `<X>`.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Server Mode" icon="server" href="/features/server-mode">
    StreamAlerts, GetAlerts and AnalyzeIssue RPCs
  </Card>
