...
| --- | --- | --- |
| **Plan** | `<plan>...</plan>` | Displayed as a compass-icon card — the actions the AI will take (the older `<reasoning>` spelling is still read) |
...
- **Type-ahead queue** — Messages typed by the user during AI processing are queued and processed once the current turn finishes. This prevents input loss. Works in **both /agent AND /coder**. The spinner shows `(N queued)` in real time as you press Enter — no need to wait for the turn to close to see the counter rise. A queued instruction is never lost on the way: it survives the final-answer prompt (if the model replies without tool calls while an instruction is pending, the coder skips the "waiting for input" prompt and goes straight to the next turn), interactive overlays such as `@ask`, and the security-prompt input guard. What you were typing without pressing Enter carries over into the editable prompt. The model receives the follow-up labeled as a new user instruction that takes priority over its current plan. If the run ends (max turns, error) with instructions still queued, they are printed back so you can resend them.
...
<Tabs>
  ...
  <Tab title="permissive">
    The allowlist is applied first. Commands outside it fall through to the **legacy denylist** (50+ regex patterns) as a fallback.
...
<Tip>
For a complete overview of all security measures, see the [Security and Hardening documentation](/features/security).
</Tip>
...
| `CHATCLI_AGENT_<NAME>_MODEL` | _(empty)_ | Model override for a specific built-in agent (e.g., `CHATCLI_AGENT_PLANNER_MODEL=claude-opus-4-6`). See [Multi-Agent](/features/multi-agent-orchestration#configuration). |
| `CHATCLI_AGENT_<NAME>_EFFORT` | _(empty)_ | Effort override for a specific built-in agent (e.g., `CHATCLI_AGENT_FORMATTER_EFFORT=low`). |
...
Details in [Session Workspace](/features/session-workspace) and [Subagent Delegation](/features/subagent-delegation).
...
<CardGroup cols={2}>
  ...
  <Card title="Session Management" icon="floppy-disk" href="/features/session-management">
