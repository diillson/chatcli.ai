...
## UI do Terminal (Renderers)

O `/coder` tem **dois eixos independentes** de configuração de UI, controlados por env vars distintas:

| Eixo | Variável | O que controla |
| --- | --- | --- |
| Estilo dos cards | [`CHATCLI_CODER_UI`](/features/compact-ui) | `full` / `minimal` / `compact` — quanto detalhe cada tool call/resposta ocupa na tela |
| Arquitetura do terminal | [`CHATCLI_TURNUI`](/features/split-pane-ui) | `on` ativa split-pane (status fixo + input fixo + scroll region); vazio = single-line spinner legado |

Os dois são **ortogonais** — qualquer combinação é válida. A escolha mais minimalista (estilo Claude Code) é `CHATCLI_CODER_UI=compact CHATCLI_TURNUI=on`. Veja [UI Compacta](/features/compact-ui) e [UI Split-Pane](/features/split-pane-ui) para detalhes de cada uma.

<Warning>
`CHATCLI_TURNUI` é **opt-in** porque depende de o terminal honrar DECSTBM + cursor save/restore. Funciona em iTerm2, Terminal.app, Hyper, Alacritty, Windows Terminal; **não funciona** no terminal integrado do GoLand. Quando não ativado, o renderer legado segue funcionando como sempre.
</Warning>
...
A lista é compartilhada com o TaskTracker via `liveTodoAdapter`, então o checkbox UI renderiza o mesmo plano que aparece em `<reasoning>`. Detalhes em [Atomic Tools](/features/atomic-tools).
...
| `delegate --prompt "..." --tools [...]` | Delegar sub-tarefa para um subagente com contexto isolado — ver [Subagent Delegation](/features/subagent-delegation) |

<Tip>
O modo `/coder` também recebe automaticamente a variável `CHATCLI_AGENT_TMPDIR` apontando para o [scratch dir da sessão](/features/session-workspace). É o caminho recomendado para criar scripts temporários e arquivos intermediários sem poluir o projeto.
</Tip>
...
Se a resposta for complexa, use o [modo multilinha](/features/multiline-input) com `---` (também aceita `` ``` `` como abridor de fence):
